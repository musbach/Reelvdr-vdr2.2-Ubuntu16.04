...
int progressUpdate(int starttime, int cur, int tot, int force)
{
  ...
  if (progress_time == 0 || progress_time != time(NULL) || force)
  {
      ...
      if (cols < 25)
          return 1; // too narrow; cannot proceed
      if (cols > 100)
          cols = 100; // too wide; stop at 100 char lengths
...
#if 0
      // barChar can be 0 for small files! because int division of tot / barLen can be 0
      numChars = cur / barChar;
#else
      numChars = (int)(barLen*((float)cur/tot));
#endif
...
#else
      if (cur == 0)
          timeLeft = 0; // cannot estimate remaining time
      else
          timeLeft = (int)(((float)timeSoFar*(tot-cur))/cur);
#endif
