...
//#define fb_ehd_dbg(format, arg...) printk(FB_DEVICE_NAME": "format"\n", ## arg)
#define fb_ehd_dbg(format, arg...)
#define fb_ehd_err(format, arg...) printk(FB_DEVICE_NAME": "format"\n", ## arg)
...
int get_hd_data (struct fb_info *info) {
	...
	if(!info)
	    return -EINVAL;  
	fb_ehd_dbg("get_hd_data %p", fb->hd_data);
	if (fb->hd_data) 
	    return 0;
	if (hdshm_lock_table()) {
	    fb_ehd_err("dev_open: lock_tables timed out");
	    return -ETIMEDOUT;
	} // if
	bse=hdshm_find_area(HDID_HDA, NULL);
	if (!bse) {
	    fb_ehd_err("dev_open: area not found");
	    hdshm_unlock_table();
	    return -ENOENT;
	} // if
	fb->hd_data = (hd_data_t *)ioremap((unsigned long)hdd.bar1+GO_BASE+bse->phys, bse->length); /* Virtual address */
	fb_ehd_dbg("dev_open %dx%d 0x%x 0x%x", fb->hd_data->video_mode.width, fb->hd_data->video_mode.height, fb->hd_data->aspect.scale, bse->phys);
	hdshm_unlock_table();
	return 0;
} // get_hd_data
...
{		
	...
	fb_ehd_dbg("update_fbinfo %i %i %x\n", w,h,xm);
...
int set_pic_mode(struct fb_info *info, int w, int h, int xm) {
	...
	fb_ehd_dbg("set_pic_mode %ix%i, %x", w, h, xm);
...
int dev_check_var(struct fb_var_screeninfo *var, struct fb_info *info) {
	...
	fb_ehd_dbg("check_var w %d h %d vw %d vh %d xo %d yo %d bps %d ww %d hh %d", 
...
int dev_set_par (struct fb_info *info) {
	...
	fb_ehd_dbg("dev_set_par");
	if(!var)
		return EINVAL;

	fb_ehd_dbg("set_var w %d h %d vw %d vh %d xo %d yo %d bps %d ww %d hh %d", 
	       var->xres, var->yres, var->xres_virtual, var->yres_virtual, 
	       var->xoffset, var->yoffset, var->bits_per_pixel, var->height, var->width);
...
//	fb_ehd_dbg("dev_pan_display");
//	fb_ehd_dbg("pan  w %d h %d vw %d vh xo %d yo %d bps %d %d ww %d hh %d", 
...
int dev_mmap(struct fb_info *info, struct vm_area_struct *vma) {
	...
	fb_ehd_dbg("dev_mmap");
	if (!vma || !info) {
		fb_ehd_err("mmap: invalid args %p %p", info, vma);
		return -EINVAL;
	} // if
	if (!(vma->vm_flags & VM_SHARED)) {
		fb_ehd_err("mmap: only support VM_SHARED mapping");
		return -EINVAL; 
	} // if
 	if (vma->vm_pgoff != 0) {
		fb_ehd_err("mmap: only support offset=0 (%ld)", vma->vm_pgoff);
		return -EINVAL; 
	} // if
	size = vma->vm_end - vma->vm_start;
	if (size > info->fix.smem_len) {
		fb_ehd_err("mmap: requested size to big 0x%x (0x%x)", size, info->fix.smem_len);
		return -EINVAL; 
	} // if
	fb_ehd_dbg("remap_pfn_range> 0x%lx, 0x%lx, %d (0x%x)", vma->vm_start, info->fix.smem_start, size, size);
...
int hdfb_init(void) {
	int ret;
	fb_ehd_dbg("init %d", has_fb);
	if (!has_fb)
		return 0;
	ret = init_fb_info();
	if(ret < 0) {
		fb_ehd_err("Unable to init fb_info: %d", ret);
        return ret;
	} // if
	...
	if(ret < 0) {
		fb_ehd_err("Unable to register framebuffer: %d", ret);
        return -EINVAL;
	} // if
	return ret;
} // hdfb_init
