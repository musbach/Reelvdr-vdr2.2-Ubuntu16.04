...
mount_policy (void)
{
	...
	NIH_LIST_FOREACH (mounts, iter) {
		...
		case TAG_SKIPPED:
			...
			break;
		case TAG_NOFAIL:
			nih_info (_("%s is %s"), MOUNT_NAME (mnt), "nofail");
			break;
		default:
	   ...
	   Tag    tag)
{
	...
	if (tag == TAG_UNKNOWN) {
		...
		} else if (has_option (mnt, "nofail", FALSE)) {
				tag = TAG_NOFAIL;
		} else {
			if (! has_option (mnt, "nobootwait", FALSE)) {
				tag = TAG_LOCAL;
...
plymouth_update (int only_clear)
{
	...
	if (plymouth_error != ERROR_NONE) {
		...
		if ((plymouth_error == ERROR_FSCK_IN_PROGRESS)
		    && fsck_in_progress)
			return;
		if ((plymouth_error == ERROR_BORED) && plymouth_mnt && (plymouth_mnt->tag == TAG_NOFAIL))  { // added by RC
			nih_error (_("Skipping mounting %s since it is marked \"nofail\" in fstab"),
				MOUNT_NAME (plymouth_mnt));
			plymouth_mnt->error = ERROR_NONE;
			skip_mount (plymouth_mnt);
			return;
		}
