...
namespace tut
{
  ...
  struct test_result
  {
     ...
     * Constructor.
     */
    test_result( const std::string& grp,int pos,result_type res, long long t )
		: group(grp),test(pos),result(res),time(t)
    {
    }
    ...
    test_result( const std::string& grp,int pos,
      ...
      : group(grp),test(pos),result(res),
		message(ex.what()),exception_typeid(typeid(ex).name()), time(0)
    {
    }
  };
  ...
  private:
    ...
    test_result run_test_(const tests_iterator& ti,safe_holder<object>& obj)
    {
      ...
      try
      {
        if( run_test_seh_(ti->second,obj, c) == false )
          throw seh("seh");
      }
      catch(const no_such_test&)
      {
      ...
      catch(...)
      {
        ...
        test_result tr(name_,ti->first,test_result::ex, 0);
        return tr;
      }

      // test passed
      test_result tr(name_,ti->first,test_result::ok, c);
      return tr;
    }
     ...
     */
    bool run_test_seh_(testmethod tm,safe_holder<object>& obj, long long &c)
    {
#if defined(TUT_USE_SEH)
      __try
      {
        ...
        __try
        {
          ...
          timespec t;
          t.tv_nsec = 0; t.tv_sec = 0; c = 0;
          clock_gettime( CLOCK_MONOTONIC, &t );
          c = t.tv_nsec / 10000 + t.tv_sec * 100000;
          (obj.get()->*tm)();
          clock_gettime( CLOCK_MONOTONIC, &t );
          c = t.tv_nsec / 10000 + t.tv_sec * 100000 - c;
#if defined(TUT_USE_SEH)
        }
        __except(handle_seh_(::GetExceptionCode()))
        {
