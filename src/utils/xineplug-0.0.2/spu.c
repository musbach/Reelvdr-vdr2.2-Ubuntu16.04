...
#include <xineutils.h>
//#include "reel_video_overlay.h"
#include "spu.h"
#include <buffer.h>
//#include <bswap.h>
#include <i386-linux-gnu/libavutil/bswap.h>
//#ifdef HAVE_DVDNAV
#  include <nav_read.h>
#  include <nav_print.h>
//#else
...
void my_spudec_decode_nav(spudec_decoder_t *this, buf_element_t *buf) {
  uint8_t                  *p;
  uint32_t                  packet_len;
  uint32_t                  stream_id;
  uint32_t                  header_len;
  ...
  switch (pci.hli.hl_gi.hli_ss) {
    case 0:
	  ...
	  this->event.vpts = 0;   
          ...
          //Start
          //printf("----------ovl_manager->add_event(ovl_manager, (void *)&this->event)--------\n"); 
          //video_overlay_print_overlay(this->event.object.overlay);
          //video_overlay_print_event(&(this->event));
          SpuPutEvents(&(this->event), this);
          //End
	...
	xine_fast_memcpy(&node->next->pci, &pci, sizeof(pci_t));
        ...
        xine_fast_memcpy(&this->pci_cur.pci, &pci, sizeof(pci_t));
        my_spudec_process_nav(this);
      }
      break;
    case 2:
      /* Use Highlight information from previous VOBU */
      //Start
      //printf("---------vor if (this->pci_cur.next)----------------\n");
      //End 
      if (this->pci_cur.next) { 
        //Start
        //printf("---------nach if (this->pci_cur.next)----------------\n");
        //End 
...
void my_spudec_clear_nav_list(spudec_decoder_t *this)
{
...
void my_spudec_update_nav(spudec_decoder_t *this)
{
...
void my_spudec_process_nav(spudec_decoder_t *this)
{
...
void my_spudec_reassembly (xine_t *xine, spudec_seq_t *seq, uint8_t *pkt_data, u_int pkt_len)
{
#ifdef LOG_DEBUG
...
void my_spudec_process (spudec_decoder_t *this, int stream_id) {
  ...
  do {
	...
	if (node->pci.hli.hl_gi.hli_s_ptm == this->spudec_stream_state[stream_id].pts)
...
#endif
        if (!my_spudec_copy_nav_to_overlay(this->stream->xine, 
	  ...
	  event.data_length = sizeof(this->buttonN);
          xine_event_send(this->stream, &event);
	  my_spudec_copy_nav_to_overlay(this->stream->xine, 
				     &node->pci, this->state.clut, 
				     this->buttonN, 0, &this->overlay, &this->overlay);
        }
      } else {
      ...
      */
      ...
      //Start
      //printf("----------ovl_manager->add_event(ovl_manager, (void *)&this->event) 2--------\n");
      //video_overlay_print_overlay(this->event.object.overlay); 
      //video_overlay_print_event(&(this->event));
      SpuPutEvents(&(this->event), this);
      //End
...
int my_spudec_copy_nav_to_overlay(xine_t *xine, pci_t* nav_pci, uint32_t* clut,
