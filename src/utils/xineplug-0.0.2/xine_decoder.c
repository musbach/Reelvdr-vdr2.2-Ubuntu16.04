...
static void spudec_decode_data (spu_decoder_t *this_gen, buf_element_t *buf) {
  ...
  my_spudec_reassembly(this->stream->xine,
  ...
  if(this->spudec_stream_state[stream_id].ra_seq.complete == 1) { 
    if(this->spudec_stream_state[stream_id].ra_seq.broken) {
      ...
      my_spudec_process(this,stream_id);
  }
}
...
#endif 
    my_spudec_copy_nav_to_overlay(this->stream->xine, &this->pci_cur.pci, this->state.clut,
...
#endif 
    //Start
    //printf("----------ovl_manager->add_event (ovl_manager, (void *)overlay_event)--------\n");
    //video_overlay_print_overlay(overlay_event->object.overlay);
    //video_overlay_print_event(overlay_event);
    SpuPutEvents(overlay_event, this);
    //End
...
//  this->decoder_class.get_description = get_description;
  this->decoder_class.identifier  = get_identifier;
  this->decoder_class.description = get_description;
