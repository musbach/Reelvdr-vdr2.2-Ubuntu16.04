...
#else
	fprintf(stderr,"hdctrld: HDID_HDA=%d \n",HDID_HDA);
	hsa=hd_get_area(HDID_HDA);
//	fprintf(stderr,"hdctrld:      hsa=%d \n",hsa);
...
/* --------------------------------------------------------------------- */
void start_decypher(void) {
	pid_t parent = getpid();
	pid_t pid = vfork();
	int exit_code;

	fprintf(stderr,"start_decypher: DeCypher HDSHM not running!\n");

	if(pid < 0) {
		fprintf(stderr,"start_decypher: fork failed!\n");
		exit(1);
		return;
	}else if(pid == 0){
		execl("/etc/init.d/decypher start","start_decypher","start",(char*)0);
		fprintf(stderr,"start_decypher: execl failed!\n");
		_exit(1);
	}else{
		fprintf(stderr,"start_decypher: process is waiting!\n");
//		wait(&exit_code);
		waitpid(pid,&exit_code,0);
		if(WIFEXITED(exit_code)) {
			fprintf(stderr,"start_decypher: wait failed!\n");
			return -1;
		} else {
			fprintf(stderr,"start_decypher: wait finshed successful!\n");
			return 0;
		}
	}
}
