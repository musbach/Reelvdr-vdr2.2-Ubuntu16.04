...
// satellit hack
    printf("hd_get_area: get_area ID %x\n", id);
    dbg1("HOST: get_area ID %x\n", id);
//    printf("hd_get_area:        &bah %i\n", &bah);
    real_length=sizeof(bah);
    printf("hd_get_area: sizeof(bah) %d\n", real_length);
    memset(&bah, 0, sizeof(bah));
//    printf("hd_get_area: after   bah %i\n", bah);
    bah.id = id;
//    printf("hd_get_area:        &bah %i\n", &bah);
    printf("hd_get_area:        hdfd %d\n", hdfd);
    printf("hd_get_area: IOCTL_HDSHM_GET_AREA %u\n", IOCTL_HDSHM_GET_AREA);
    ret = ioctl(hdfd, IOCTL_HDSHM_GET_AREA, &bah);
    printf("hd_get_area:         ret %d\n", ret);
//    printf("hd_get_area:        &bah %i\n", &bah);
    printf("hd_get_area:        hdfd %d\n", hdfd);
    printf("hd_get_area: IOCTL_HDSHM_GET_AREA %u\n", IOCTL_HDSHM_GET_AREA);
    printf("hd_get_area: errno      %s \n",strerror(errno));
...
//    int ret;
    ...
    dbg1("ioctl unmap\n");
    ioctl(hdfd, IOCTL_HDSHM_UNMAP_AREA, handle->id);
    handle->mapped = NULL;
}
...
//    int ret;
    ...
    ioctl(hdfd, IOCTL_HDSHM_GET_AREA, &bah1);
...
{
    printf("hd_init:        hdfd %d\n", hdfd);
    if (hdfd == -1)
    {
        printf("hd_init:    SHMHDDEV %s\n", SHMHDDEV);
        hdfd = open(SHMHDDEV, O_RDWR);
        printf("hd_init: errno is %d and hdfd is %d\n",errno,hdfd);
        printf("hd_init:        hdfd %d\n", hdfd);
        if (hdfd == -1)
            return -1;
    }
