...
int cPngUtils::ReadJpeg(char *sPath)
{
...
int cPngUtils::WriteFile ( unsigned char nTransparenz, int xpos, int ypos, char *sPath, bool bHasTransparency, int rel_width, int rel_height, int rel_rowsize )
{
...
bool cPngUtils::PngIsReadable(char *path)
{
    Byte header[8];
...
bool cPngUtils::ReadPng(char *path)
{
    png_structp png_ptr;
    png_infop   info_ptr;
...
bool cPngUtils::ReadRawBitmap(char *path)
{
   long nPixel;
   imgheader h;
...
int cPngUtils::DumpRaw ( char *sPath, int step )
{
    FILE *outfile;
    long nPixel;
