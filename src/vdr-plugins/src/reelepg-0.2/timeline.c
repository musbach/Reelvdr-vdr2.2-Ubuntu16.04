...
//#include "../epgsearch/services-reel4.h"
#include <vdr/menu.h>
...
//					cTimer *lTimer = Timers.GetMatch(mEvent, &lMatch);
					cTimer *lTimer = Timers.GetMatch(mEvent, (eTimerMatch *)lMatch);
					if (lMatch!=tmFull)
						lTimer = NULL;
...
case k1       : setScale(1, gReelEPGCfg.mScaleHeight); lState = osContinue; break;
		...
		case kRed     : recordEvent(); lState = osContinue; break; // doesn't work!!!
...
//			if (Timers.GetMatch(mSelEvent, &lMatch) && (lMatch==tmFull))
			if (Timers.GetMatch(mSelEvent, (eTimerMatch *)lMatch) && (lMatch==tmFull))
				lRed = tr("Edit");
...
void cTimeLine::recordEvent() {
	...
	isyslog("Information: cTimeLine::recordEvent()");
	cPlugin *lPlugin = cPluginManager::GetPlugin("epgsearch");
	isyslog("Information: cTimeLine::recordEvent() after epgsearch");
	if(!lPlugin)
		return;
	isyslog("Information: cTimeLine::recordEvent() after lPlugin");
	if(!mSelEvent)
		return;
	isyslog("Information: cTimeLine::recordEvent() after mSelEvent");
	if (mSelEvent->EndTime() < time(NULL)) {
		isyslog("Information: cTimeLine::recordEvent() in if <time(NULL)");
		deleteOsd();
		isyslog("Information: cTimeLine::recordEvent() in if after deleteOSD");
		...
		isyslog("Information: cTimeLine::recordEvent() in if after Epgsearch");
		if(!lPlugin->Service("Epgsearch-search-v1.0",(void *)&lData) || !lData.pResultMenu) {
			createOsd();
			return;
		 } // if
		isyslog("Information: cTimeLine::recordEvent() in if after 2nd if");
		mMenu = lData.pResultMenu;
		mMenu->Display();
	} else {
		isyslog("Information: cTimeLine::recordEvent() in else <time(NULL)");
		int  lMatch = tmNone;
//		cTimer *lTimer = Timers.GetMatch(mSelEvent, &lMatch);
		cTimer *lTimer = Timers.GetMatch(mSelEvent, (eTimerMatch *)lMatch);
		isyslog("Information: cTimeLine::recordEvent() in else after cTimer");
		if (lMatch!=tmFull)
			isyslog("Information: cTimeLine::recordEvent() in else in 2nd if");
			lTimer = NULL;
		deleteOsd();
		isyslog("Information: cTimeLine::recordEvent() in else after deleteOSD");
		Epgsearch_exttimeredit_v1_0 lData = { lTimer?lTimer:new cTimer(mSelEvent), !lTimer, mSelEvent};
		isyslog("Information: cTimeLine::recordEvent() in else after Epgsearch_exttimeredit_v1_0");
		if(!lPlugin->Service("Epgsearch-exttimeredit-v1.0",(void *)&lData) || !lData.pTimerMenu) {
			isyslog("Information: cTimeLine::recordEvent() in else in 3rd if");
			createOsd();
			isyslog("Information: cTimeLine::recordEvent() in else in 3rd if after createOSD");
			return;
		 } // if
		mMenu = lData.pTimerMenu;
		isyslog("Information: cTimeLine::recordEvent() in else after mMenu");
		mMenu->Display();
		isyslog("Information: cTimeLine::recordEvent() in else after mMenu->Display");
	} // if
