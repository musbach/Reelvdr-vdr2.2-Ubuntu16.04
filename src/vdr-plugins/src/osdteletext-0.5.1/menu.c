...
//static variables
int
    TeletextBrowser::currentPage = 0x100;       //Believe it or not, the teletext numbers are somehow hexadecimal
int
    TeletextBrowser::currentSubPage = 0;
tChannelID
    TeletextBrowser::channel;
int
    TeletextBrowser::currentChannelNumber = 0;
TeletextBrowser *
    TeletextBrowser::self = 0;
...
void
TeletextBrowser::ChannelSwitched(int ChannelNumber)
{
    ...
    IntMap::iterator it;
    //channelPageMap.insert(IntMap::value_type(currentChannelNumber, currentPage));
...
eOSState
TeletextBrowser::ProcessKey(eKeys Key)
{
    switch (Key)
    {
    case k1:
    ...
    case kNone:                //approx. every second
        ...
        if (pageFound && ttSetup.autoUpdatePage && cursorPos == 0
            && !selectingChannel && (PageCheckSum() != checkSum))
        {
            ShowPage();
        }
        ...
        if (ttSetup.inactivityTimeout
            && (time(NULL) - lastActivity > ttSetup.inactivityTimeout * 60))
            return osEnd;
        break;
        break;
    case kUp:
...
void
TeletextBrowser::ExecuteAction(eTeletextAction e)
{
    switch (e)
    {
    case Zoom:
    ...
    case DarkScreen:
        if (Display::GetBackgroundColor() == clrBlack)
            Display::SetBackgroundColor((tColor) ttSetup.
                                        configuredClrBackground);
        else
...
eTeletextAction
TeletextBrowser::TranslateKey(eKeys Key)
{
    switch (Key)
    {
    case kRed:
        return (eTeletextAction) ttSetup.mapKeyToAction[ActionKeyRed];
    case kGreen:
        return (eTeletextAction) ttSetup.mapKeyToAction[ActionKeyGreen];
    case kYellow:
        return (eTeletextAction) ttSetup.mapKeyToAction[ActionKeyYellow];
    case kBlue:
        return (eTeletextAction) ttSetup.mapKeyToAction[ActionKeyBlue];
    case kPlay:
        return (eTeletextAction) ttSetup.mapKeyToAction[ActionKeyPlay];
        //case kPause:   return (eTeletextAction)ttSetup.mapKeyToAction[ActionKeyPause];
    case kStop:
        return (eTeletextAction) ttSetup.mapKeyToAction[ActionKeyStop];
        //case kRecord:   return (eTeletextAction)ttSetup.mapKeyToAction[ActionKeyRecord];
    case kFastFwd:
        return (eTeletextAction) ttSetup.mapKeyToAction[ActionKeyFastFwd];
    case kFastRew:
        return (eTeletextAction) ttSetup.mapKeyToAction[ActionKeyFastRew];
    default:
        return (eTeletextAction) 100;   //just to keep gcc quiet
    }
}


void
TeletextBrowser::SetNumber(int i)
{
    ...
    if (i < 0)
    {
        for (; i < 0; i++)
        {
            ...
            case 1:
                currentPage =
                    currentPage - 256 * GET_HUNDREDS(currentPage) +
                    256 * GET_HUNDREDS(pageBeforeNumberInput);
                break;
            case 2:
                currentPage =
                    currentPage - 16 * GET_TENS(currentPage) +
                    16 * GET_TENS(pageBeforeNumberInput);
                break;
            }
            cursorPos--;
        }
...
static inline bool
onlyDecimalDigits(int x)
{
    return ((x & 0xE) < 0xA) &&
        (((x >> 4) & 0xE) < 0xA) && (((x >> 8) & 0xE) < 0xA);
}

//after 199 comes 1A0, but if these pages exist, they contain no useful data, so filter them out
int
TeletextBrowser::nextValidPageNumber(int start, bool direction)
{
    do
    {
        if (direction)
...
void
TeletextBrowser::ChangePageRelative(bool back)
{
    ...
    do
    {
        currentPage = nextValidPageNumber(currentPage, !back);
        if (currentPage > 0x899)
            currentPage = 0x100;
        if (currentPage < 0x100)
            currentPage = 0x899;
...
void
TeletextBrowser::ChangeSubPageRelative(bool back)
{
    int oldsubpage = currentSubPage;

    do
    {
        currentSubPage = nextValidPageNumber(currentSubPage, !back);
        if (currentSubPage > 0x99)
            currentSubPage = 0;
        if (currentSubPage < 0)
            currentSubPage = 0x99;
...
bool
TeletextBrowser::CheckFirstSubPage(int startWith)
{
    int oldsubpage = currentSubPage;
    ...
    do
    {
        ...
        //currentSubPage++;
        currentSubPage =
            nextValidPageNumber(currentSubPage, DirectionForward);
...
TeletextBrowser::CheckPage()
{
    ...
    if (!
        (fd =
         s->openForReading(PageID(channel, currentPage, currentSubPage),
                           false)))
        return false;
...
void
TeletextBrowser::SetPreviousPage(int oldPage, int oldSubPage, int newPage)
{
    if (oldPage != newPage)
    {
...
void
TeletextBrowser::ShowPageNumber()
{
    char str[8];
...
void
TeletextBrowser::ShowAskForChannel()
{
    if (selectingChannel)
    {
        char *str;
...
TeletextBrowser::DecodePage()
{
    ...
    else
    {
        ...
        ShowPageNumber();
        char str[80];
        snprintf(str, 80, "%s %3x-%02x %s", tr("Page"), currentPage,
                 currentSubPage, tr("not found"));
        Display::DrawMessage(str);
        Display::ReleaseFlush();
...
void
ChannelStatus::ChannelSwitch(const cDevice * Device, int ChannelNumber)
{
