...
int cScCiAdapter::Read(unsigned char *Buffer, int MaxLength)
{
  ...
  if(cam && rb && Buffer && MaxLength>0) {
    ...
    if(data) {
      ...
      else PRINTSCF(L_GEN_DEBUG,"internal: sc-ci %d rb frame size exceeded %d",cardIndex,s);
      frame.Del();
...
:stall(MAX_STALL_MS)
{
  ...
  PRINTSCF(L_CORE_CSA,"%d: batch_size=%d", devId, cs);
...
#endif
  {
    PRINTSCF(L_CORE_CSA,"%s: Error allocating memory for DeCSA", __FUNCTION__);
    return false;
  }
...
/*
#if APIVERSNUM >= 10711
cScDvbDevice::cScDvbDevice(int Adapter, int Frontend, int cafd) :cDvbDevice(Adapter,Frontend)
#else
cScDvbDevice::cScDvbDevice(int n,                     int cafd) :cDvbDevice(n)
#endif
{
...
//    cScDvbDevice *dev=dynamic_cast<cScDvbDevice *>(cDevice::GetDevice(n));
    cDvbDevice *dev=dynamic_cast<cDvbDevice *>(cDevice::GetDevice(n));
//    if(dev) dev->cScDvbDevice.EarlyShutdown(); //to be checked
    }
}
...
//    cScDvbDevice *dev=dynamic_cast<cScDvbDevice *>(cDevice::GetDevice(n));
    cDvbDevice *dev=dynamic_cast<cDvbDevice *>(cDevice::GetDevice(n));
//    if(dev) dev->LateInit(); //to be checked
    }
}
...
#if HACK <= 0
  return true;
#else //APIVERSNUM >= 10711
  if(!vdr_nci || !vdr_ud) {
    PRINTSCF(L_GEN_ERROR,"Failed to locate VDR symbols. Plugin not operable");
    return false;
    }
  if(NumDevices()>0) {
    PRINTSCF(L_GEN_ERROR,"Number of devices != 0 on init. Put SC plugin first on VDR commandline! Aborting.");
    return false;
    }
...
#else // not REEL_AVANTGARDE 
  for(i=0; i<MAX_DVBDEVICES; i++) {
#endif // not REEL_AVANTGARDE
    if(UseDevice(NextCardIndex())) {
      char name[128];
      DvbName(DEV_DVB_FRONTEND,i,name,sizeof(name));
      if(access(name,F_OK)==0) {
        PRINTSCF(L_GEN_DEBUG,"probing %s",name);
        int f=open(name,O_RDONLY);
        if(f>=0) {
          close(f);
          PRINTSCF(L_GEN_DEBUG,"capturing device %d",i);
#ifdef REEL_AVANTGARDE	  
//          new cScDvbDevice(i,-1);
          new cDvbDevice(i,-1); // to be checked
#else //REEL_AVANTGARDE
          new cScDvbDevice(i,DvbOpen(DEV_DVB_CA,i,O_RDWR));
#endif //REEL_AVANTGARDE
          found++;
          }
        else {
          if(errno!=ENODEV && errno!=EINVAL) PRINTSCF(L_GEN_ERROR,"open %s failed: %s",name,strerror(errno));
          break;
          }
        }
      else {
        if(errno!=ENOENT) PRINTSCF(L_GEN_ERROR,"access %s failed: %s",name,strerror(errno));
        break;
        }
      }
...
#else //REEL_AVANTGARDE
  NextCardIndex(MAX_DVBDEVICES-i);
#endif //REEL_AVANTGARDE 
  if(found>0) PRINTSCF(L_GEN_INFO,"captured %d video device%s",found,found>1 ? "s" : "");
  else PRINTSCF(L_GEN_INFO,"no DVB device captured");
#ifdef MCLISC
   return true;
#else //MCLISC
  return found>0;
...
//  fd_dvr=DvbOpen(DEV_DVB_DVR,adapter,frontend,O_RDONLY|O_NONBLOCK,true);
  fd_dvr=DvbOpen(DEV_DVB_DEMUX,adapter,frontend,O_RDWR|O_NONBLOCK); // to be checked
#else
  ...
  if(fd_dvr>=0) {
    tsMutex.Lock();
#ifdef RBMINI
    tsBuffer=new cDeCsaTSBuffer(fd_dvr,MEGABYTE(16),CardIndex()+1,decsa,ScActive());
#else
