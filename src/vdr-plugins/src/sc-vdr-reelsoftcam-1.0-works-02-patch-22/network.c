  ...
  while(Running()) {
    ...
    for(cNetSocket *so=socks.First(); so;) {
      ...
      if(so->connected && so->activity.TimedOut()) {
        PRINTSCF(L_CORE_NET,"idle timeout, disconnected %s:%d",so->hostname,so->port);
        so->Disconnect();
        }
      so->Unlock();
      so=next;
...
bool cNetSocket::GetAddr(struct sockaddr_in *saddr, const char *Hostname, int Port)
{
  ...
  PRINTSCF(L_GEN_ERROR,"socket: name lookup error for %s",Hostname);
  return false;
}
  ...
  if((so=socket(PF_INET,Udp?SOCK_DGRAM:SOCK_STREAM,proto))>=0) {
    ...
    if(flgs>=0) {
      if(fcntl(so,F_SETFL,flgs|O_NONBLOCK)==0)
        return so;
      else PRINTSCF(L_GEN_ERROR,"socket: fcntl SETFL failed: %s",*StrError(errno));
      }
    else PRINTSCF(L_GEN_ERROR,"socket: fcntl GETFL failed: %s",*StrError(errno));
    close(so);
    }
  else PRINTSCF(L_GEN_ERROR,"socket: socket failed: %s",*StrError(errno));
  return -1;
}
...
bool cNetSocket::Connect(const char *Hostname, int Port, int timeout)
{
  ...
  if(GetAddr(&socketAddr,hostname,port) && (sd=GetSocket(udp))>=0) {
    if(!quietlog) PRINTSCF(L_CORE_NET,"connecting to %s:%d/%s (%d.%d.%d.%d)",
    ...
    if(r==0) connected=true;
      ...
      if(Select(false,timeout)>0) {
        ...
        if(getsockopt(sd,SOL_SOCKET,SO_ERROR,&r,&l)==0) {
          if(r==0)
            connected=true;
          else PRINTSCF(L_GEN_ERROR,"socket: connect failed (late): %s",*StrError(r));
          }
        else PRINTSCF(L_GEN_ERROR,"socket: getsockopt failed: %s",*StrError(errno));
        }
      else PRINTSCF(L_GEN_ERROR,"socket: connect timed out");
      }
    else PRINTSCF(L_GEN_ERROR,"socket: connect failed: %s",*StrError(errno));
...
bool cNetSocket::Bind(const char *Hostname, int Port)
{
  ...
  struct sockaddr_in socketAddr;
  if(GetAddr(&socketAddr,hostname,port) && (sd=GetSocket(udp))>=0) {
    if(!quietlog) PRINTSCF(L_CORE_NET,"socket: binding to %s:%d/%s (%d.%d.%d.%d)",
               hostname,port,udp?"udp":"tcp",
               (socketAddr.sin_addr.s_addr>> 0)&0xff,(socketAddr.sin_addr.s_addr>> 8)&0xff,(socketAddr.sin_addr.s_addr>>16)&0xff,(socketAddr.sin_addr.s_addr>>24)&0xff);
    int r;
    ...
    else PRINTSCF(L_GEN_ERROR,"socket: bind failed: %s",*StrError(errno));
    }
  Unlock();
  Disconnect();
  return false;
...
int cNetSocket::Read(unsigned char *data, int len, int timeout)
{
  ...
  if(len<0) { len=-len; blockmode=false; }
    ...
    PRINTSCF(L_GEN_DEBUG,"internal: zero length on socket read");
    errno=EINVAL;
    return -1;
    }
  int cnt=0, r;
  ...
  do {
    ...
    if(r>0) {
      ...
      if(r<0) PRINTSCF(L_GEN_ERROR,"socket: read failed: %s",*StrError(errno));
      else if(r>0) cnt+=r;
      else {
        PRINTSCF(L_GEN_ERROR,"socket: EOF on read");
        errno=ECONNRESET;
        }
      }
...
int cNetSocket::Write(const unsigned char *data, int len, int timeout)
{
  ...
  do {
    ...
    if(r>0) {
      ...
      if(r<0) PRINTSCF(L_GEN_ERROR,"socket: write failed: %s",*StrError(errno));
      else if(r>0) cnt+=r;
      }
    } while(r>0 && cnt<len);
  Activity();
  ...
  if(GetAddr(&saddr,Host,Port)) {
    ...
    do {
      ...
      if(r>0) {
        ...
        if(r<0) PRINTSCF(L_GEN_ERROR,"socket: sendto %d.%d.%d.%d:%d failed: %s",(saddr.sin_addr.s_addr>> 0)&0xff,(saddr.sin_addr.s_addr>> 8)&0xff,(saddr.sin_addr.s_addr>>16)&0xff,(saddr.sin_addr.s_addr>>24)&0xff,Port,*StrError(errno));
        else if(r>0) cnt+=r;
        }
      } while(r>0 && cnt<len);
    }
...
int cNetSocket::Select(bool forRead, int timeout)
{
  if(sd>=0) {
    ...
    if(timeout>0 && timeout<60)
      PRINTSCF(L_GEN_DEBUG,"socket: internal: small timeout value %d",timeout);
    ...
    if(r>0) return 1;
      ...
      PRINTSCF(L_GEN_ERROR,"socket: select failed: %s",*StrError(errno));
      return -1;
      }
    else {
      if(timeout>0 && !quietlog) PRINTSCF(L_CORE_NET,"socket: select timed out (%d ms)",timeout);
      errno=ETIMEDOUT;
      return 0;
      }
    }
