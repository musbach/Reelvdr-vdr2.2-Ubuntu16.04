...
cMapMem::cMapMem(unsigned short Offset, int Size)
{
  ...
  if((mem=(unsigned char *)malloc(size)))
    memset(mem,0,size);
//  PRINTSCF(L_SYS_EMU,"mapmem: new map off=%04x size=%04x",offset,size);
}
...
cMapRom::cMapRom(unsigned short Offset, const char *Filename, int InFileOffset)
{
  ...
  if(fm && fm->Map()) {
    ...
    PRINTSCF(L_SYS_EMU,"maprom: new map off=%04x size=%04x",offset,size);
    }
}
...
cMapEeprom::cMapEeprom(unsigned short Offset, const char *Filename, int OtpSize, int InFileOffset)
{
  ...
  if(fm && fm->Map()) {
    addr=fm->Addr()+InFileOffset;
    size=fm->Size()-InFileOffset;
    PRINTSCF(L_SYS_EMU,"mapeeprom: new map off=%04x size=%04x otp=%04x",offset,size,otpSize);
    }
}
...
c6805::~c6805()
{
  if(LOG(L_SYS_CPUSTATS)) {
    ...
    PRINTSCF(L_SYS_CPUSTATS,"opcode statistics");
    for(i=0; i<256; i++)
      if((j=stats[sort[i]])) PRINTSCF(L_SYS_CPUSTATS,"opcode %02x: %d",sort[i],j);
    }
  ClearMapper();
  delete loglb;
}
...
bool c6805::AddMapper(cMap *map, unsigned short start, int size, unsigned char seg)
{
  if(map && map->IsFine()) {
    ...
    else PRINTSCF(L_SYS_EMU,"6805: too many mappers");
    }
  else PRINTSCF(L_SYS_EMU,"6805: mapper not ready (start=%02x:%04x size=%04x)",seg,start,size);
  delete map;
  return false;
}
...
void c6805::AddBreakpoint(unsigned short addr)
{
  if(numBp<MAX_BREAKPOINTS) {
    bp[numBp++]=addr;
    PRINTSCF(L_SYS_EMU,"6805: setting breakpoint at 0x%04x",addr);
    }
  else PRINTSCF(L_SYS_EMU,"6805: too many breakpoints");
}
...
// 3 - unsupported instruction
  ...
  while (1) {
    ...
    Stepper();
    if(sp<spLow) {
      PRINTSCF(L_SYS_EMU,"stack overflow (count=%d)",count);
      return 1;
      }
    if(spHi>spLow && sp>spHi) {
      PRINTSCF(L_SYS_EMU,"stack underflow (count=%d)",count);
      return 1;
      }
    if(pc>0x0400 && mapMap[pc+PAGEOFF(pc,cr)]==0) {
      PRINTSCF(L_SYS_EMU,"NX protection at %04x (count=%d)",pc,count);
      return 1;
      }
    count++;
    ...
    if(!LOG(L_SYS_DISASM) && LOG(L_SYS_DISASM80)) {
      ...
      if(doDisAsm && !flag) PRINTSCF(disAsmLogClass,"[...]");
      doDisAsm=flag;
      }

    if(doDisAsm && !disAsmHeader) {
      PRINTSCF(disAsmLogClass,"cr:-pc- aa xx yy dr -sp- VHINZC -mem@pc- -mem@sp- -cycles-");
      disAsmHeader=true;
      }
    ...
    switch(ins) {
      ...
      case 0x31:
      case 0x32:
        PRINTSCF(L_SYS_EMU,"pre-byte %02x in command decoding (count=%d)",ins,count);
        loglb->cLineBuff::Flush();
        return 3;
      default:
        PRINTSCF(L_SYS_EMU,"unsupported instruction 0x%02x (count=%d)",ins,count);
        loglb->cLineBuff::Flush();
        return 3;
      }
    ...
    for(int i=numBp-1 ; i>=0 ; i--) {
      if(bp[i]==pc) {
        PRINTSCF(L_SYS_EMU,"6805: breakpoint at %04x (count=%d)",pc,count);
        return 0;
        }
      }
    if(count>=max_count) {
      PRINTSCF(L_SYS_EMU,"max. instruction counter exceeded (count=%d)",count);
      return 2;
      }
    }
}
