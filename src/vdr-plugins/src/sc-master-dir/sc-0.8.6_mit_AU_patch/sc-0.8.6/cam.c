...
bool cScDvbDevice::Initialize(void)
{
  ...
  if(cDevice::NumDevices()>0) {
 // satellit HACK PRINTF(L_GEN_ERROR,"Number of devices != 0 on init. Put SC plugin first on VDR commandline! Aborting.");
    PRINTF(L_GEN_ERROR,"Number of devices != 0 on init: %i Put SC plugin first on VDR commandline! Aborting.",NumDevices());
    return false;
    }
  *vdr_nci=0; *vdr_ud=vdr_save_ud;

  int i, found=0, frontend=1;
  bool ReportError=true;
  for(i=0; i<MAXDVBDEVICES; i++) {
    if(UseDevice(NextCardIndex())) {
      char name[128];
// satellit HACK  DvbName(DEV_DVB_FRONTEND,i,name,sizeof(name));
      DvbName(name,i,frontend);
...
// satellit HACK  new cScDvbDevice(i,DvbOpen(DEV_DVB_CA,i,O_RDWR));
          new cScDvbDevice(i,DvbOpen(DEV_DVB_CA,i,frontend,O_RDWR, ReportError));
          found++;
          }
        else {
