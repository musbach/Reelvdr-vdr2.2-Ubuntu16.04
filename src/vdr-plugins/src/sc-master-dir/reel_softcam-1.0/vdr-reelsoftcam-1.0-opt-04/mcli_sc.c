...
//#define DBG(x...) printf(x);isyslog(x)
#define DBG(x...)
...
cScMcliDevice::cScMcliDevice() {
	DBG(">>>>>new cScMcliDevice CardIndex %d count %d\n", CardIndex(), ca_count);
#ifdef RBMINI
	dmx_fd = -1;
#else
	decsa = NULL;
...
cScMcliDevice::~cScMcliDevice() {
	DBG(">>>>>delete cScMcliDevice CardIndex %d\n", CardIndex());
	EarlyShutdown();
	if(HasSC()) 
		StopSC();
} // cScMcliDevice::~cScMcliDevice
...
bool cScMcliDevice::GetPrgCaids(int source, int transponder, int prg, caid_t *c) {
	DBG(">>>>>cScMcliDevice::GetPrgCaids CardIndex %d\n", CardIndex());
...
void cScMcliDevice::CiStartDecrypting(void) {
	DBG(">>>>>cScMcliDevice::CiStartDecrypting CardIndex %d for %s\n", CardIndex(), CurChan() ? CurChan()->Name() : "<NULL>");
...
//	DBG(">>>>>cScMcliDevice::CiAllowConcurrent CardIndex %d\n", CardIndex());
	return true;
...
#if APIVERSNUM < 10500
	DBG("cScMcliDevice::UseSC tuned %d ca %d enable %d ProvidesCa %d override %d\n", IsTunedToTransponderConst(Channel), Channel->Ca(), GetCaEnable(), ProvidesCa(Channel), GetCaOverride());
#else
//	DBG("cScMcliDevice::UseSC tuned %d ca %d enable %d ProvidesCa %d override %d\n", IsTunedToTransponderConst(Channel), Channel->Ca(), GetCaEnable(), ciadapter ? ciadapter->ProvidesCa(Channel->Caids()):-1, GetCaOverride());
#endif
	if(Channel->Ca() && (Channel->Ca()<CA_ENCRYPTED_MIN)) return false;
	if(Channel->Ca() && (Channel->Ca()>=CA_ENCRYPTED_MIN)) return true;
	DBG("cScMcliDevice::UseSC -> Default %d\n", GetCaOverride());
	return GetCaOverride();
} // cScMcliDevice::UseSC

bool cScMcliDevice::SetChannelDevice(const cChannel *Channel, bool LiveView) {
	DBG(">>>>>cScMcliDevice::SetChannelDevice %s Live %d CardIndex %d\n", Channel ? Channel->Name() : "???", LiveView, CardIndex());
...
/* satellit HACK	if(!IsTunedToTransponder(Channel)) {   */
	...
	DBG(">>>>>cScMcliDevice::SetChannelDevice %s -> RETURN %d\n", Channel ? Channel->Name() : "???", ret);
	return ret;
...
bool cScMcliDevice::ProvidesChannel (const cChannel * Channel, int Priority, bool * NeedsDetachReceivers) const {
	DBG(">>>>>cScMcliDevice::ProvidesChannel %s Prio %d CaId %d CardIndex %d max %d (SC %d)\n", Channel->Name(), Priority, Channel->Ca(), CardIndex(), max_ca, HasSC());
	...
	if(UseSC(Channel)) {
		...
		DBG(">>>>>cScMcliDevice::ProvidesChannel USE NC %s CaId %d CardIndex %d -> %d detach %d\n", Channel->Name(), Channel->Ca(), CardIndex(), (int)ret, NeedsDetachReceivers ? (int)*NeedsDetachReceivers : -1);
	} // if
	DBG(">>>>>cScMcliDevice::ProvidesChannel %s CaId %d CardIndex %d -> RETURN %d cam %p ca %d\n", Channel->Name(), Channel->Ca(), CardIndex(), ret, cam, Channel->Ca());
...
int cScMcliDevice::ProvidesCa(const cChannel *Channel) const {
	DBG(">>>>>cScMcliDevice::ProvidesCa %s CaId %d CardIndex %d\n", Channel->Name(), Channel->Ca(), CardIndex());
	if(HasFreeSC() || HasSC()) {
...
//	DBG(">>>>>cScMcliDevice::HandleTsData %p %d CardIndex %d\n", buffer, len, CardIndex());
...
void *cScMcliDevice::StartReader(cScMcliDevice *dev) {
	DBG(">>>>>cScMcliDevice::StartReader\n");
	dev->thread_running=true;
	dev->ProcessRead();
	dev->thread_running=false;
	DBG(">>>>>cScMcliDevice::StartReader DONE\n");
	return NULL;
