...
myMenuRecordingTitle::myMenuRecordingTitle(myMenuRecordings *MenuRecordings,cRecording *Recording)
:cOsdMenu(tr("Change title"),12)
{
 menurecordings=MenuRecordings;
 recording=Recording;
 strn0cpy(title,"",sizeof(title));
 strn0cpy(shortText,"",sizeof(shortText));

 if(recording && recording->Info()) {
   if(recording->Info()->Title())
     strn0cpy(title,recording->Info()->Title(),sizeof(title));
   if(recording->Info()->ShortText())
     strn0cpy(shortText,recording->Info()->ShortText(),sizeof(shortText));
 }
 Add(new cMenuEditStrItem(tr("Title"),title,sizeof(title),trVDR(FileNameChars)));
 Add(new cMenuEditStrItem(tr("Episode"),shortText,sizeof(shortText),trVDR(FileNameChars)));
 // cRemote::Put(kRight);
 Display();
}

myMenuRecordingTitle::~myMenuRecordingTitle()
{
}

eOSState myMenuRecordingTitle::ProcessKey(eKeys Key)
{
 ...
 if(state==osContinue)
 {
  if(Key==kOk)
  {
   if(strchr(title,'.')==title)
   {
    Skins.Message(mtError,tr("Invalid title!"));
    cRemote::Put(kRight);
    return osContinue;
   }
   if(strchr(shortText,'.')==shortText)
   {
    Skins.Message(mtError,tr("Invalid episode!"));
    cRemote::Put(kRight);
    return osContinue;
   }

   recording->SetTitle(title,shortText);
   recording->WriteInfo();
   state=osBack;
   menurecordings->Set(true,NULL);
  }
  if(Key==kBack)
   return osBack;
 }
...
myMenuRenameRecording::myMenuRenameRecording(cRecording *Recording,const char *DirBase,const char *DirName):cOsdMenu(tr("Rename directory"),12)
{
 isdir=false;
 recording=Recording;
...
eOSState myMenuRenameRecording::ProcessKey(eKeys Key)
{
  ...
  if(state==osContinue)
  {
    if(Key==kOk)
    {
      ...
      if(isdir)
#if APIVERSNUM > 20101
	asprintf(&oldname,"%s%s%s/%s",cVideoDirectory::Name(),tmppath?"/":"",dirbase?dirbase:"",dirname);
#else
...
#if APIVERSNUM > 20101
      asprintf(&newname,"%s%s%s/%s%s",cVideoDirectory::Name(),tmppath?"/":"",tmppath?tmppath:"",tmpname,isdir?"":strrchr(recording->FileName(),'/'));
#else
...
myMenuMoveRecording::myMenuMoveRecording(cRecording *Recording,const char *DirBase,const char *DirName,const char *Base,int Level):cOsdMenu(Base?Base:"")
{
 ...
 SetHelp(tr("Button$Cancel"),tr("Button$Move"),tr("Button$Create"),NULL);
 SetTitle(tr("Move recording"));
 Display();
}
...
eOSState myMenuMoveRecording::MoveRec()
{
  ...
  if(dirname)
#if APIVERSNUM > 20101
    asprintf(&oldname,"%s%s%s/%s",cVideoDirectory::Name(),dirbase?"/":"",tmpdirbase?tmpdirbase:"",tmpdirname);
#else
...
#if APIVERSNUM > 20101
    asprintf(&newname,"%s%s%s%s",cVideoDirectory::Name(),dir?"/":"",dir?dir:"",strrchr(dirname?oldname:recording->FileName(),'/'));
#else
        ...
        else
        {
          ...
          if(freemb-recmb > 0  || Interface->Confirm(tr("Target filesystem filled - try anyway?")))
          {
            for(cRecording *rec=Recordings.First();rec;rec=Recordings.Next(rec))
            {
              ...
              {
#if APIVERSNUM > 20101
		char *buf=ExchangeChars(strdup(oldname+strlen(cVideoDirectory::Name())+1),false);
#else
...
myMenuRecordingDetails::myMenuRecordingDetails(cRecording *Recording):cOsdMenu(tr("Details"),12)
{
 ...
 Add(new cMenuEditIntItem(tr("Lifetime"),&lifetime,0,MAXLIFETIME, NULL, tr("unlimited")));

 Add(new cOsdItem(" ", osUnknown, false), false, NULL);
 Add(new cOsdItem(" ", osUnknown, false), false, NULL);
 Add(new cOsdItem(" ", osUnknown, false), false, NULL);
 Add(new cOsdItem(tr("Info:"), osUnknown , false));
 Add(new cOsdItem(tr("In this menu you can adjust the point in time"), osUnknown , false));
 Add(new cOsdItem(tr("a recording gets deleted automatically."), osUnknown , false));
 Display();
}
