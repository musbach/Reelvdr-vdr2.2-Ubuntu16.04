    ...
    if(!stat64(filename,&ds)) {
      if(S_ISREG(ds.st_mode)) {
        ...
        if(fd>=0) {
          ...
          else PRINTSCF(L_GEN_ERROR,"mapping failed on %s: %s",filename,strerror(errno));
          close(fd); fd=-1;
          }
        else PRINTSCF(L_GEN_ERROR,"error opening filemap %s: %s",filename,strerror(errno));
        }
      else PRINTSCF(L_GEN_ERROR,"filemap %s is not a regular file",filename);
      }
    else PRINTSCF(L_GEN_ERROR,"can't stat filemap %s: %s",filename,strerror(errno));
...
time_t cStructLoader::MTime(bool log)
{
  ...
  if(stat64(path,&st)!=0) {
    if(log) {
      PRINTSCF(L_GEN_ERROR,"failed fstat %s: %s",path,strerror(errno));
      PRINTSCF(L_GEN_WARN,"automatic reload of %s disabled",path);
      }
    st.st_mtime=0;
    }
  return st.st_mtime;
...
void cStructLoader::CheckAccess(void)
{
  if(access(path,R_OK|W_OK)!=0) {
    if(errno!=EACCES)
      PRINTSCF(L_GEN_ERROR,"failed access %s: %s",path,strerror(errno));
    PRINTSCF(L_GEN_WARN,"no write permission on %s. Changes will not be saved!",path);
...
bool cStructLoader::CheckUnmodified(void)
{
  ...
  if(mtime && mtime<curr_mtime && SL_TSTFLAG(SL_WATCH)) {
     PRINTSCF(L_CORE_LOAD,"abort save as file %s has been changed externaly",path);
     return false;
     }
  return true;
}
...
void cStructLoader::LoadFinished(void)
{
  ...
  if(!SL_TSTFLAG(SL_LOADED))
    PRINTSCF(L_CORE_LOAD,"loading %s terminated with error. Changes will not be saved!",path);
}

void cStructLoader::OpenFailed(void)
{
  if(SL_TSTFLAG(SL_VERBOSE) && !SL_TSTFLAG(SL_SHUTUP)) {
    PRINTSCF(L_GEN_ERROR,"failed open %s: %s",path,strerror(errno));
...
void cStructLoader::Load(bool reload)
{
  ...
  if(f) {
    ...
    if(!reload) {
      ...
      PRINTSCF(L_CORE_LOAD,"detected change of %s",path);
      if(IsModified())
        PRINTSCF(L_CORE_LOAD,"discarding in-memory changes");
    ...
    if(doload) {
      ...
      PRINTSCF(L_GEN_INFO,"loading %s from %s",type,path);
      CheckAccess();
      int lineNum=0, num=0;
      char buff[4096];
      while(fgets(buff,sizeof(buff),f)) {
        lineNum++;
        if(!index(buff,'\n') && !feof(f)) {
          PRINTSCF(L_GEN_ERROR,"file %s readbuffer overflow line#%d",path,lineNum);
        ...
        if(hasContent) {
          ...
          *ls=0; it=ParseLine(skipspace(buff)); *ls=save;
          if(!it) {
            PRINTSCF(L_GEN_ERROR,"file %s has error in line #%d",path,lineNum);
            ls=buff;
            }
          else num++;
          }
        ...
        if(it) {
          ...
          PRINTSCF(L_GEN_ERROR,"out of memory loading file %s",path);
          SL_CLRFLAG(SL_LOADED);
          break;
          }
        }
      ListUnlock();
      PRINTSCF(L_CORE_LOAD,"loaded %d %s from %s",num,type,path);
      PostLoad();
      }
    else ListUnlock();
...
void cStructLoader::Save(void)
{
  if(CheckDoSave()) {
    ...
    if(f.Open()) {
      ...
      Modified(false);
      ListUnlock();
      PRINTSCF(L_CORE_LOAD,"saved %s to %s",type,path);
      }
    }
}
...
{
  PRINTSCF(L_CORE_DYN,"structloaders: registering loader %s",ld->type);
  ld->next=first;
  first=ld;
}
...
bool cEcmInfo::AddCaDescr(const unsigned char *descr, int len)
{
  ...
  if(descr && (!caDescr || caDescrLen!=len || memcmp(caDescr,descr,len)!=0)) {
    ...
    else PRINTSCF(L_GEN_ERROR,"malloc failed in cEcmInfo::AddCaDescr()");
    }
  return res;
}
...
{
  PRINTSCF(L_CORE_DYN,"registering key type %c%s",pkt->type,Super?" (super)":"");
  pkt->next=first;
  first=pkt;
}
...
void cPlainKeys::Trigger(int Type, int Id, int Keynr)
{
  if(lastkey.NotLast(Type,Id,Keynr))
    PRINTSCF(L_CORE_AUEXTERN,"triggered from findkey (%s)",*KeyString(Type,Id,Keynr));
  ExternalUpdate();
}
...
cPlainKey *cPlainKeys::NewFromType(int type)
{
  ...
  PRINTSCF(L_CORE_LOAD,"unknown key type '%c', adding dummy",type);
...
void cPlainKeys::PostLoad(void)
{
  ...
  if(Count() && LOG(L_CORE_KEYS)) {
    for(cPlainKey *dat=First(); dat; dat=Next(dat))
      PRINTSCF(L_CORE_KEYS,"keys %s",*dat->ToString(false));
    }
  ListUnlock();
}
...
void cPlainKeys::HouseKeeping(void)
{
  if(trigger.TimedOut()) {
    ...
    if(externalAU) PRINTSCF(L_CORE_AUEXTERN,"triggered from housekeeping");
    ExternalUpdate();
    }
}
...
void cPlainKeys::ExternalUpdate(void)
{
  if(externalAU && ScSetup.AutoUpdate>0) {
    if(last.TimedOut()) {
      ...
      if(!Active())
        Start();
      else PRINTSCF(L_CORE_AUEXTERN,"still running");
      Unlock();
      }
    else PRINTSCF(L_CORE_AUEXTERN,"denied, min. timeout not expired");
    }
}
...
{
  last.Set(EXT_AU_MIN);
  PRINTSCF(L_CORE_AUEXTERN,"starting...");
  cTimeMs start;
  cPipe pipe;
  ...
  pipe.Close();
  PRINTSCF(L_CORE_AUEXTERN,"done (elapsed %d)",(int)start.Elapsed());
}
