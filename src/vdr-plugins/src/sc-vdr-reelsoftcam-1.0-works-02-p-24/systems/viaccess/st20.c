...
unsigned int cST20::GetReg(int reg) const
{
  switch(reg) {
    ...
    default:   PRINTSCF(L_SYS_ST20,"getreg: unknown reg"); return ERRORVAL;
    }
}
...
void cST20::SetReg(int reg, unsigned int val)
{
  switch(reg) {
    ...
    default:   PRINTSCF(L_SYS_ST20,"setreg: unknown reg"); return;
    }
}
  ...
  while(Iptr!=0) {
    ...
    switch(op1>>4) {
	  ...
	  default: 
            ...
            PRINTSCF(L_SYS_ST20,"unknown opcode %X",operand);
            return ERR_ILL_OP;
	  }
        CLEAR_OP();
        break;
    ...
    if(--count<=0 && operand==0) {
      PRINTSCF(L_SYS_ST20,"instruction counter exceeded");
      return ERR_CNT;
      }
    }
  return 0;
