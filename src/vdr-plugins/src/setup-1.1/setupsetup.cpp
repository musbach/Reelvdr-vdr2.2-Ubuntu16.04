...
//    _allowedChar = " .,:;+-*~?!\"$%&/\()=`'_abcdefghijklmnopqrstuvwxyz0123456789";
    _allowedChar = (char*)" .,:;+-*~?!\"$%&/\()=`'_abcdefghijklmnopqrstuvwxyz0123456789";
...
//	int sizeS=sizeof(_menuSuffix);
//       snprintf(_menuSuffix, sizeS, Value);
    }else if (strcmp(Name, _strEntryPrefix) == 0) {
//	int sizeP=sizeof(_entryPrefix);
//        snprintf(_entryPrefix, sizeP, Value);
    }else{
        return false;
    }
    return true;
