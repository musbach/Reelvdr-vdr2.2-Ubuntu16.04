...
cMenuEditSearchExt::cMenuEditSearchExt(cSearchExt *SearchExt, bool New, bool Template,
                                       bool openSearchTimerMenu_)
    :cOsdMenu(tr("Edit search"),32), openSearchTimerMenu(openSearchTimerMenu_)
{
...
void cMenuEditSearchExt::Set()
{
	...
	if (data.action == searchTimerActionRecord)
	{
		...
		// do not pass a placeholder text. If there is a placeholder, InEditMode() always reports that item is in EditMode
		// and the "Directory Selector" menu is never offered!
	    Add(new cMenuEditStrItem(IndentMenuItem(tr("Directory")), data.directory, sizeof(data.directory), NULL, NULL));
...
#ifdef REELVDR
       if(menu_mode == expert_mode)
         Add(new cMenuEditStraItem(IndentMenuItem(trVDR("Priority")),     &tmpprio, 3, PriorityTexts));
#else
         Add(new cMenuEditIntItem(IndentMenuItem(trVDR("Priority")), &data.Priority, 0, MAXPRIORITY));
#endif
            if(menu_mode == expert_mode)
         Add(new cMenuEditIntItem(IndentMenuItem(trVDR("Lifetime")), &data.Lifetime, 0, MAXLIFETIME, NULL, tr("unlimited")));
