...
bool cSearchExt::Parse(const char *s, int id)
{
   char *line;
   char *pos;
   char *pos_next;
...
#ifdef USEMYSQL
void cSearchExt::UpdateDB()
{
    bool SetServer = false;
    // Update TimerSearch to Database
    ...
    if(SetServer)
        TimerSearchMysql->UpdateTimerSearch(this);
...
void cSearchExts::Add(cSearchExt* searchExt)
{
#ifdef USEMYSQL
    if(UseDatabase_ && ((Setup.ReelboxModeTemp == eModeClient) || (Setup.ReelboxModeTemp == eModeServer))) // Client & Server
    {
        bool SetServer = false;
        // Add TimerSearch to Database
        cTimerSearchMysql *TimerSearchMysql = new cTimerSearchMysql(MYSQLREELUSER, MYSQLREELPWD, "vdr");
        if((Setup.ReelboxModeTemp == eModeClient) && Setup.NetServerIP && strlen(Setup.NetServerIP)) // Client
            SetServer = TimerSearchMysql->SetServer(Setup.NetServerIP);
        else
...
#endif
    cList<cSearchExt>::Add(searchExt);
}

void cSearchExts::Del(cSearchExt* searchExt)
{
#ifdef USEMYSQL
    if(UseDatabase_ && ((Setup.ReelboxModeTemp == eModeClient) || (Setup.ReelboxModeTemp == eModeServer))) // Client & Server
    {
        ...
        if((Setup.ReelboxModeTemp == eModeClient) && Setup.NetServerIP && strlen(Setup.NetServerIP)) // Client
            ...
            SetServer = TimerSearchMysql->SetServer("localhost");
        if(SetServer)
            TimerSearchMysql->DeleteTimerSearch(searchExt->ID);
        delete TimerSearchMysql;
    }
#endif
    cList<cSearchExt>::Del(searchExt);
}
...
void cSearchExts::SetFilename(const char *FileName)
{
    free(fileName);
    if (FileName)
        fileName = strdup(FileName);
...
bool cSearchExts::LoadDB()
{
    ...
    if(SetServer)
    {
        TimerSearchMysql->LoadDB(&LastEventID);
        delete TimerSearchMysql;
        return true;
    }
    delete TimerSearchMysql;
    esyslog("ERROR epgsearch (%s,%i): LoadDB failed\n", __FILE__, __LINE__);
    return false;
}

void cSearchExts::SyncData()
{
    bool SetServer = false;
    ...
    if(SetServer)
        TimerSearchMysql->Sync(&LastEventID);
    delete TimerSearchMysql;
}
#endif
