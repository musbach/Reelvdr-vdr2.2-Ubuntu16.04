...
#if VDRVERSNUM == 10503
#warning "YOU NEED A PATCHED VDR 1.5.3 OR ReelSkin WILL CRASH!"
#endif

static const char VERSION[] = "0.99";
static const char DESCRIPTION[] = trNOOP("ReelBox skin 3rd generation");
static const char MAINMENUENTRY[] = "ReelBox Skin"; // needed for Setup entry too!!
...
#ifdef REELVDR
    return MAINMENUENTRY; /* ReelVDR needs this for the setup-entry */
#else
    return NULL; /* no menu, no MainMenuEntry */
#endif
  }
...
bool cPluginSkinReel::ProcessArgs(int argc, char *argv[])
{
...
#endif
      case 'i':
        ReelConfig.SetEPGImagesDir(optarg);
        fImagesDirSet = true;
        break;
      case 'l':
  ...
  {
#ifdef REELVDR
      ReelConfig.SetImagesDir("/usr/share/reel/skinreel3/Blue"); // Set default Image Directory to blue theme 
#else
      ReelConfig.SetImagesDir("/usr/share/vdr/skinreel3/Blue"); // Set default Image Directory to blue theme      
#endif     
      fImagesDirSet = true;
  }
#ifdef REELVDR
  if (fLogodirSet == false ) { ReelConfig.SetLogoDir("/usr/share/reel/channellogos/"); }
#else
  if (fLogodirSet == false ) { ReelConfig.SetLogoDir("/usr/share/vdr/channellogos/"); }
#endif
...
bool cPluginSkinReel::Start(void)
{
  ...
  if (!fImagesDirSet) {
    ...
    std::string dir;
    dir = string(cPlugin::ConfigDirectory(PLUGIN_NAME_I18N)) +  "/epgimages";
    ReelConfig.SetImagesDir(dir.c_str());
    fImagesDirSet = true;
  }
#ifdef HAVE_FREETYPE
  if (!fFontsDirSet) {
    /* set fonts directory */
    std::string dir;
    dir = string(cPlugin::ConfigDirectory(PLUGIN_NAME_I18N)) + "/fonts";
    ReelConfig.SetFontsDir(dir.c_str());
    fFontsDirSet = true;
  }
#endif
...
#else
  return new cMenuSetupGeneral(&ReelConfig);
#endif
}
...
//  debug("cPluginSkinReel::SetupParse()");
  ...
  else if (!strcasecmp(Name, "ShowSourceInChannelinfo"))   ReelConfig.showSourceInChannelinfo = atoi(Value);
  else if (!strcasecmp(Name, "ProgressBarType")){          ReelConfig.pgBarType = atoi(Value);
                                                           if(ReelConfig.pgBarType == 0)
                                                                progressbarheight = 9;
                                                           else 
...
bool cPluginSkinReel::Service(const char *Id, void *Data)
{
  if(!strcmp(Id, "setThumb")){
	...
	return true;
  } else if (!strcmp(Id, "setId3Infos")) {
        if (Data) {
            ID3Tags = *((std::vector<struct ID3Tag>*)Data);
        } else { /* clear the id3-infos */
            ID3Tags.clear();
        }
    return true;
  }
