...
void cSkinReelDisplayMenu::SideNote(const cStringList * strList) {
    ...
    // font colour
    tColor NormalFg = Theme.Color(clrMenuItemNotSelectableFg);
    tColor fgColour = NormalFg;

#define IMAGE_STR "IMAGE:"
#define FONT_COLOUR_STR "FONTCOLOUR:"
#define STARTFROM "STARTFROM"
    ...
    int y_max = yBodyBottom - IMGICONWIDTH - 2*Gap;
    // Draw each string in its own line
    for ( ; i < strList->Size(); ++i) {

        if (y >=  y_max - sideNoteFont->Height())
            break;

        s = strList->At(i);

	if (strncmp(s, STARTFROM, strlen(STARTFROM))==0) {
		std::string str = s;
		str = str.substr(strlen(STARTFROM));
		y = atoi(str.c_str());
	} else if (strncmp(s, IMAGE_STR, strlen(IMAGE_STR))==0)
	{
		const char* path = s + strlen(IMAGE_STR);
		cPngUtils png;

		if (!png.ReadPng(path)) {
			esyslog("%s:%d %s ReadPng failed '%s'", __FILE__, __LINE__, __PRETTY_FUNCTION__, path);
			continue;
		}
		int x = xSideLeft;

		if (y + png.Height() > y_max) {
			esyslog("%s:%d %s png '%s' too tall", __FILE__, __LINE__, __PRETTY_FUNCTION__, path);
			continue;
		}
		if (x + png.Width()  > xSideRight){
			esyslog("%s:%d %s png '%s' too wide", __FILE__, __LINE__, __PRETTY_FUNCTION__, path);
			continue;
		}

        osd->SetImagePath(imgUnbufferedEnum, path);
        osd->DrawImage(imgUnbufferedEnum, x,y, true);

        y += png.Height() + 2*Gap;
    } else
        if (strncmp(s, FONT_COLOUR_STR, strlen(FONT_COLOUR_STR))==0)
        {
            const char* fontColourString = s + strlen(FONT_COLOUR_STR);
            tColor t = Theme.Color(fontColourString);
            if (t)
            {
                fgColour = t;
            }
            ...
            {
                if (strcasecmp(fontColourString, "normal") == 0)
                {
                    fgColour = NormalFg;
                }
                else
                    esyslog("%s:%d %s New font colour: '%s' NOT found\n", __FILE__, __LINE__, __PRETTY_FUNCTION__,
                            fontColourString);
            }
        ...
        if (strcmp(s, "FOLDER ICON") == 0) {
            ...
            const cFont *sideNoteFont = regularFont11;
            size_t i = 0;
            int x = xSideLeft;

            //printf("Trying \033[0;91m'%s'\033[0m\n", s);

            // find any progress bar strings and draw them
            // strings like "[|||   ]" may follow and/or be followed by texts

            while (i < strlen(s))
            {
                /** any progress bars?*/
                int pb_start = 0, pb_end = 0, progressCount = 0;

                // get the length of progress bar if any
                int pbtotal = pgbar(s + i, pb_start, pb_end, progressCount);
#if 0
                printf("\033[0;92m'%s'\033[0m got pb_start %d, pb_end %d, pb_count %d, pb_total %d\n", s,
                       pb_start, pb_end, progressCount, pbtotal);
#endif
                // no progressbar
                if (!pbtotal)
                    break;

                // first display the text before progressbar
                if (pb_start != 0)
                {
                    char buffer[512];
                    strncpy(buffer, s+i, pb_start);
                    buffer[pb_start] = 0;
                    osd->DrawText(x, y,
                                  buffer,
                                  fgColour, clrTransparent,
                                  sideNoteFont,
                                  xSideRight-x);

                    x += sideNoteFont->Width(buffer);// + sideNoteFont->Width("D");
                }
                ...
                int yPB = y + (sideNoteFont->Height() - progressbarheight)/2 ; // centre it

                int percentCompleted = (progressCount*100)/pbtotal;
                /* make sure percentage is within [0,100] */
                percentCompleted = std::max(0  , percentCompleted);
                percentCompleted = std::min(100, percentCompleted);

                /** draw progressbar */
                DrawProgressBar(osd, xPB, yPB, xPB + (int)(pbLength*percentCompleted/100), xPB + pbLength);

                x += pbLength;// + sideNoteFont->Width("D");
            } // while
...
#define W 126
    ...
    const cChannel *channel = Channels.GetByChannelID(Event->ChannelID(),false,false);
    if (channel)
        logoPath = ReelConfig.GetLogoPath(channel->Name());
    ...
    while (!file.eof()) {
      ...
      if(strBuff.find("#") != 0) /* Skip the comments */
        if(strBuff.find(";") != std::string::npos) { /* Skip the faulty lines (missing ;-seperator) */
          ...
          std::string::size_type idx = strBuff.find(";linespace=");
          if (idx != std::string::npos)
          {
              std::string str = strBuff.substr(idx + strlen(";linespace="));
              tempMenuType.linespacing = atoi(str.c_str());
              printf("\033[7;91mLineSpacing='%s'->%d\33[0m\n",str.c_str(), tempMenuType.linespacing);
          }
...
bool GetMenuTypeFromMenuList(std::string Title, MenuTypeStruct& menuType)
{
    unsigned int i = 0;
    while (i < MenuList.size()) {
      if(Title == trVDR(MenuList.at(i).Title.c_str())) {
        menuType = MenuList.at(i);
        return true;
      }
      ++i;
    } // while
...
bool GetMenuTypeFromMenuListWildCard(std::string Title, MenuTypeStruct& menuType)
{
    unsigned int i = 0;
    while (i < MenuListWildcard.size()) {
      if(Title.find(trVDR(MenuListWildcard.at(i).Title.c_str())) == 0 ) {
        menuType = MenuListWildcard.at(i);
        return true;
      }
      ++i;
    } // while
...
int GetLineSpacing(std::string Title)
{
    MenuTypeStruct menuType;
    bool found = GetMenuTypeFromMenuList(Title, menuType);

    if (found)
        return menuType.linespacing;

    found = GetMenuTypeFromMenuListWildCard(Title, menuType);
    if (found)
        return menuType.linespacing;
...
#define MP3COVERSTRING "SHOWINGMP3COVER$"
  ...
  if(isButtoned)
      ...
      lineHeight = (pFontList->Height() * GetLineSpacing(strTitle))/100; //change
...
void cSkinReelDisplayMenu::SetRecording(const cRecording *Recording)
{
  ...
  cChannel *channel = Channels.GetByChannelID(((cRecordingInfo *)Info)->ChannelID(),false,false);
