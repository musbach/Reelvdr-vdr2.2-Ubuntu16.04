...
#ifdef REELVDR
            if (filetype == 0x01 && slideshow) { // MPEG-Still AND show slide
                //if (S_Verbose)
                //    printf("Disable reelbox bkgpicture in cRadioAudio::RassDecode(uchar*, int)\n");
                ...
                else cPluginManager::CallAllServices("DisableAudioPicPlayer");
                }
#endif
...
void cRadioAudio::DisableRadioTextProcessing()
{
    ...
    if (RDSReceiver != NULL && rdsdevice != NULL) {
        ...
        if (RT_MsgShow) {
            ...
            if ((S_RtOsdTags == 1 && RT_PlusShow) || S_RtOsdTags >= 2) {
                if (!RDS_PSShow || !strstr(RTP_Title, "---") || !strstr(RTP_Artist, "---")) {
                    ...
                    osd->DrawText(fwidth, 6+yoff+fheight*(ii++), RTP_Title, fcolor, clrTransparent, ftext, Setup.OSDWidth-4, ftext->Height());
                    osd->DrawText(4, 3+yoff+fheight*(ii), stext[2], fcolor, clrTransparent, ftext, fwidth-5, ftext->Height());
                    osd->DrawText(fwidth, 3+yoff+fheight*(ii++), RTP_Artist, fcolor, clrTransparent, ftext, Setup.OSDWidth-4, ftext->Height());
                    }
                else {
                    char *temp; 
                    asprintf(&temp, "%s", "");
...
void cRTplusOsd::Load(void) 
{
    ...
    if (rtp_content.prog_StatShort != NULL) {
        snprintf(text, sizeof(text), "\t%s:\t%s", tr("Stat.Short"), rtp_content.prog_StatShort);
        Add(new cOsdItem(hk(text)));
        }
    ...
    if (rtp_content.sms_Studio != NULL) {
        snprintf(text, sizeof(text), "\t%s:\t%s", tr("SMS-Studio"), rtp_content.sms_Studio);
        Add(new cOsdItem(hk(text)));
        }
