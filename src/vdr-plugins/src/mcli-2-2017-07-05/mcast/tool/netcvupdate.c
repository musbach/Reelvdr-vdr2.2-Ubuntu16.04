...
#include <sys/un.h>
#include "../client/api_server.h"
#include "../common/recv_ccpp.h"
#endif
...
int api_init(char *path)
{
	...
	printf("===> api_init sock_name.sun_path:%s sizeof:%d path:%s sizeof:%d \n",sock_name.sun_path,sizeof(sock_name.sun_path),path,sizeof(path));
        sock_name_len = sizeof(struct sockaddr_un);
//        sock_name_len = sizeof(struct sock_name);
	printf("===> api_init sock_name_len:%d sizeof(sock_name.sun_family):%d sock_name.sun_family:%d \n",sock_name_len,sizeof(sock_name.sun_family),sock_name.sun_family);
	...
	printf("===> api_init sock_comm:%d sock_name_len:%d path:%s \n",sock_comm,sock_name_len,path) ;
//        if (connect(sock_comm, (struct sockaddr*)&sock_name, sock_name_len) < 0) {
        if (connect(sock_comm, (struct sockaddr_un*)&sock_name, sock_name_len) < 0) {
                warn ("connect failure to %s: %s (are you root? Is vdr running?)\n",path, strerror(errno));
		return -1;
        }
	return 0;
}
...
int generate_script(char *filename, char *tmpname, char *ip, char *iface, char *user, char *pwd)
{
	...
	while(!feof(f)) {
		char line[256];
//		fgets(line,255,f);
		if (fgets((void*) line,255,f)!= NULL )fprintf(stderr,"Can't read array <%s>: %s\n",line,strerror(errno));
//		if (fgets(*line,255,f)!= NULL )fprintf(stderr,"Can't read array <%s>: %s\n",line,strerror(errno));
...
//		fgets(line, 1023,file);
		if (fgets((void*) line,1023,file)!= NULL )fprintf(stderr,"Can't read array <%s>: %s\n",line,strerror(errno));
		line[1023]=0;
...
int get_uuids(char **uuids, int max)
{
	...
	printf("===> get_uuids before api_init  socket_path:%s \n",socket_path);

	if (api_init(socket_path)==-1) {
		exit(-1);
	}
	printf("===> get_uuids after api_init \n");
...
int show_uuids(void)
{
	char *uuids[256]={0};
	int num_uuids,n;
	printf("===> show_uuide before get_uuids uuids \n");
	num_uuids=get_uuids(uuids,256);
	printf("===> show_uuide after get_uuids uuids num_uuids=%d \n",num_uuids);
	for(n=0;n<num_uuids;n++) {
		printf("%s    %s\n",uuids[n],versions[n]);
	}
	return 0;
...
int main(int argc, char **argv)
{
	...
	while(1) {
		...
		case 'A':
			...
			break;
		case 'l':
	                printf("===> main before show_uuids c=%c \n",c);
			show_uuids();
			break;
		case 'd':
		...
		case 'q':
			...
			break;
		default:
			printf("===> ERROR, option not found %d \n",c);
			usage();
			break;
		}
	}
