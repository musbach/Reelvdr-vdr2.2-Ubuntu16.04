...
{
  if(!keyfile) PRINTSCF(L_CORE_DYN,"feature: using feature KEYFILE");
  keyfile=true;
}

void cFeature::NeedsSmartCard(void)
{
  if(!smartcard) PRINTSCF(L_CORE_DYN,"feature: using feature SMARTCARD");
  smartcard=true;
}
  ...
  switch(ecm->caId>>8) {
    case 0x06: // Irdeto
      ...
      if(sync && check->caid.sys06.current>0 && chid!=check->caid.sys06.current) {
        PRINTSCF(L_CORE_ECMPROC,"ecmcheck(%s): chid %04x != current %04x",name,chid,check->caid.sys06.current);
        return 3;
        }
/*
          ...
          if(cur==max) {
            for(int i=0 ; i<max ; i++) {
              if(check->caid.sys06.chids[i]==0) {
                ...
                PRINTSCF(L_CORE_ECMPROC,"ecmcheck(%s): zero chid",name);
                return 1;
                }
              for(int j=i+1 ; j<=max ; j++) {
                if(check->caid.sys06.chids[i]==check->caid.sys06.chids[j]) {
                  check->caid.sys06.startFlag=false;
                  PRINTSCF(L_CORE_ECMPROC,"ecmcheck(%s): duplicate chid %04x",name,check->caid.sys06.chids[i]);
                  return 1;
                  }
                }
              }
...
void cSystem::KeyOK(cPlainKey *pk)
{
  if(lastkey.NotLast(pk->type,pk->id,pk->keynr)) {
    ...
    PRINTSCF(L_CORE_ECM,"system: using key %s",*pk->ToString(true));
    doLog=true;
    }
}
...
void cSystem::KeyFail(int type, int id, int keynr)
{
  ...
  if(lastkey.NotLast(type,id,keynr) && doLog)
    PRINTSCF(L_CORE_ECM,"system: no key found for %s",*keys.KeyString(type,id,keynr));
}
...
{
  PRINTSCF(L_CORE_DYN,"systems: registering CA system %s, pri %d, ident %04X",sysLink->name,sysLink->pri,nextSysIdent);
  sysLink->next=first;
...
{
  int num=0;
  PRINTSCF(L_CORE_LOAD,"** registered systems:");
  for(cSystemLink *sl=first; sl; sl=sl->next, num++)
    PRINTSCF(L_CORE_LOAD,"** %-16s  (pri %3d)",sl->name,sl->pri);
  if(num<1)
    PRINTSCF(L_GEN_WARN,"No registered systems! Won't decode anything! Where are your libs?");
  for(cSystemLink *sl=first; sl; sl=sl->next)
...
cMsgCache::cMsgCache(int NumCache, int StoreSize)
{
  ...
  if(caches) {
    ...
    if(storeSize>0) {
      ...
      if(!stores) PRINTSCF(L_GEN_ERROR,"msgcache: no memory for store area");
      }
    }
  else PRINTSCF(L_GEN_ERROR,"msgcache: no memory for cache");
}
...
void cMsgCache::SetMaxFail(int maxfail)
{
  ...
  PRINTSCF(L_CORE_MSGCACHE,"%d/%p: maxFail set to %d",getpid(),this,maxFail);
}
...
void cMsgCache::Clear(void)
{
  ...
  ptr=0;
  PRINTSCF(L_CORE_MSGCACHE,"%d/%p: clear",getpid(),this);
}
...
int cMsgCache::Get(const unsigned char *msg, int len, unsigned char *store)
{
  ...
  while((s=FindMsg(hash))) {
    ...
    s->mode|=WAIT;
    PRINTSCF(L_CORE_MSGCACHE,"%d/%p: msg already queued. waiting to complete",getpid(),this);
    wait.Wait(mutex);
    }
  int id;
  if(!s) {
    while(1) {
      ...
      if(!(s->mode&QUEUED)) break;
      s->mode|=WAIT;
      PRINTSCF(L_CORE_MSGCACHE,"%d/%p: queue overwrite protection id=%d",getpid(),this,ptr+1);
      wait.Wait(mutex); // don't overwrite queued msg's
      }
    id=ptr+1;
    memcpy(s->hash,hash,HASHLEN);
    s->mode=QUEUED;
    PRINTSCF(L_CORE_MSGCACHE,"%d/%p: queued msg with id=%d",getpid(),this,id);
    ptr++; if(ptr>=numCache) { ptr=0; PRINTSCF(L_CORE_MSGCACHE,"msgcache: roll-over (%d)",numCache); }
    return id;
    }
  else {
    id=(s-&caches[0])+1;
    if(s->mode==GOOD) {
      ...
      PRINTSCF(L_CORE_MSGCACHE,"%d/%p: msg is cached as GOOD (%d)",getpid(),this,id);
      return 0;
      }
    else if(s->mode>=FAIL1 && s->mode<=FAIL2) {
      PRINTSCF(L_CORE_MSGCACHE,"%d/%p: msg is cached as FAIL%d (%d)",getpid(),this,s->mode,id);
      s->mode|=QUEUED;
      return id;
      }
    else {
      PRINTSCF(L_CORE_MSGCACHE,"%d/%p: msg is cached as FAILN (%d)",getpid(),this,id);
      return -1;
      }
    }
}
