...
#ifdef STREAM_INIT
  DBSCG(fprintf(stderr,":::::::::: BEGIN STREAM INIT\n"));
#endif
#ifdef STREAM_NORMAL
  DBSCG(fprintf(stderr,":::::::::: BEGIN STREAM NORMAL\n"));
#endif
#ifdef STREAM_INIT
for(j=0;j<64;j++){
  DBSCG(fprintf(stderr,"precall prerot stream_in[%2i]=",j));
  DBSCG(dump_mem("",sb+BYPG*j,BYPG,BYPG));
}

DBSCG(dump_mem("stream_prerot ",sb,GROUP_PARALLELISM*8,BYPG));
...
#endif
DBSCG(dump_mem("stream_postrot",sb,GROUP_PARALLELISM*8,BYPG));

for(j=0;j<64;j++){
  DBSCG(fprintf(stderr,"precall stream_in[%2i]=",j));
  DBSCG(dump_mem("",sb+BYPG*j,BYPG,BYPG));
}
#endif
  ...
  for(i=0;i<8;i++){
    for(b=0;b<4;b++){
DBSCG(fprintf(stderr,"dbg from iA A[%i][%i]=",i,b));
DBSCG(dump_mem("",(unsigned char *)&iA[i][b],BYPG,BYPG));
DBSCG(fprintf(stderr,"                                       dbg from iB B[%i][%i]=",i,b));
DBSCG(dump_mem("",(unsigned char *)&iB[i][b],BYPG,BYPG));
...
for(dbg=0;dbg<4;dbg++){
  DBSCG(fprintf(stderr,"dbg A0[%i]=",dbg));
  DBSCG(dump_mem("",(unsigned char *)&regs->A[aboff+0][dbg],BYPG,BYPG));
  DBSCG(fprintf(stderr,"dbg B0[%i]=",dbg));
  DBSCG(dump_mem("",(unsigned char *)&regs->B[aboff+0][dbg],BYPG,BYPG));
}
...
////////////////////////////////////////////////////////////////////////////////
  ...
  for(i=0;i<8;i++){
    ...
    DBSCG(fprintf(stderr,"--BEGIN EXTERNAL LOOP %i\n",i));
    ...
    for(j=0; j<4; j++){
      ...
      DBSCG(fprintf(stderr,"---BEGIN INTERNAL LOOP %i (EXT %i, INT %i)\n",j,i,j));
...
for(dbg=0;dbg<4;dbg++){
  DBSCG(fprintf(stderr,"extra_B[%i]=",dbg));
  DBSCG(dump_mem("",(unsigned char *)&extra_B[dbg],BYPG,BYPG));
}
...
for(dbg=0;dbg<4;dbg++){
  DBSCG(fprintf(stderr,"next_A0[%i]=",dbg));
  DBSCG(dump_mem("",(unsigned char *)&regs->A[aboff-1][dbg],BYPG,BYPG));
}
...
for(dbg=0;dbg<4;dbg++){
  DBSCG(fprintf(stderr,"next_B0[%i]=",dbg));
  DBSCG(dump_mem("",(unsigned char *)&regs->B[aboff-1][dbg],BYPG,BYPG));
}
...
for(dbg=0;dbg<4;dbg++){
  DBSCG(fprintf(stderr,"D[%i]=",dbg));
  DBSCG(dump_mem("",(unsigned char *)&regs->D[dbg],BYPG,BYPG));
}
...
for(dbg=0;dbg<4;dbg++){
  DBSCG(fprintf(stderr,"F[%i]=",dbg));
  DBSCG(dump_mem("",(unsigned char *)&regs->F[dbg],BYPG,BYPG));
}
DBSCG(fprintf(stderr,"r="));
DBSCG(dump_mem("",(unsigned char *)&regs->r,BYPG,BYPG));
for(dbg=0;dbg<4;dbg++){
  DBSCG(fprintf(stderr,"E[%i]=",dbg));
  DBSCG(dump_mem("",(unsigned char *)&regs->E[dbg],BYPG,BYPG));
}
...
*/
      ...
      regs->q=s7b;
for(dbg=0;dbg<4;dbg++){
  DBSCG(fprintf(stderr,"X[%i]=",dbg));
  DBSCG(dump_mem("",(unsigned char *)&regs->X[dbg],BYPG,BYPG));
}
for(dbg=0;dbg<4;dbg++){
  DBSCG(fprintf(stderr,"Y[%i]=",dbg));
  DBSCG(dump_mem("",(unsigned char *)&regs->Y[dbg],BYPG,BYPG));
}
for(dbg=0;dbg<4;dbg++){
  DBSCG(fprintf(stderr,"Z[%i]=",dbg));
  DBSCG(dump_mem("",(unsigned char *)&regs->Z[dbg],BYPG,BYPG));
}
DBSCG(fprintf(stderr,"p="));
DBSCG(dump_mem("",(unsigned char *)&regs->p,BYPG,BYPG));
DBSCG(fprintf(stderr,"q="));
DBSCG(dump_mem("",(unsigned char *)&regs->q,BYPG,BYPG));
...
for(dbg=0;dbg<8;dbg++){
  DBSCG(fprintf(stderr,"op[%i]=",dbg));
  DBSCG(dump_mem("",(unsigned char *)&cb_g[8*i+dbg],BYPG,BYPG));
}
#endif

DBSCG(fprintf(stderr,"---END INTERNAL LOOP\n"));

    } // INTERNAL LOOP

DBSCG(fprintf(stderr,"--END EXTERNAL LOOP\n"));
  ...
  for(k=0;k<10;k++){
    for(b=0;b<4;b++){
DBSCG(fprintf(stderr,"moving forward AB k=%i b=%i\n",k,b));
...
#ifdef STREAM_NORMAL
for(j=0;j<64;j++){
  DBSCG(fprintf(stderr,"postcall prerot cb[%2i]=",j));
  DBSCG(dump_mem("",(unsigned char *)(cb+BYPG*j),BYPG,BYPG));
}
...
for(j=0;j<64;j++){
  DBSCG(fprintf(stderr,"postcall postrot cb[%2i]=",j));
  DBSCG(dump_mem("",(unsigned char *)(cb+BYPG*j),BYPG,BYPG));
}
#endif

#ifdef STREAM_INIT
  DBSCG(fprintf(stderr,":::::::::: END STREAM INIT\n"));
#endif
#ifdef STREAM_NORMAL
  DBSCG(fprintf(stderr,":::::::::: END STREAM NORMAL\n"));
#endif
