...
int cTripleDes::PadMessage(unsigned char *data, int len)
{
  ...
  if(len+noPadBytes+1 >= CWS_NETMSGSIZE-8) {
    PRINTSCF(L_CC_NEWCAMD,"message overflow in cTripleDes::PadMessage");
    return -1;
    }
...
{
  if((len-2) % 8 || (len-2)<16) {
    PRINTSCF(L_CC_NEWCAMD,"warning: encrypted data size mismatch");
    return;
    }
  DES_cblock ivec;
...
void cCardClientNewCamd::InitProtoVers(int vers)
{
  ...
  PRINTSCF(L_CC_NEWCAMD,"now using protocol version %d (cdLen=%d)",protoVers,cdLen);
  loginOK=false;
}
...
bool cCardClientNewCamd::SendMessage(const unsigned char *data, int len, bool UseMsgId, const struct CustomData *cd, comm_type_t commType)
{
  if(len<3||len+cdLen+4>CWS_NETMSGSIZE) {
    PRINTSCF(L_CC_NEWCAMD,"bad message size %d in SendMessage",len);
    return false;
    }
  ...
  if((len=cTripleDes::PadMessage(netbuf,len))<0) {
    PRINTSCF(L_CC_NEWCAMD,"PadMessage failed");
    return false;
    }
  if((data=cTripleDes::Encrypt(netbuf,len,netbuf))==0) {
    PRINTSCF(L_CC_NEWCAMD,"Encrypt failed");
    return false;
    }
...
int cCardClientNewCamd::ReceiveMessage(unsigned char *data, bool UseMsgId, struct CustomData *cd, comm_type_t commType)
{
  ...
  if(cCardClient::RecvMsg(netbuf,2)<0) {
    PRINTSCF(L_CC_NEWCAMD,"failed to read message length");
    return 0;
    }
  int mlen=WORD(netbuf,0,0xFFFF);
  if(mlen>CWS_NETMSGSIZE-2) {
   PRINTSCF(L_CC_NEWCAMD,"receive message buffer overflow");
   return 0;
   }
  if(cCardClient::RecvMsg(netbuf+2,mlen,200)<0) {
    PRINTSCF(L_CC_NEWCAMD,"failed to read message");
    return 0;
    }
  mlen+=2;
  cTripleDes::Decrypt(netbuf,mlen); mlen-=sizeof(DES_cblock);
  if(XorSum(netbuf+2,mlen-2)) {
    PRINTSCF(L_CC_NEWCAMD,"checksum error");
    return 0;
    }
    ...
    switch(commType) {
      ...
      case COMMTYPE_CLIENT:
        if(netMsgId!=WORD(netbuf,2,0xFFFF)) {
          PRINTSCF(L_CC_NEWCAMD,"bad msgid %04x != %04x ",netMsgId,WORD(netbuf,2,0xFFFF));
          return -2;
          }
        break;
      default:
        PRINTSCF(L_CC_NEWCAMD,"unknown commType %x",commType);
        return -1;
      }
    }
...
bool cCardClientNewCamd::Init(const char *config)
{
  ...
  char str[32];
  PRINTSCF(L_CC_CORE,"%s: username=%s password=%s key=%s",name,username,password,HexStr(str,configKey,14));
  return true;
}
...
bool cCardClientNewCamd::Login(void)
{
  ...
  if(so.Read(randData,sizeof(randData))<0) {
    PRINTSCF(L_CC_NEWCAMD,"no connect answer from %s:%d",hostname,port);
    Logout();
    return false;
    }
  ...
  if(!SendMessage(buffer,buffer[2]+3,true) || CmdReceive()!=MSG_CLIENT_2_SERVER_LOGIN_ACK) {
    PRINTSCF(L_CC_NEWCAMD,"failed to login to cardserver for username %s (proto %d)",username,protoVers);
  ...
  if(buffer[0] == MSG_CARD_DATA) {
    ...
    if(emmProcessing && !emmAllowed)
      PRINTSCF(L_CC_EMM,"%s: EMM disabled from config",name);
    }
  loginOK=true;
  return true;
}
...
bool cCardClientNewCamd::ProcessECM(const cEcmInfo *ecm, const unsigned char *data, unsigned char *cw, int cardnum)
{
  ...
  while((n=ReceiveMessage(buffer,true))==-2)
    PRINTSCF(L_CC_NEWCAMD,"msg ID sync error. Retrying...");
  switch(n) {
    case 19: // ecm was decoded
      ...
      return true;
    case 3:
      PRINTSCF(L_CC_ECM,"%s: card was not able to decode the channel",name);
      break;
    default:
      PRINTSCF(L_CC_NEWCAMD,"unexpected server response (code %d)",n);
      break;
    }
  return false;
}
      ...
      while((data=ad.Assembled())) {
        ...
        if(id>0 || emmAllowed>1) {
          if(SendMessage(data,len,true,0)) {
            ...
            if(len>=3) {
              if(!(buffer[1]&0x10))
                PRINTSCF(L_CC_EMM,"EMM rejected by card");
              }
            else
              PRINTSCF(L_CC_NEWCAMD,"unexpected server response (code %d)",len);
            }
