...
private:
  ...
  int length;
#if REELVDR
  cString placeHolderText; /* shown when 'value' is empty */
#endif  
#if REELVDR
  char *allowed;
#else
...
public:
#if REELVDR
  cMenuEditStrItem(const char *Name, char *Value, int Length, const char *Allowed = NULL, const char* PlaceHolderText = NULL);
#else
