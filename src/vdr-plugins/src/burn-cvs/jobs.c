...
#if APIVERSNUM >= 10716
         m_isTS( !recording_->IsPesRecording() )
#else
...
#define MAXCHAPTER (4 * 60 * 60)
   ...
   double job::get_requant_factor( bool cut_ ) const
   {
      ...
      /* TB: FIXME: requant_factor-calculation fails for HD-recordings */
      /*     but as requantization is not done at all - return constant value */  
      return videoSize > diskFree ? 0.0 : 1.0;
#if 0
...
//         return global_setup().CustomDiskSize;
   ...
   const char* job::get_disk_size_capacity() const
   {
    ...
    /*sets recommended disk size
     *   for sizes GREATER than 0 and less than 50000MB
     *   else none is returned*/
   void job::set_recommended_disk() 
   {
        int sizeMB = get_tracks_size() / MEGABYTE(1);
        int index = 0;

        if (get_tracks_size()>0)
        {
            /*find disk that is large enough to accommodate sizeMB of data*/
            for ( index = 1; //skip the first value, corresponds to disk "none"
                    index < disksize_count && disksize_values[index] < sizeMB ;
                    ++index)
                    ; // yes, Empty loop body!

            if (index >= disksize_count || index < 0) index = 0;
        }   

        m_options.DiskSize = index; 
   }
