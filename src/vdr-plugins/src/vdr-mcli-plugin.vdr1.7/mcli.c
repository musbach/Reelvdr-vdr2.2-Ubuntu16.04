...
cOsdObject *cPluginMcli::AltMenuAction (void)
{
	...
	mmi_info_t m;
	if (CamPollText (&m) > 0) {
		dsyslog ("NetCeiver %s CAM slot %d Received %s valid for:\n", m.uuid, m.slot, m.mmi_text);
...
#ifdef DEBUG_RESOURCES
				dsyslog("Can Steal CAM on slot %d from %d\n", slot, d->d()->CardIndex()+1);
#endif
				if(force) {
					d->d ()->SetTempDisable (true);
#ifdef DEBUG_RESOURCES
					dsyslog("Stole CAM on slot %d from %d\n", slot, d->d()->CardIndex()+1);
#endif
				}
				return true;
			}
		...
		for (int n = 0; n < nc_list->nci_num; n++) {
			...
			if ((now - nci->lastseen) > MCLI_DEVICE_TIMEOUT) {
				if(CAMPoolDel(nci->uuid)) {
					dsyslog  ("mcli: Remove CAMs from NetCeiver %s\n", nci->uuid);
					isyslog ("mcli: Remove CAMs from NetCeiver %s\n", nci->uuid);
				}
			} else {
				int cpa = CAMPoolAdd(nci);
				if(cpa==1) {
					dsyslog ("mcli: Add CAMs from NetCeiver %s -> %d\n", nci->uuid, cpa);
