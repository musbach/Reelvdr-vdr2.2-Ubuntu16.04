...
bool cCardClientRadegast::CheckLength(const unsigned char *buff, int len)
{
  ...
  if(len>255) PRINTSCF(L_CC_RDGD,"cmd %02x: nano too long %d > 255",buff[0],len);
  else if(l>max) PRINTSCF(L_CC_RDGD,"cmd %02x: msg too long %d > %d",buff[0],l,max);
  return l<=max;
}
...
bool cCardClientRadegast::Login(void)
{
  ...
  PRINTSCF(L_CC_LOGIN,"%s: connected to %s:%d",name,hostname,port);
  ...
  if(buff[0]==0x91) {
    PRINTSCF(L_CC_RDGD,"got server hello, assuming V4 mode");
  ...
  else PRINTSCF(L_CC_RDGD,"no server hello, assuming old mode");
    ...
    for(int l=GetNanoStart(buff); l<len; l+=buff[l+1]+2) {
      switch(buff[l]) {
        case 0x05:
          ...
          else PRINTSCF(L_CC_RDGD,"wrong cw length %d from server",buff[l+1]);
          break;
        case 0x04:
          PRINTSCF(L_CC_ECM,"%s: key not available from server",name);
          break;
        default:
          PRINTSCF(L_CC_RDGD,"unknown nano %02x in ECM response",buff[l]);
          break;
        }
      }
    }
  else PRINTSCF(L_CC_RDGD,"bad ECM response from server %02x != 02",buff[0]);
  return false;
}
