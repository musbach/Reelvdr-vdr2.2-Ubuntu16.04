...
bool cViacessExtendedKeys::GetKey(int provId, char keyId, int keyNr, unsigned char *key, int len, int log, cPlainKey **ppk)
{
  ...
  if(pk && pk->Size()==len) {
    pk->Get(key);
    if(log>1) PRINTSCF(L_SYS_KEY,"found key \"%s\"",*pk->ToString(true));
    if(ppk) *ppk = pk;
    return true;
    }
  if(log) PRINTSCF(L_SYS_KEY,"missing key \"V %06X %c1 ...\"",provId, keyId);
  return false;
}
...
void cSystemViaccess::ParseCADescriptor(cSimpleList<cEcmInfo> *ecms, unsigned short sysId, int source, const unsigned char *data, int len)
{
  ...
  if(pid>=0xAA && pid<=0xCF) {
    PRINTSCF(L_CORE_ECMPROC,"viaccess: dropped \"fake\" ecm pid 0x%04x",pid);
    return;
    }
            ...
            switch(parm) {
              ...
              case 0xEF: // crypted key(s)
                ...
                if(plen==sizeof(newKey[0])) {
                  ...
                  PRINTSCF(L_SYS_EMM,"key length mismatch %d!=%d",plen,(int)sizeof(newKey[0]));
                  cnt=scanlen;
                  }
                break;
              case 0xF0: // signature
                ...
                if(!memcmp(&scan[cnt],hbuff,sizeof(hbuff))) {
                  ...
                  while(numKeys--) {
                    ...
                    PRINTSCF(L_SYS_EMM,"%02X%02X %02X %s %s - KEY %06X.%02X -> %s",
                  ...
                  PRINTSCF(L_SYS_EMM,"%02X%02X %02X %s %s - FAIL",mkey->ident[0],mkey->ident[1],mkey->keyno,addr,ptext[updtype]);
                cnt=scanlen;
                break;
                }
              }
