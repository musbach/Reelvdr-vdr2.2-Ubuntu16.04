...
bool cIrdCardInfos::Update(cIrdCardInfo *ci, const unsigned char *pmk, const unsigned char *id)
{
  ...
  if(memcmp(ci->PMK,pmk,sizeof(ci->PMK))) {
    PRINTSCF(L_GEN_INFO,"new PMK for I card %s: %s",HexStr(str2,ci->hexSer,sizeof(ci->hexSer)),KeyStr(str,pmk));
    memcpy(ci->PMK,pmk,sizeof(ci->PMK));
    res=true;
    }
  if(id && memcmp(ci->provId,id,sizeof(ci->provId))) {
    PRINTSCF(L_GEN_INFO,"new PrvID for I card %s: %s",HexStr(str2,ci->hexSer,sizeof(ci->hexSer)),HexStr(str,id,sizeof(ci->provId)));
...
bool cSystemIrd::ProcessECM(const cEcmInfo *ecm, unsigned char *source)
{
  ...
  if(data==0 || date==-1) {
    if(doLog) PRINTSCF(L_SYS_ECM,"incomplete ECM structure");
    return false;
    }
...
void cSystemIrd::ProcessEMM(int pid, int caid, const unsigned char *data)
{
  ...
  if(maxIndex>n-5) {
    PRINTSCF(L_SYS_EMM,"%d: bad packet length (%d > %d)",CardNum(),maxIndex,n-5);
    maxIndex=n-5;
    }
  bool badnano=false;
  while(!badnano && index<maxIndex) {
	...
	break;
      ...
      default:
        PRINTSCF(L_SYS_EMM,"%d: unhandled nano 0x%02x",CardNum(),nano);
        break;
      }
    index+=nlen+2;
    }
  if(badnano || index!=maxIndex) {
    PRINTSCF(L_SYS_EMM,"%d: bad nano/bad paket",CardNum());
    return;
    }
