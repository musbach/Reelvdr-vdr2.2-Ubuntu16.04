...
bool cSystemFakeNDS::MapFlash(const char *name, int len, const unsigned char *appmd)
{
  ...
  if(!fm->Addr()) {
    ...
    if(fm->Size()<len || memcmp(md,appmd,16)) {
      mapMD5ok=false;
      if(doLog) PRINTSCF(L_SYS_ECM,"MD5 checksum failed on flash bin");
      return false;
      }
    mapMD5ok=true;
    }
...
bool cSystemFakeNDS::ProcessECM(const cEcmInfo *ecm, unsigned char *data)
{
  ...
  if(len<7 || data[3]!=0x00 || data[4]!=0x00 || data[5]!=0x01) {
    if(doLog) PRINTSCF(L_SYS_ECM,"ECM format check failed");
    return false;
    }
  int iLen=data[6];
  if(iLen+6>len || iLen<0x1C) {
    if(doLog) PRINTSCF(L_SYS_ECM,"IRD block length check failed");
    return false;
    }
  ...
  if(iLen==0x1C && data[6+11]==0x0F && data[6+12]==0x40) { // PW
    ...
    if(!(data[6+9]&1) || !(data[6+10]&16)) {
      if(doLog) PRINTSCF(L_SYS_ECM,"no CW in IRD block");
      return false;
      }
    ...
    if(doLog) PRINTSCF(L_SYS_ECM,"failed to detect provider");
    return false;
    }
