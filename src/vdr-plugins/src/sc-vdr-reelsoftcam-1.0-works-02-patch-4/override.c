...
char *cValidityRange::Parse3(char *s)
{
  ...
  if(log) PRINTSCF(L_CORE_LOAD,"override: RANGE format error");
  return 0;
}
...
bool cValidityRange::ParseCaidRange(const char *str)
{
  if(sscanf(str,"%x-%x",&fromCaid,&toCaid)<1) {
    PRINTSCF(L_CORE_LOAD,"override: CAID format error");
    return false;
    }
  if(fromCaid<0x0001 || fromCaid>0xFFFF ||
     (toCaid>0 && (toCaid<0x0001 || toCaid>0xFFFF || fromCaid>toCaid))) {
    PRINTSCF(L_CORE_LOAD,"override: CAID range error");
    return false;
    }
  return true;
}
...
bool cValidityRange::ParseFreqRange(const char *str)
{
  if(sscanf(str,"%d-%d",&fromFreq,&toFreq)<1) {
    PRINTSCF(L_CORE_LOAD,"override: FREQ format error");
    return false;
    }
  if(fromFreq<1 || fromFreq>50000 ||
     (toFreq>0 && (toFreq<1 || toFreq>50000 || fromFreq>toFreq))) {
    PRINTSCF(L_CORE_LOAD,"override: FREQ range error");
    return false;
    }
  return true;
}
...
bool cValidityRange::ParseSourceRange(const char *str)
{
  ...
  if((l=sscanf(str,"%a[^-:]-%a[^-:]",&s1,&s2))>=1) {
    if(s1 && (fromSource=cSource::FromString(s1))>0 &&
      ...
      else PRINTSCF(L_CORE_LOAD,"override: SOURCE range error");
      }
    else PRINTSCF(L_CORE_LOAD,"override: SOURCE parse error");
    }
  else PRINTSCF(L_CORE_LOAD,"override: SOURCE format error");
  free(s1); free(s2);
  return res;
}
...
bool cOverrideCat::Parse(char *str)
{
  if((str=Parse2(str))) {
    if(sscanf(str,"%x:%x",&caid,&pid)==2) {
      PRINTSCF(L_CORE_OVER,"cat: %s - caid %04x pid %04x",*Print(),caid,pid);
      return true;
      }
    PRINTSCF(L_CORE_LOAD,"override: CAT format error");
    }
  return false;
}
...
{
  PRINTSCF(L_CORE_OVER,"cat: added caid %04x pid %04x",caid,pid);
  buff[0]=0x09;
  buff[1]=0x04;
  buff[2]=(caid>>8)&0xFF;
  buff[3]= caid    &0xFF;
...
bool cOverrideEmmCaid::Parse(char *str)
{
  if((str=Parse3(str))) {
    if(sscanf(str,"%x",&caid)==1) {
      PRINTSCF(L_CORE_OVER,"emmcaid: %s - caid %04x",*Print(),caid);
      return true;
      }
    PRINTSCF(L_CORE_LOAD,"override: EMMCAID format error");
    }
  return false;
}
...
{
  if(log) PRINTSCF(L_CORE_OVER,"emmcaid: %04x",caid);
  return caid;
}
...
bool cOverrideEcmTable::Parse(char *str)
{
  if((str=Parse3(str))) {
    if(sscanf(str,"%x",&table)==1) {
      PRINTSCF(L_CORE_OVER,"ecmtable: %s - table %02x",*Print(),table);
      return true;
      }
    PRINTSCF(L_CORE_LOAD,"override: ECMTABLE format error");
    }
  return false;
}
...
{
  if(log) PRINTSCF(L_CORE_OVER,"ecmtable: %02x",table);
  return table;
}
  ...
  if((str=Parse3(str))) {
    ...
    do {
      ...
      if(sscanf(&str[l],"%x%n/%x%n",&table[num],&n,&mask[num],&n)<1) {
        PRINTSCF(L_CORE_LOAD,"override: EMMTABLE format error");
        return false;
        }
      n+=l; num++;
...
unsigned char *cRewriter::Alloc(int len)
{
  ...
  if(!mem) PRINTSCF(L_CORE_OVER,"rewriter %s: failed to alloc rewrite buffer",name);
  return mem;
}
  ...
  if((str=Parse3(str))) {
    ...
    if(sscanf(str,"%x:%a[^:]",&caid,&name)>=1) {
      ...
      if(name) {
        ...
        else PRINTSCF(L_CORE_LOAD,"override: REWRITER name error");
        }
      else {
        strcpy(rw,"<none>");
        rewriterId=0;
      ...
      if(res) {
        PRINTSCF(L_CORE_OVER,"tunnel: %s - to %04x, rewriter %s",*Print(),caid,rw);
        }
      }
    else PRINTSCF(L_CORE_LOAD,"override: TUNNEL format error");
    free(name);
    }
  return res;
}
...
{
  if(log) PRINTSCF(L_CORE_OVER,"tunnel: to %04x (%d)",caid,rewriterId);
  if(id) *id=rewriterId;
  return caid;
}
  ...
  if((str=Parse2(str,true))) {
    ...
    do {
      ...
      if(sscanf(&str[l],"%x%n",&caid[num],&n)!=1) {
        PRINTSCF(L_CORE_LOAD,"override: IGNORE format error");
        return false;
        }
      n+=l; num++;
  ...
  if((str=Parse2(str))) {
    ...
    do {
      ...
      if(sscanf(&str[l],"%x%n/%x%n",&caid[num],&n,&prov[num],&n)<1) {
        PRINTSCF(L_CORE_LOAD,"override: ECMPRIO format error");
        return false;
        }
      if(prov[num]>=0 && !UsesProvId(caid[num])) {
        PRINTSCF(L_CORE_LOAD,"override: ECMPRIO provider ID not supported for caid %04x",caid[num]);
        return false;
        }
      n+=l; num++;
...
int cOverrideEcmPrio::GetPrio(int Caid, int Prov)
{
  ...
  PRINTSCF(L_CORE_OVER,"ecmprio: %04x/%x pri %d",Caid,Prov,pri);
  return pri;
}
