...
bool cSmartCardDataCryptoworks::Parse(const char *line)
{
  ...
  if(!strncasecmp(line,"IPK",3))       { type=dtIPK;  line+=3; }
    ...
    PRINTSCF(L_CORE_LOAD,"smartcarddatacryptoworks: format error: datatype");
    return false;
    }
  line=skipspace(line);
  if(GetHex(line,id,IdLen())!=IdLen()) {
    PRINTSCF(L_CORE_LOAD,"smartcarddatacryptoworks: format error: caid/serial");
    return false;
    }
  line=skipspace(line);
  if(type==dtPIN) {
    for(int i=0; i<4; i++) {
      if(!isdigit(*line)) {
        PRINTSCF(L_CORE_LOAD,"smartcarddatacryptoworks: format error: pin");
        return false;
        }
      pin[i]=*line++;
      }
    ...
    if(GetHex(line,buff,64,true)!=64) {
      PRINTSCF(L_CORE_LOAD,"smartcarddatacryptoworks: format error: ipk/ucpk");
      return false;
      }
...
bool cSmartCardCryptoworks::Init(void)
{
  if(atr->histLen<6 || atr->hist[1]!=0xC4 || atr->hist[4]!=0x8F || atr->hist[5]!=0xF1) {
    PRINTSCF(L_SC_INIT,"doesn't look like a Cryptoworks card");
    return false;
    }
  infoStr.Begin();
  ...
  else PRINTSCF(L_SC_ERROR,"reading MF-ID failed, using default 3F20");

  unsigned char Caid[2], serial[5];
  if(!SelectFile(0x2F01) || ReadRecord(buff,0xD1)<4) {
    PRINTSCF(L_SC_ERROR,"reading record 2f01/d1 failed");
    return false;
    }
  ...
  if(ReadRecord(buff,0x80)<7) {
    PRINTSCF(L_SC_ERROR,"reading record 2f01/80 failed");
    return false;
    }
  ...
  PRINTSCF(L_SC_INIT,"card v.%d (pindown=%d) caid %04x serial %s MF %04X",atr->hist[2],atr->hist[3],caid,HexStr(str,&buff[2],5),mfid);
  if(ReadRecord(buff,0x9F)>=3) {
    const char *n="(unknown)";
    if(ReadRecord(buff+10,0xC0)>=18) n=(char *)buff+10+2;
    infoStr.Printf("Issuer:     0x%02x (%.16s)\n",buff[2],n);
    PRINTSCF(L_SC_INIT,"card issuer: 0x%02x %.16s",buff[2],n);
    }
  if(ReadRecord(buff,0x9E)>=66) {
    ...
    if(entry) {
      PRINTSCF(L_SC_EXTRA,"got IPK from smartcard.conf");
      if(rsa.RSA(&buff[2],&buff[2],64,exp,entry->key,false)>0) {
        ...
        if(buff[2] == ((mfid&0xFF)>>1)) {
          ...
          ucpkValid=true;
          PRINTSCF(L_SC_INIT,"got UCPK from IPK/ISK");
          }
        else PRINTSCF(L_SC_ERROR,"UCPK check failed %02x != %02x",buff[2],(mfid&0xFF)>>1);
        }
      else PRINTSCF(L_SC_ERROR,"RSA failed for UCPK");
      }
    }
  if(!ucpkValid) {
    ...
    if(entry) {
      ...
      ucpkValid=true;
      PRINTSCF(L_SC_INIT,"got UCPK from smartcard.conf");
      }
    }
  if(!ucpkValid) PRINTSCF(L_GEN_WARN,"no valid UCPK for cryptoworks smartcard");
  ...
  for(unsigned int i=0; i<count ; i++) {
    if(SelectFile(0x1F00+provId[i])) {
      ...
      PRINTSCF(L_SC_INIT,"provider %d: 0x%02x %.16s",i,provId[i],n);
        ...
        if(SelectFile(fn[j])) {
          ...
          if(IsoWrite(ins,&ins[5]) && Status() && (l=GetLen())>0) {
            if(first) {
              ...
              PRINTSCF(L_SC_INIT,"| id | chid | stat | date              | version  | name        ");
              first=false;
              }
            infoStr.Printf(  "--+----+--+-----------------+------\n"
                           "%s\n",
            ...
            while(IsoRead(insb2,buff) && !EndOfData() && Status()) {
              ...
              PRINTSCF(L_SC_INIT,"| %02x | %04x |  %02x  | %s-%s | %08x | %.12s",
  ...
  if(disableParental) {
    ...
    if(SelectFile(0x2F11) && ReadRecord(buff,atr->hist[3])>=7) {
      pinOK=true;
      PRINTSCF(L_SC_INIT,"read PIN from card.");
      }
    if(!pinOK) {
      ...
      if(entry) {
        ...
        pinOK=true;
        PRINTSCF(L_SC_INIT,"got PIN from smartcard.conf.");
        }
      }
    if(pinOK) {
      ...
      PRINTSCF(L_SC_INIT,"your card PIN is %.4s",&buff[2]);
      // parental rating
      // 0x00      - undefined
      ...
      if(IsoWrite(ins,&buff[2]) && Status())
        PRINTSCF(L_SC_INIT,"parental rating set to %02x",buff[6]);
      else
        PRINTSCF(L_SC_ERROR,"failed to set parental rating.");
      }
    else PRINTSCF(L_SC_INIT,"no PIN available.");
    }
  ...
  if(l>5) {
    ...
    if(IsoWrite(ins4c,&data[5]) && Status() &&
      ...
      for(int i=0; i<l && r<2; ) {
        ...
        switch(buff[i]) {
          case 0xDB: // CW
            PRINTSCF(L_SC_EXTRA,"nano DB (cw)");
            if(n==0x10) {
            ...
            break;
          case 0xDF: // signature
            PRINTSCF(L_SC_EXTRA,"nano DF %02x (sig)",n);
            if(n==0x08) {
              if((buff[i+2]&0x50)==0x50) {
                if(buff[i+3]&0x01) PRINTSCF(L_SC_ERROR,"not decyphered. PIN protected?");
                else if(!(buff[i+5]&0x80)) PRINTSCF(L_SC_ERROR,"missing entitlement. Check your subscription");
                else r|=2;
                }
              }
              ...
              if(ucpkValid) {
                if(rsa.RSA(&buff[i+2],&buff[i+2],n,exp,ucpk,false)<=0) {
                  PRINTSCF(L_SC_ERROR,"camcrypt RSA failed.");
                  return false;
                  }
                HEXDUMP(L_SC_EXTRA,&buff[i+2],n,"after camcrypt");
                if(!memmem(&buff[i+2+4],n-4,nanoD4,sizeof(nanoD4))) {
                  PRINTSCF(L_SC_ERROR,"camcrypt failed. Check IPK/UCPK in your smartcard.conf!");
                  return false;
                  }
                r=0; l=n-4; n=4;
                }
              else {
                PRINTSCF(L_SC_ERROR,"valid UCPK needed for camcrypt! Check your smartcard.conf for a IPK/UCPK!");
                return false;
                }
              }
            break;
          default:
            PRINTSCF(L_SC_EXTRA,"nano %02x (unhandled)",buff[i]);
            break;
          }
        i+=n+2;
        }
