...
bool cAuxSrv::Login()
{
  ...
  PRINTSCF(L_SYS_MAP,"auxsrv: connecting to %s:%d",auxAddr,auxPort);
  ...
  if(so.Connect(auxAddr,auxPort)) {
    ...
    PRINTSCF(L_SYS_MAP,"auxsrv: login write failed");
    }
  so.Disconnect();
  return false;
}
...
{
  if(!auxEnabled) {
    PRINTSCF(L_SYS_MAP,"auxsrv: AUXserver is disabled!");
    return -1;
    }
  if(len>500 || outlen>500) return -1;
  cMutexLock lock(this);
  if(!so.Connected() && !Login()) return false;
  PRINTSCF(L_SYS_MAP,"auxsrv: calling map%02x",map);
        ...
        if(so.Read(buff+5,l,200)>0) {
          if(buff[4]==0x00) {
            ...
            if(l==outlen+4) {
              ...
              else PRINTSCF(L_SYS_MAP,"auxsrv: bad footer in map%02x response",map);
              }
            else PRINTSCF(L_SYS_MAP,"auxsrv: bad length in map%02x response (got=%d want=%d)",map,l-4,outlen);
            }
          else PRINTSCF(L_SYS_MAP,"auxsrv: map%02x not successfull (unsupported?)",map);
          }
        else PRINTSCF(L_SYS_MAP,"auxsrv: map%02x read failed (2)",map);
        }
      else PRINTSCF(L_SYS_MAP,"auxsrv: bad response to map%02x",map);
      }
    else PRINTSCF(L_SYS_MAP,"auxsrv: map%02x read failed",map);
    }
  else  PRINTSCF(L_SYS_MAP,"auxsrv: map%02x write failed",map);
  so.Disconnect();
  return -1;
}
#endif //HAS_AUXSRV
...
bool cN2Prov0101::Algo(int algo, const unsigned char *hd, unsigned char *hw)
{
  if(algo!=0x40 && algo!=0x60) {
    PRINTSCF(L_SYS_ECM,"%04X: unknown MECM algo %02x",id,algo);
    return false;
    }
  if(!Init(id,102)) {
    PRINTSCF(L_SYS_ECM,"%04X: failed to initialize ROM",id);
    return false;
    }
  ...
  if(!(pk=keys.FindKey('N',mecmKeyId,keyNr,sizeof(ideaKey)))) {
    PRINTSCF(L_SYS_KEY,"missing %04x %02x MECM key",mecmKeyId,keyNr);
    return false;
    }
  pk->Get(ideaKey);
  ...
  if(memcmp(data,mecmCode,8)) {
    PRINTSCF(L_SYS_ECM,"%04X: MECM %02x decrypt signature failed",id,keyNr);
    return false;
    }
...
bool cN2Prov0101::RomInit(void)
{
  ...
  while(!Run(7000)) {
    switch(GetPc()) {
      case 0x537d:
        PRINTSCF(L_SYS_EMU,"%04x: ROM init successfull",id);
        return true;
      default:
        PRINTSCF(L_SYS_EMU,"%04x: ROM init failed: unexpected breakpoint",id);
        break;
      }
    }
  return false;
...
bool cN2Prov0101::ProcessMap(int f)
{
  ...
  switch(f) {
    ...
    case 0x57:
      ...
      break;
    default:
      PRINTSCF(L_SYS_EMU,"%04x: map call %02x not emulated",id,f);
      return false;
    }
  ...
  switch(f) {
    ...
    case 0x0f:  // set DES size
      ...
      if(desSize!=0x10 && desSize!=0x18) {
        PRINTSCF(L_SYS_EMU,"%04x: invalid DES key size %02x",id,desSize);
        return false;
        }
      break;
    default:
      PRINTSCF(L_SYS_EMU,"%04x: DES map call %02x not emulated",id,f);
      return false;
  }
  return true;
}
...
bool cN2Prov0101::RomCallbacks(void)
{
  ...
  switch(ea) {
    ...
    case 0x5F5E: //BrainDead
      PRINTSCF(L_SYS_EMU,"%04x: emu hung at %04x",id,ea);
      return false;
    ...
    default:
      PRINTSCF(L_SYS_EMU,"%04X: unknown ROM breakpoint %04x",id,ea);
      return false;
    }
  if(dopop) {
    if(ea>=0x8000) PopCr();
...
void cN2Prov0101::TimerHandler(unsigned int num)
{
  if(hwMapper) {
    ...
    for(int t=0; mask; mask>>=1,t++)
      if(mask&1) {
        if(t==2) {
          PRINTSCF(L_SYS_EMU,"0101: Timer interrupt %u @ %04x",t,GetPc());
          RaiseException(9);
