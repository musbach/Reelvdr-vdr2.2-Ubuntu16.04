...
bool cSmartCardSeca::Init(void)
{
  ...
  if(atr->T!=0 || atr->histLen<7 || memcmp(&atr->hist[3],atrTester,4)) {
    PRINTSCF(L_SC_INIT,"doesn't looks like a Seca card");
    return false;
    }
  ...
  PRINTSCF(L_SC_INIT,"cardtype: %s %d.%d",type,atr->hist[2]&0x0F,atr->hist[2]>>4);
  
  ResetIdSet();
  unsigned char buff[MAX_LEN];
  if(!IsoRead(ins0e,buff) || !Status()) {
    PRINTSCF(L_SC_ERROR,"reading card serial failed");
    return false;
    }
  SetCard(new cCardSeca(&buff[2]));
  PRINTSCF(L_SC_INIT,"card serial number: %llu",Bin2LongLong(&buff[2],6));
  infoStr.Printf("Type: %s %d.%d  Serial: %llu\n",type,atr->hist[2]&0x0F,atr->hist[2]>>4,Bin2LongLong(&buff[2],6));

  if(!IsoRead(ins16,buff) || !Status()) {
    PRINTSCF(L_SC_ERROR,"reading provider map failed");
    return false;
    }
  ...
  if(LOG(L_SC_INIT)) {
    ...
    PRINTSCF(L_SC_INIT,"card has %d providers (0x%04x)",n,provMap);
    }

  for(int i=0 ; i<16 ; i++) {
    if(provMap&(1<<i)) {
      PRINTSCF(L_SC_INIT,"reading info for provider index %d",i);
      ins12[2]=i;
      if(!IsoRead(ins12,buff) || !Status()) {
        PRINTSCF(L_SC_ERROR,"reading provider info failed");
        return false;
        }
      ins32[2]=i;
      ...
      char str[20];
      PRINTSCF(L_SC_INIT,"provider 0x%02x%02x '%.16s' expires %s pbm %s",
...
bool cSmartCardSeca::CheckAccess(const unsigned char *data, const cProviderScSeca *p)
{
  ...
  if(sci->date[0]>p->date[0] || (sci->date[0]==p->date[0] && sci->date[1]>p->date[1]))
    return false;
  char str[20];  
  PRINTSCF(L_SC_PROC,"channelinfo date %s pbm %s",Date(sci->date),HexStr(str,sci->pbm,sizeof(sci->pbm)));
  int result=0;
...
bool cSmartCardSeca::Decode(const cEcmInfo *ecm, const unsigned char *data, unsigned char *cw)
{
  ...
  if(p && ecm->Data()) {
    PRINTSCF(L_SC_PROC,"provider 0x%04x index %d '%.16s' (expires %s)",cParseSeca::ProvId(data),p->index,p->name,Date(p->date));
    ...
    else PRINTSCF(L_SC_ERROR,"update your subscription to view this channel");
    }
  return false;
}
...
bool cSmartCardSeca::Update(int pid, int caid, const unsigned char *data)
{
  ...
  if(blocker==0 || (data[0]==0x82 && blocker==2) || (data[0]==0x84 && blocker==1)) {
    cProviderScSeca *p=(cProviderScSeca *)FindProv(data);
    if(p && MatchEMM(data)) {
      PRINTSCF(L_SC_PROC,"got %s update",data[0]==0x82?"UNIQUE":"SHARED");
      const unsigned char *payload;
