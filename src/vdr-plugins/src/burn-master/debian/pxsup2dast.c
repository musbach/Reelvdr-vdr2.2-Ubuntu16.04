#if 0 /*
 #
 # $Id: pxsup2dast.c 2308 2009-01-11 14:53:57Z too $
 #
...
static void get_line(eu8 ** data, int width, Png4File * picfile) {
    int nibble = -1;
    int col = 0;
    int number, cindex;
...
static void makebitmap(eu8 * data, int w, int h, int top, int bot,
    ...
    for (i = 0; i < h / 2; i++) {
	get_line(&top_ibuf, w, &picfile);
	get_line(&bot_ibuf, w, &picfile); }
