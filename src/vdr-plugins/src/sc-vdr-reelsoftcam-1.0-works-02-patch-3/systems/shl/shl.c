...
:cAction("pidhopper",DvbNum)
{
  ...
  PRINTSCF(L_SYS_HOPPER,"pid hopper started (%d filters)",numFilters);
}
...
cPidHopper::~cPidHopper()
{
  ...
  PRINTSCF(L_SYS_HSTATS,"active pids: (now=%d)\n",now);
  for(int i=0; i<numFilters; i++) {
    struct FilterData *dd=&d[i];
    if(dd->filter) {
      if(dd->filter->Pid()>=0)
        PRINTSCF(L_SYS_HSTATS,"%2d. pid %04x lastTime=%4d lastRead=%4d created=%4d count=%4d",
                num++,
                dd->filter->Pid(),
...
void cPidHopper::PurgePacket(struct FilterData *dd, struct FilterPacket *p)
{
  while(dd->first) {
    ...
    dd->first=del->next;
    if(dd->filter) PRINTSCF(L_SYS_HOPPER,"PID %04x purging packet %p (time=%d)",dd->filter->Pid(),del,del->lastTime);
    free(del);
    if(del==p) break;
    }
...
{
  Lock();
  PRINTSCF(L_SYS_HOPPER,"cutoff (now=%d)",now);
  for(int i=0; i<numFilters; i++) {
    struct FilterData *dd=&d[i];
      ...
      if(d[i].filter==filter) {
        do {
          ...
          if(p) {
            ...
            d[i].last=p;
            PRINTSCF(L_SYS_HOPPER,"PID %04x queued packet %p (time=%d len=%d) (%s)",d[i].filter->Pid(),p,p->lastTime,l,d[i].first==p?"first":"queue");
            }
          len-=l; data+=l;
          } while(len>0);
        sleep.Broadcast();
...
int cPidHopper::Read(int pid, int & now, unsigned char *buff, int len, int timeout)
{
  Lock();
  timeout+=time_ms();
  PRINTSCF(L_SYS_HOPPER,"PID %04x read (now=%d)",pid,now);
  int n=-1;
  while(time_ms()<timeout) {
    ...
    for(struct FilterPacket *p=dd->first; p; p=p->next) {
      if(p->lastTime>=now) {
        ...
        now=p->lastTime;
        PRINTSCF(L_SYS_HOPPER,"PID %04x returning packet %p (time=%d)",dd->filter->Pid(),p,p->lastTime);
        PurgePacket(dd,p);
        dd->lastRead=time_ms();
        dd->readCount++;
        dd->lastTime=dd->first ? dd->first->lastTime : -1;
        ...
        PRINTSCF(L_SYS_HOPPER,"PID %04x skipping packet %p (time=%d)",dd->filter->Pid(),p,p->lastTime);
        if(!p->next) {
          PRINTSCF(L_SYS_HOPPER,"PID %04x queue empty",dd->filter->Pid());
          PurgePacket(dd,p);
          break;
          }
        }
...
bool cSystemShl::ProcessECM(const cEcmInfo *ecm, unsigned char *source)
{
  ...
  if(SCT_LEN(source)<(ENC_OFF+ENC_LEN+3)) {
    PRINTSCF(L_SYS_VERBOSE,"short ecm");
    return false;
    }
  if(memcmp(tester,&source[3],sizeof(tester))) { // new SHL
    ...
    if(!hasPrvId) {
      ...
      if(!hasPrvId) {
        PRINTSCF(L_SYS_ECM,"no provider ID (logger enabled?)");
        mutex.Unlock();
        return false;
        }
      }
    ...
    if(!prv) {
      PRINTSCF(L_SYS_ECM,"provider %04x not supported",prvId);
      mutex.Unlock();
      return false;
      }
    mutex.Unlock();
    if(!ph) {
      PRINTSCF(L_SYS_ECM,"using card %d",CardNum());
      ph=new cPidHopper(CardNum(),prv->filters);
      if(!ph) {
        PRINTSCF(L_SYS_ECM,"no pid hopper");
        return false;
        }
#if 1
...
{
  if(SCT_LEN(data)<150) {
    PRINTSCF(L_SYS_VERBOSE,"data is too short");
    return;
    }
  const unsigned int dataStart=data[33];
  if((dataStart+35)>184) {
    PRINTSCF(L_SYS_VERBOSE,"data broken (%d)",dataStart);
    return;
    }
  ...
  for(int i=0; i<n;) {
    switch(buffer[i]) {
      case 0x0a:
        ...
        if(!hasPrvId) {
          const struct ShlProv *prv=FindProv(prvId);
          PRINTSCF(L_SYS_EMM,"provider id %04x (%s)\n",prvId,prv ? prv->name:"unknown");
          wait.Broadcast();
          }
        hasPrvId=true;
        mutex.Unlock();
