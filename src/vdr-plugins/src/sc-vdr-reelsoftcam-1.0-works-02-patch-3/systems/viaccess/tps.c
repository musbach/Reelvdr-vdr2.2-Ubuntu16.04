...
void cTransponderTime::Set(time_t st)
{
  ...
  if(sattime<0 || reftime.Elapsed()>5*1000) {
    ...
    PRINTSCF(L_SYS_TIME,"%x:%x: %s (delta %ld)",source,transponder,str,st-now);
    }
}
...
time_t cTransponderTime::Now(void)
{
  ...
  if(sattime<0) {
    if(!wait.TimedWait(mutex,3*1000)) {
      PRINTSCF(L_SYS_TIME,"%x:%x: unsuccessfull wait",source,transponder);
      return -1;
      }
    }
...
cSatTime::cSatTime(int CardNum, int Source, int Transponder)
{
  ...
  if(!ttime) {
    ...
    PRINTSCF(L_SYS_TIME,"%x:%x: created new transpondertime",Source,Transponder);
    }
  else PRINTSCF(L_SYS_TIME,"%x:%x: using existing transpondertime",Source,Transponder);
  CheckHandler();
}
...
void cSatTime::CheckHandler(void)
{
  if(ttime) {
    if(!cSoftCAM::TriggerHook(cardNum,HOOK_SATTIME) && !ttime->HasHandler()) {
      ...
      PRINTSCF(L_SYS_TIME,"added hook");
      }
    }
}
...
int cOpenTVModule::AddPart(const unsigned char *data, int slen)
{
  if(Id(data)==id) {
    ...
    if(mlen!=modlen || (off+rec)>mlen || !mem) {
      PRINTSCF(L_SYS_TPSAU,"length mismatch while adding to OpenTV module");
      return -1;
      }
    ...
    if(received==mlen) {
      if(!Decompress()) {
        PRINTSCF(L_SYS_TPSAU,"failed to decompress OpenTV module");
        return -1;
        }
      if(!ParseSections()) {
        PRINTSCF(L_SYS_TPSAU,"DATA & CODE section not located in OpenTV module");
        return -1;
        }
      Dump();
      return 1;
...
#warning Dumping TPS AU data
  ...
  if(fd>=0) {
    write(fd,mem,modlen);
    close(fd);
    PRINTSCF(L_SYS_TPSAU,"dumped to file '%s'",fname);
    }
  }
#endif
}
  ...
  for(int idx=0; idx<modlen;) {
	...
	data.data=mem+idx+8;
      ...
      case COMP_SECTION_HDR:
        PRINTSCF(L_GEN_DEBUG,"OpenTV module still compressed in ParseSections()");
        return 0;
      default:
        PRINTSCF(L_SYS_TPSAU,"unknown section hdr %08x (%c%c%c%c) in OpenTV module",hdr,(hdr>>24)&0xFF,(hdr>>16)&0xFF,(hdr>>8)&0xFF,hdr&0xFF);
        break;
      }
    idx+=s_len;
    }
    ...
    for(int cnt=8; cnt>0; mask<<=1,cnt--) {
      if(mask&0x80) {
        ...
        if(from<out_start || from>=out_ptr || len>(out_end-out_ptr)) {
          PRINTSCF(L_SYS_TPSAU,"length mismatch in OpenTV decompress");
          return false;
          }
      ...
      if(out_end<=out_ptr) {
        if(out_end!=out_ptr) {
          PRINTSCF(L_SYS_TPSAU,"pointer mismatch in OpenTV decompress");
          return false;
          }
...
const cTpsKey *cTpsKeys::GetV2Key(int id)
{
  ...
  else PRINTSCF(L_SYS_KEY,"missing %.4x TPS key\n",id);
  return 0;
}
...
void cTpsKeys::Check(time_t now, int cardnum)
{
  ...
  if(tpsAuMode==AU_TPSBIN && lastLoad.Elapsed()>(nokey ? LOADBIN_TIME/60 : LOADBIN_TIME)) {
    PRINTSCF(L_SYS_TPSAU,"loading "TPSBIN" triggered");
    LoadBin();
    if(now>0) Purge(now);
    lastLoad.Set();
    }
  if(tpsAuMode==AU_STREAM && lastAu.Elapsed()>(nokey ? TPSAU_TIME/60 : TPSAU_TIME)) {
    if(ScSetup.AutoUpdate>0) {
      PRINTSCF(L_SYS_TPSAU,"TPS AU triggered");
      if(!cSoftCAM::TriggerHook(cardnum,HOOK_TPSAU)) {
        cTpsAuHook *hook=new cTpsAuHook;
        cSoftCAM::AddHook(cardnum,hook);
        PRINTSCF(L_SYS_TPSAU,"TPS AU hook added");
        }
      }
    lastAu.Set();
    }
...
{
  PRINTSCF(L_SYS_TPSAU,"purging TPS keylist");
  bool del=false;
  ListLock(true);
...
void cTpsKeys::GetFirstLast(void)
{
  if(Count()>0) {
    ...
    for(; k; k=Next(k)) {
      if(k->Timestamp()<last)
        PRINTSCF(L_SYS_TPSAU,"TPS keys not in accending order!");
      last=k->Timestamp();
      }
    PRINTSCF(L_SYS_TPS,"%d TPS keys available (from %s to %s)",Count(),*Time(first),*Time(last));
    ListUnlock();
    }
  else {
    last=first=0;
    PRINTSCF(L_SYS_TPS,"no TPS keys available");
    }
}

bool cTpsKeys::ProcessAu(const cOpenTVModule *mod)
{
  PRINTSCF(L_SYS_TPSAU,"processing TPS AU data");
...
*/
  ...
  if(!kd || !cb1 || !cb2 || !cb3) {
    PRINTSCF(L_SYS_TPSAU,"couldn't locate all pointers in data section (%d,%d,%d,%d)",kd,cb1,cb2,cb3);
    return false;
    }

  unsigned int end=(kd>cb1 && kd>cb2 && kd>cb3) ? kd : datahdr->dlen;
  unsigned int off=min(cb1,min(cb2,cb3))-2;
  PRINTSCF(L_SYS_TPSAU,"pointers in data section kd=%d cb1=%d cb2=%d cb3=%d - dlen=%d off=%d end=%d - cb1=%d cb2=%d cb3=%d",kd,cb1,cb2,cb3,datahdr->dlen,off,end,cb1-off,cb2-off,cb3-off);
  ...
  if(!pk) {
    PRINTSCF(L_SYS_KEY,"missing V 7C00 TPSMK%d key",mkidx);
    return false;
    }
  pk->Get(key);

  if(sec[6]>=d+datahdr->dlen) {
    PRINTSCF(L_SYS_TPSAU,"section 5 exceeds buffer");
    return false;
    }
  int keylen=0;
  ...
  if(sec[6]+keylen>=d+datahdr->dlen) {
    PRINTSCF(L_SYS_TPSAU,"section 6 exceeds buffer");
    return false;
    }
  ...
  PRINTSCF(L_SYS_TPSAU,"got %d keys from AU data",nlist->Count());
...
bool cTpsKeys::LoadBin(void)
{
  ...
  if(!tpsbin) {
    PRINTSCF(L_SYS_TPS,"no filemap for "TPSBIN);
    return false;
    }
  else if(!tpsbin->Map()) {
    PRINTSCF(L_SYS_TPS,"mapping failed for "TPSBIN);
    return false;
    }
  const unsigned char *a=tpsbin->Addr();
  int len=tpsbin->Size();
  if(len<(68+56) || memcmp(mark,a+32,sizeof(mark)) || memcmp(mark,a+48,sizeof(mark))) {
    PRINTSCF(L_SYS_TPS,TPSBIN" format not recognised");
    tpsbin->Unmap();
    return false;
    }
  ...
  a+=68; len-=68;
  if(len<size) {
    PRINTSCF(L_SYS_TPS,TPSBIN" key data length mismatch");
    tpsbin->Unmap();
    return false;
    }
  ...
  PRINTSCF(L_SYS_TPSAU,"loaded %d keys from "TPSBIN" file",nlist->Count());
  Join(nlist);
  ...
  while(len>64) {
    if(!memcmp(alg_mark,a,sizeof(alg_mark)) && *(a+11)==0x02) {
      ...
      if(len<size) {
        PRINTSCF(L_SYS_TPS,TPSBIN" algo data length mismatch");
        break;
        }
      ...
      PRINTSCF(L_SYS_TPSAU,"algo pointers in "TPSBIN" off=%04x cb1=%04x cb2=%04x cb3=%04x size=%04x",off,cb1,cb2,cb3,size);
...
void cTpsKeys::PreLoad(void)
{
  ...
  if(!loadlist) PRINTSCF(L_SYS_TPS,"no memory for loadlist");
}
...
bool cTpsKeys::ParseLinePlain(const char *line)
{
  ...
  if(line[0]=='X') {
    if(line[1]=='S') {
      if(algomem) PRINTSCF(L_SYS_TPS,"multiple start extentions during cache load");
      if(sscanf(&line[2],"%x %x %x %x",&algolen,&cb1off,&cb2off,&cb3off)==4) {
        ...
        else PRINTSCF(L_SYS_TPS,"no memory for algo");
        }
      else PRINTSCF(L_SYS_TPS,"bad format in start extention");
      }
    else if(line[1]=='C') {
      if(algomem) {
        int off, len;
        ...
        PRINTSCF(L_SYS_TPS,"bad format in code extention");
        }
      else PRINTSCF(L_SYS_TPS,"unexpected code extention");
      }
    else PRINTSCF(L_SYS_TPS,"unknown extention during cache load");
    }
  else {
    if(GetHex(line,tmp,sizeof(tmp))) {
      ...
      else PRINTSCF(L_SYS_TPS,"CRC failed during cache load");
      }
    }
  return false;
}
    ...
    if(pid==0) { // PAT
      ...
      if(pat.CheckCRCAndParse()) {
        ...
        for(SI::Loop::Iterator it; pat.associationLoop.getNext(assoc,it);) {
          if(!assoc.isNITPid() && assoc.getServiceId()==AUSID) {
            pmtpid=assoc.getPid();
            PRINTSCF(L_SYS_TPSAU,"got PMT pid %04x for SID %04x",pmtpid,AUSID);
            cPid *pid=pids.First();
            if(pid && pid->filter) {
              pid->filter->Start(pmtpid,0x02,0xFF,0x00,false);
              pid->filter->Flush();
              }
            else PRINTSCF(L_GEN_DEBUG,"internal: no pid/filter in cTpsAuHook/pat");
            return;
            }
          }
        PRINTSCF(L_SYS_TPSAU,"no PMT pid found for SID %04x",AUSID);
        BailOut();
        }
      }
      ...
      if(pmt.CheckCRCAndParse() && pmt.getServiceId()==AUSID) {
        ...
        for(SI::Loop::Iterator it; pmt.streamLoop.getNext(stream,it); ) {
          if(stream.getStreamType()==0x05) {
            aupid=stream.getPid();
            PRINTSCF(L_SYS_TPSAU,"got AU pid %04x",aupid);
            cPid *pid=pids.First();
            if(pid && pid->filter) {
              pid->filter->Start(aupid,0x87,0xFF,0x00,false);
              pid->filter->Flush();
              }
            else PRINTSCF(L_GEN_DEBUG,"internal: no pid/filter in cTpsAuHook/pmt");
            return;
            }
          }
        PRINTSCF(L_SYS_TPSAU,"could not locate AU pid in PMT %04x data",pmtpid);
        BailOut();
        }
      }
    else if(pid==aupid) {
      if(cOpenTVModule::Id(data)==2) {
        ...
        if(mod) {
          ...
          if(r>0) {
            PRINTSCF(L_SYS_TPSAU,"received complete OpenTV module ID 2");
  ...
  switch(mode) {
    ...
    case 3: if(mem) {
              if(!DecryptAlgo3(key,data))
                PRINTSCF(L_SYS_TPS,"decrypt failed in algo 3");
              }
            else PRINTSCF(L_SYS_TPS,"no callbacks for algo 3 registered");
            break;
    default: PRINTSCF(L_SYS_TPS,"unknown TPS decryption algo %d",mode); break;
    }
}
...
bool cTPSDecrypt::RegisterAlgo3(const unsigned char *data, int cb1, int cb2, int cb3, int len)
{
  ...
  st20Inited=false;
  PRINTSCF(L_SYS_TPSAU,"registered callbacks for algo 3");
  return true;
}
...
bool cTPSDecrypt::Handle80008003(const unsigned char *src, int len, unsigned char *dest)
{
  ...
  if(cb1off && InitST20()) {
    ...
    if(err<0) {
      PRINTSCF(L_SYS_TPSAU,"ST20 processing failed in callback1 (%d)",err);
      return false;
      }
...
bool cTPSDecrypt::DecryptAlgo3(const unsigned char *key, unsigned char *data)
{
  ...
  if(cb2off && cb3off && InitST20()) {
    ...
    if(err<0) {
      PRINTSCF(L_SYS_TPS,"ST20 processing failed in callback2 (%d)",err);
      return false;
      }
    ...
    if(err<0) {
      PRINTSCF(L_SYS_TPS,"ST20 processing failed in callback3 (%d)",err);
      return false;
      }
...
int cTPS::Decrypt(int cardNum, int Source, int Transponder, unsigned char *data, int len)
{
  if(!sattime) {
    ...
    if(!sattime) {
      PRINTSCF(L_SYS_TPS,"failed to create time class");
      return -1;
      }
    }
  ...
  if((opmode&4) && data[0]==0xD2 && data[1]==0x01 && data[2]==0x01) {
    data+=3; len-=3; ret=3;
    if(data[0]==0x90) PRINTSCF(L_SYS_TPS,"TPS v1 is no longer supported");
  ...
  for(int i=0; i<len; i+=data[i+1]+2) {
    if(!k && (doTPS || doPre || doPost)) {
      PRINTSCF(L_SYS_TPS,"no TPS key available for current time of day");
      return -1;
      }
    switch(data[i]) {
      case 0xDF:
