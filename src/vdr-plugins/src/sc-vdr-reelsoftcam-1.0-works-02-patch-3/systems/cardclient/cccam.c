...
bool cCCcamCard::Connect(void)
{
  ...
  if(!ccam_fd) {
    PRINTSCF(L_CC_CCCAM,"%d: socket failed: %s",cardnum,strerror(errno));
    return false;
    }
  ...
  PRINTSCF(L_CC_CCCAM,"%d: using camd socket '%s'",cardnum,serv_addr_un.sun_path);
  if(connect(ccam_fd,(const sockaddr*)&serv_addr_un,sizeof(serv_addr_un))!=0) {
    PRINTSCF(L_CC_CCCAM,"%d: connect failed: %s",cardnum,strerror(errno));
    Disconnect();
    return false;
    }
  PRINTSCF(L_CC_CCCAM,"%d: opened camd socket",cardnum);
  return true;
}
...
void cCCcamCard::WriteCaPmt(void)
{
  ...
  if(capmt) {
    for(int retry=2; retry>0; retry--) {
      ...
      PRINTSCF(L_CC_CCCAM,"%d: write failed: %s",cardnum,strerror(errno));
      Disconnect();
      }
    }
}
...
bool cCardClientCCcam::Init(const char *config)
{
  ...
  PRINTSCF(L_CC_CORE,"%s: socket=%s",name,path);
...
bool cCardClientCCcam::Login(void)
{
  ...
  so.SetQuietLog(true);
  PRINTSCF(L_CC_CCCAM,"Bound to port %d, starting UDP listener",port);
  Start();
  return true;
}
...
bool cCardClientCCcam::ProcessECM(const cEcmInfo *ecm, const unsigned char *data, unsigned char *cw, int cardnum)
{
  ...
  if(ecm->ecm_pid!=c->Pid() || !c->Connected()) { // channel change
    ...
    if(!descr) {
      PRINTSCF(L_CC_CCCAM,"%d: no CA descriptor for caid %04x sid %d prov %04x",cardnum,ecm->caId,ecm->prgId,ecm->provId);
      return false;
      }
...
#define PID_POS 27
  ...
  if(!c->GetCw(cw,timeout)) {
    PRINTSCF(L_CC_CCCAM,"%d: getting CW timedout, re-writing CAPMT",cardnum);
    // somethings up, so we will send capmt again.
    c->WriteCaPmt();
    if(!c->GetCw(cw,timeout)) {
      PRINTSCF(L_CC_CCCAM,"%d: getting CW timedout again, failed ECM",cardnum);
      c->Disconnect();
      failedcw++;
      if(failedcw>=10) {
        // CCcam is having problems lets mark it for a restart....
        PRINTSCF(L_CC_CCCAM,"%d: marked CCcam for restart",cardnum);
  ...
  PRINTSCF(L_CC_CCCAM,"%d: got a CW",cardnum);
  failedcw=0;
  return true;
}
