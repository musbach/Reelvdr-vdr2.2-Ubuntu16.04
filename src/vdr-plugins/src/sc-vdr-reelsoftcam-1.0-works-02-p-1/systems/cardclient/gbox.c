...
bool cCardClientGbox::ProcessECM(const cEcmInfo *ecm, const unsigned char *data, unsigned char *cw, int cardnum)
{
  ...
  if(!descr) {
    PRINTSCF(L_CC_GBOX,"no CA descriptor for caid %04x sid %d prov %04x",caid,ecm->prgId,ecm->provId);
    return false;
    }
  ...
  if(sizeof(pmt)+n+8>sizeof(buff)) {
    PRINTSCF(L_CC_GBOX,"CA descriptor buffer overflow %d",sizeof(pmt)+n+8);
    return false;
    }
  ...
  if(so.SendTo("127.0.0.1",8004,buff,sizeof(pmt)+n+4)<0) {
    PRINTSCF(L_CC_GBOX,"failed to send PMT data. GBOX running?");
    return false;
    }

  if((n=GetMsg(0x8a,buff,sizeof(buff)))<0) {
    PRINTSCF(L_CC_GBOX,"failed to get ECM port. GBOX running?");
    return false;
    }
  int pidnum=-1;
  if(n>=2) {
  ...
  if(pidnum<0) {
    PRINTSCF(L_CC_ECM,"%s: unable to decode for CAID %04X/PID %04X",name,caid,pid);
    return false;
    }

  n=SCT_LEN(data);
  if(n>=256) {
    PRINTSCF(L_CC_ECM,"%s: ECM section too long %d > 255",name,n);
    return false;
    }
  buff[0]=0x88;
  buff[1]=(pid>>8)&0x1F;
  ...
  if(so.SendTo("127.0.0.1",8005+pidnum,buff,n)<0) {
    PRINTSCF(L_CC_GBOX,"failed to send ECM data. GBOX running?");
    return false;
    }

  if(GetMsg(0x89,buff,sizeof(buff))<0) {
    PRINTSCF(L_CC_GBOX,"failed to get CW. GBOX running?");
    return false;
    }
  if(n<17) {
    PRINTSCF(L_CC_GBOX,"bad CW answer from GBOX");
    return false;
    }
  memcpy(cw,&buff[1],16);
  return true;
