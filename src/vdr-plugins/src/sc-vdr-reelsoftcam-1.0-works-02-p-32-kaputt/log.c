...
struct LogConfig logcfg = {1,0,0,0,0,0,"/var/log/vdr-sc"};
//struct LogConfig logcfg = {1,0,0,0,0,0,0};
//struct LogConfig logcfg = {1,0,0,0,0,0,"/dev/tty"};
//struct LogConfig logcfg = {1,0,0,0,0,0,"/dev/stout"};
...
//static bool logfileShutup=true, logfileReopen=false;
...
void cLogging::PrivateLogPrint(const struct LogHeader *lh, const char *txt)
{
  if(logcfg.logFile) {
    ...
    if(logfileReopen) {
      ...
      if(logfile) {
        PRINTSCF(L_GEN_DEBUG,"logfile closed, reopen as '%s'",logcfg.logFilename);
        fclose(logfile);
        logfile=0;
        }
      }
    if(!logfile && !logfileShutup) {
      ...
      if(logfile) {
        ...
        if(logfileSize<0) {
          logfileSize=0;
          PRINTSCF(L_GEN_ERROR,"cannot determine size of logfile '%s', assuming zero",logcfg.logFilename);
          }
        PRINTSCF(L_GEN_DEBUG,"logfile '%s' opened",logcfg.logFilename);
        }
      else {
        logfileShutup=true;
        PRINTSCF(L_GEN_ERROR,"failed to open logfile '%s': %s",logcfg.logFilename,strerror(errno));
        }
      }
    if(logfile) {
      ...
      if(logcfg.maxFilesize>0 && logfileSize>((long long)logcfg.maxFilesize*1024)) {
        ...
        if(rename(logcfg.logFilename,name)) {
          logfileShutup=true;
          PRINTSCF(L_GEN_ERROR,"failed to rotate logfile: %s",strerror(errno));
          PRINTSCF(L_GEN_ERROR,"logging to file disabled!");
          }
        free(name);
        }
      }
