...
void cSmartCardSlot::Action(void)
{
  while(Running()) {
    if(card) {
      ...
      if(!DeviceIsInserted()) {
        card->Unlock();
        PRINTSCF(L_CORE_SC,"%d: card removed (usecount=%d)",slotnum,usecount);
        SetCard(0);
        }
      else if(needsReset) {
        PRINTSCF(L_CORE_SC,"%d: card reset requested",slotnum);
        int mode=DeviceCurrentMode()&SM_MASK;
        if(!DeviceSetMode(mode,ISO_BAUD)
            || !CardReset()
            || !card->Setup(this,mode,&atr,sb)) {
          card->Unlock();
          PRINTSCF(L_CORE_SC,"%d: card re-init failed",slotnum);
          SetCard(0); dead=true;
          }
        }
      if(card) card->Unlock();
      ...
      if(!dead) {
        PRINTSCF(L_CORE_SC,"%d: new card inserted",slotnum);
          ...
          if(DeviceSetMode(mode,ISO_BAUD)) {
            if((singlereset && resetdone) || (resetdone=CardReset())) {
              for(cSmartCardLink *scl=smartcards.First(); scl; scl=smartcards.Next(scl)) {
                ...
                PRINTSCF(L_CORE_SC,"%d: checking for %s card",slotnum,scl->Name());
              ...
              PRINTSCF(L_CORE_SC,"%d: no card handler found",slotnum);
              }
            else PRINTSCF(L_CORE_SC,"%d: reset/atr error",slotnum);
            }
          else PRINTSCF(L_CORE_SC,"%d: failed to set serial mode %s",slotnum,serModes[mode]);
          }
        dead=true;
        PRINTSCF(L_CORE_SC,"%d: can't initialise new card, ignoring port until card reinserted",slotnum);
        }
      }
    else {
      if(dead) PRINTSCF(L_CORE_SC,"%d: card removed, port reactivated",slotnum);
      dead=false;
      }
done:
    if(firstRun) {
...
{
  PRINTSCF(L_CORE_SC,"%d: reseting card (sermode %s)",slotnum,serModes[DeviceCurrentMode()]);
  DeviceToggleReset();
...
int cSmartCardSlot::Procedure(unsigned char ins, int restLen)
{
  ...
  do {
    ...
    PRINTSCF(L_GEN_DEBUG,"cSmartCardSlot::Procedure 1. buff:%d r:%d",buff,r);
    if((buff&0xF0)==0x60 || (buff&0xF0)==0x90) { // SW1/SW2
      sb[0]=buff;
//    LBPUT("in cSmartCardSlot::Procedure: buff=%02x (ins=%02x)",buff,ins);
...
bool cSmartCardSlot::Test(bool res)
{
 if(!res) {
   TriggerReset();
   PRINTSCF(L_CORE_SC,"%d: reset triggered",slotnum);
   }
 return res;
}
...
{
  LDUMP(L_CORE_SC,cmd,CMD_LEN,"cSmartCardSlot::IsoRead %d: -> INS:",slotnum);
  ...
  while(1) {
    ...
    PRINTSCF(L_GEN_DEBUG,"cSmartCardSlot::IsoRead 1. r:%d len:%d",r,len);
...
bool cSmartCardSlot::ParseAtr(void)
{
  ...
  if(rawatr[0]==0x03) {
    PRINTSCF(L_CORE_SC,"%d: indirect convention detected",slotnum);
    Invert(atr.atr,atr.atrLen);
    atr.convention=SM_INDIRECT;
    }
  else if(rawatr[0]==0x3B) {
    PRINTSCF(L_CORE_SC,"%d: direct convention detected",slotnum);
    atr.convention=SM_DIRECT;
    }
  else if(rawatr[0]==0x3F) {
    PRINTSCF(L_CORE_SC,"%d: inverted indirect convention detected",slotnum);
    atr.convention=SM_INDIRECT_INV;
    }
  else {
    PRINTSCF(L_CORE_SC,"%d: byte mode not supported 0x%02x",slotnum,rawatr[0]);
    return false;
    }
  ...
  if(atr.T!=0 && len+1<=atr.atrLen) {
    ...
    if(cs==0) PRINTSCF(L_CORE_SC,"%d: atr checksum ok",slotnum);
    else if(cs==rawatr[0]) PRINTSCF(L_CORE_SC,"iso: %d: atr checksum is TS (not ISO compliant)",slotnum);
    else {
      PRINTSCF(L_CORE_SC,"%d: atr checksum FAILED (cs:%02x)",slotnum,cs);
      return false;
      }
    }
  else PRINTSCF(L_CORE_SC,"%d: atr checksum not given/not required",slotnum);

  if(atr.atrLen>len) PRINTSCF(L_CORE_SC,"%d: long atr read=%d len=%d",slotnum,atr.atrLen,len);
  atr.wwt=960*atr.WI*atr.F/(clock/1000);
  atr.bwt=(int)(960*(float)(1<<atr.BWI)*372/(clock/1000));
  PRINTSCF(L_CORE_SC,"%d: indicated wwt(T0)=%d ms, bwt(T1)=%d ms (at %.4f MHz)",slotnum,atr.wwt,atr.bwt,(float)clock/1000000);
  return true;
}
...
{
  PRINTSCF(L_CORE_DYN,"registering cardslot type %s",scl->name);
  scl->next=first;
  first=scl;
}
  ...
  if(sscanf(line,"%31[^:]:%n",type,&num)==1) {
    for(cSmartCardSlotLink *scs=first; scs; scs=scs->next) {
      if(!strcasecmp(type,scs->name)) {
        ...
        else PRINTSCF(L_GEN_ERROR,"failed to create cardslot");
        return false;
        }
      }
    PRINTSCF(L_GEN_ERROR,"unknown cardslot type '%s'",type);
    }
  return false;
}
...
bool cSmartCardSlotSerial::DeviceOpen(const char *cfg)
{
  ...
  if(sscanf(cfg,"%255[^:]:%d:%d:%d",devName,&invCD,&invRST,&clock)>=3) {
    ...
    PRINTSCF(L_CORE_SERIAL,"%s: open serial port",devName);
    fd=open(devName,O_RDWR|O_NONBLOCK|O_NOCTTY);
    if(fd>=0) {
      SetDtrRts();
      PRINTSCF(L_CORE_SERIAL,"%s: init done",devName);
      PRINTSCF(L_CORE_LOAD,"cardslot: added serial port %s as port %d (%s CD, %s RESET, CLOCK %d)",
                devName,slotnum,invCD?"inverse":"normal",invRST?"inverse":"normal",clock);
      return true;
      }
    else PRINTSCF(L_GEN_ERROR,"%s: open failed: %s",devName,strerror(errno));
    }
  else PRINTSCF(L_GEN_ERROR,"bad parameter for cardslot type 'serial'");
  return false;
}

void cSmartCardSlotSerial::SetDtrRts(void)
{
  PRINTSCF(L_CORE_SERIAL,"%s: set DTR/RTS",devName);
...
{
  if(fd>=0) {
    PRINTSCF(L_CORE_SERIAL,"%s: shutting down",devName);
    Flush();
    unsigned int modembits=0;
    CHECK(ioctl(fd,TIOCMSET,&modembits));
    close(fd); fd=-1;
    PRINTSCF(L_CORE_SERIAL,"%s: shutdown done",devName);
    }
}
...
speed_t cSmartCardSlotSerial::FindBaud(int baud)
{
  ...
  for(int i=0; i<(int)(sizeof(BaudRateTab)/sizeof(struct BaudRates)); i++) {
    ...
    if(abs(d)<=300) {
      PRINTSCF(L_CORE_SERIAL,"%s: requested baudrate %d -> %d (%+.2f%%)",devName,baud,b,(float)d/100.0);
      return BaudRateTab[i].apival;
      }
    }
  PRINTSCF(L_CORE_SERIAL,"%s: requested baudrate %d -> custom",devName,baud);
  return B0;
}
  ...
  if(fd>=0) {
    ...
    if(ioctl(fd,TIOCGSERIAL,&s)<0) {
      if(custom || custombaud) {
        PRINTSCF(L_GEN_ERROR,"%s: get serial failed: %s",devName,strerror(errno));
        return false;
        }
      PRINTSCF(L_CORE_SERIAL,"%s: get serial failed: %s",devName,strerror(errno));
      PRINTSCF(L_CORE_SERIAL,"%s: custombaud not used, try to continue...",devName);
      }
    else if(!custom && ((s.flags&ASYNC_SPD_MASK)==ASYNC_SPD_CUST || s.custom_divisor!=0)) {
      s.custom_divisor=0;
      s.flags &= ~ASYNC_SPD_MASK;
      if(ioctl(fd,TIOCSSERIAL,&s)<0) {
        PRINTSCF(L_GEN_ERROR,"%s: set serial failed: %s",devName,strerror(errno));
        return false;
        }
      custombaud=false;
      }
    if(!tcsetattr(fd,TCSANOW,&tio)) {
      if(custom) {
        ...
        PRINTSCF(L_CORE_SERIAL,"%s: custom: baud_base=%d baud=%d devisor=%d -> effective baudrate %d (%+.2f%% off)",devName,s.baud_base,baud,s.custom_divisor,s.baud_base/s.custom_divisor,(float)(s.baud_base/s.custom_divisor-baud)/(float)baud);
        if(ioctl(fd,TIOCSSERIAL,&s)<0) {
          PRINTSCF(L_GEN_ERROR,"%s: set serial failed: %s",devName,strerror(errno));
          return false;
          }
    ...
    else PRINTSCF(L_GEN_ERROR,"%s: tcsetattr failed: %s",devName,strerror(errno));
    }
  return false;
}
...
{
  PRINTSCF(L_CORE_SERIAL,"%s: read len=%d timeout=%d:%d",devName,len,timeout,initialTimeout);
  ...
  while(n<len) {
    ...
    if(r<=0) {
      if(r==0) PRINTSCF(L_CORE_SERIAL,"%s: read bogus eof",devName);
      if(errno==EAGAIN) {
        struct pollfd u;
        u.fd=fd; u.events=POLLIN;
        r=poll(&u,1,to);
        if(r<0) {
          PRINTSCF(L_GEN_ERROR,"%s: read poll failed: %s",devName,strerror(errno));
          return -1;
          }
        else if(r==0) { // timeout
          PRINTSCF(L_CORE_SERIAL,"%s: read timeout (%d ms)",devName,to);
        ...
        PRINTSCF(L_GEN_ERROR,"%s: read failed: %s",devName,strerror(errno));
        return -1;
        }
      }
    n+=r; to=timeout;
...
{
  PRINTSCF(L_CORE_SERIAL,"%s: write len=%d delay=%d",devName,len,delay);
  ...
  while(n<len) {
    ...
    do {
      r=poll(&u,1,500);
      if(r<0) {
        PRINTSCF(L_GEN_ERROR,"%s: write poll failed: %s",devName,strerror(errno));
        return -1;
        }
      else if(r==0) { // timeout
        PRINTSCF(L_CORE_SERIAL,"%s: write timeout",devName);
        return -2;
        }
      } while(r<1);
    r=write(fd,mem+n,delay>0?1:len-n);
    if(r<0 && errno!=EAGAIN) {
      PRINTSCF(L_GEN_ERROR,"%s: write failed: %s",devName,strerror(errno));
      return -1;
      }
    if(r>0) n+=r;
...
void cSmartCardSlotSerial::DeviceToggleReset(void)
{
  ...
  if(mask&TIOCM_RTS) { mask&=~TIOCM_RTS; PRINTSCF(L_CORE_SERIAL,"%s: toggle RTS, now off",devName); }
  else               { mask|= TIOCM_RTS; PRINTSCF(L_CORE_SERIAL,"%s: toggle RTS, now on",devName); }
...
bool cSmartCardSlotSerial::DevicePTS(void)
{
  ...
  PRINTSCF(L_CORE_SC,"%d: PTS cycle: calculated baudrate %d",slotnum,baud);

  if(atr.Tspec<0) {
    PRINTSCF(L_CORE_SC,"%d: negotiable mode",slotnum);
#ifdef NO_PTS_PROTO
    PRINTSCF(L_CORE_SC,"%d: PTS disabled at compile time!!!",slotnum);
    return true;
#else
    ...
    if(DeviceWrite(req,4)!=4) {
      PRINTSCF(L_CORE_SC,"%d: PTS request, write failed",slotnum);
      return false;
      }
    if(DeviceRead(conf,4,100,100)!=4) {
      PRINTSCF(L_CORE_SC,"%d: PTS request, echo readback failed",slotnum);
      return false;
      }
    int n=DeviceRead(conf,-16,200,1000);
    if(n>0) LDUMP(L_CORE_SC,conf,n,"%d: PTS confirm:",slotnum);
    if(n<1 || conf[0]!=0xFF || XorSum(conf,n)!=0) {
      PRINTSCF(L_CORE_SC,"%d: PTS confirm, bad format",slotnum);
      return false;
      }
    if(n<4 || conf[1]!=req[1] || conf[2]!=req[2]) {
      PRINTSCF(L_CORE_SC,"%d: PTS not confirmed",slotnum);
      return false;
      }
#endif // NO_PTS_PROTO
    }
  else
    PRINTSCF(L_CORE_SC,"%d: specific mode Tspec=%d",slotnum,atr.Tspec);

  int mode=DeviceCurrentMode();
  if(atr.N==255) mode|=SM_1SB;
  if(!DeviceSetMode(mode,baud)) {
    PRINTSCF(L_CORE_SC,"%d: setting baudrate %d failed",slotnum,baud);
    return false;
    }
  return true;
}
...
bool cSmartCardSlotSerial::DeviceIsInserted(void)
{
  ...
  PRINTSCF(L_CORE_SERIAL,"%s: CAR is %sactive (lines:%s%s%s%s%s%s%s%s%s)",
...
bool cSmartCardSlotSRPlus::DeviceOpen(const char *cfg)
{
  if(sscanf(cfg,"%255[^:]:%d",devName,&clock)>=1) {
    ...
    if(fd>=0) {
      SetDtrRts();
      PRINTSCF(L_CORE_SERIAL,"%s: init done",devName);
      PRINTSCF(L_CORE_LOAD,"cardslot: added smartreader+ port %s as port %d (CLOCK %d)",
                devName,slotnum,clock);
      return true;
      }
    else PRINTSCF(L_GEN_ERROR,"%s: open failed: %s",devName,strerror(errno));
    }
  else PRINTSCF(L_GEN_ERROR,"bad parameter for cardslot type 'srplus'");
  return false;
}
...
bool cSmartCardSlotSRPlus::DevicePTS(void)
{
  ...
  PRINTSCF(L_CORE_SERIAL,"%s: SmartReader+ mode %.4f MHz",devName,cl/1000000.0);
  return true;
}
...
bool cSmartCardSlotSRPlus::SRConfig(int F, float D, int fs, int N, int T, int inv)
{
  ...
  PRINTSCF(L_CORE_SERIAL,"%s: SR+ options F=%d D=%f fs=%d N=%d T=%d inv=%d",devName,F,D,fs,N,T,inv);
  // Set SmartReader+ in CMD mode.
  struct termios term;
  if(tcgetattr(fd,&term)==-1) {
    PRINTSCF(L_GEN_ERROR,"%s: tcgetattr failed: %s",devName,strerror(errno));
    return false;
    }
  term.c_cflag&=~CSIZE;
  term.c_cflag|=CS5;
  if(tcsetattr(fd,TCSADRAIN,&term)==-1) {
    PRINTSCF(L_GEN_ERROR,"%s: tcsetattr failed: %s",devName,strerror(errno));
    return false;
    }
  ...
  if(tcsendbreak(fd,0)==-1) {
    PRINTSCF(L_GEN_ERROR,"%s: tcsendbreak failed: %s",devName,strerror(errno));
    return false;
    }
...
bool cSmartCardSlotCCID::DeviceOpen(const char *cfg)
{
  if(!cfg || sscanf(cfg,"%255[^:]",devName)==1) {
    ...
    PRINTSCF(L_CORE_SC,"%d: establish context: %lx:%s",slotnum,rv,pcsc_stringify_error(rv));
    if(rv==SCARD_S_SUCCESS) {
      ...
      PRINTSCF(L_CORE_SC,"%d: connect '%s': %lx:%s",slotnum,devName,rv,pcsc_stringify_error(rv));
      if(rv==SCARD_S_SUCCESS) {
        needsReOpen=false;
        return true;
        }
  ...
  else PRINTSCF(L_GEN_ERROR,"bad parameter for cardslot type 'ccid'");
  return false;
}
...
bool cSmartCardSlotCCID::Reset(void)
{
  ...
  if(rv!=SCARD_S_SUCCESS) {
    PRINTSCF(L_CORE_SC,"%d: reset failed: %lx:%sn",slotnum,rv,pcsc_stringify_error(rv));
    return false;
    }
...
int cSmartCardSlotCCID::Transmit(const unsigned char *request, int requestlen, unsigned char *answer, int answerlen)
{
  ...
  switch(dwProtocol) {
    ...
    case SCARD_PROTOCOL_T1:
    	...
    	break;
    default:
  	PRINTSCF(L_CORE_SC,"%d: protocol failure: %ld",slotnum,dwProtocol);
        return 0;
    }
  ...
  if(rv!=SCARD_S_SUCCESS) {
     PRINTSCF(L_CORE_SC, "%d: transmit failed: %lx:%s\n",slotnum,rv,pcsc_stringify_error(rv));
     return -1;
     }
  return answlen;
}
...
{
  PRINTSCF(L_CORE_SERIAL,"%s: open serial port (emulator)",devName);
  PRINTSCF(L_CORE_SERIAL,"%s: init done",devName);
  buffCount=0; cmdStart=true; rts=false; flag=false;
  remTime=time(0); dsr=true;
  PRINTSCF(L_CORE_LOAD,"cardslot: added emulated port %s as port %d",devName,slotnum);
  return true;
}
...
{
  PRINTSCF(L_CORE_SERIAL,"%s: read len=%d timeout=%d",devName,len,timeout);
  if(len<0) {
    len=-len;
...
void cSmartCardSlotEmuGeneric::DeviceToggleReset(void)
{
  ...
  if(!rts) {
    EmuPushAtr();
    PRINTSCF(L_CORE_SERIAL,"%s: toggle RTS, now off",devName);
    }
  else { PRINTSCF(L_CORE_SERIAL,"%s: toggle RTS, now on",devName); }
}
...
bool cSmartCardSlotEmuIrdeto::DeviceOpen(const char *cfg)
{
  ...
  if(sscanf(cfg,"%31[^:]",acs)>=0) {
    if(!strcasecmp(acs,"acs384")) acs384=true;
      ...
      PRINTSCF(L_GEN_ERROR,"unknown ACS type");
      return false;
      }
    return cSmartCardSlotEmuGeneric::DeviceOpen(0);
    }
  else PRINTSCF(L_GEN_ERROR,"bad parameter for cardslot type '%s'",devName);
  return false;
}
  ...
  if(rmem[0]==0x21 && rmem[1]==0xc1) { // IFS
    ...
    for(const struct Resp *r=&resp[0]; 1 ;r++) {
      ...
      if(cmd==r->cmd && idata==r->idata && (!isdt || dt_count[idata&0xF]==r->cc || r->cc<0)) {
        ...
        if(isdt) {
          dt_count[idata&0xF]++;
          PRINTSCF(L_CORE_SERIAL,"%s: dt%02x count now %d",devName,idata&0xf,dt_count[idata&0xF]);
          }
        break;
        }
      }
...
int cSmartCard::SerRead(unsigned char *data, int len, int to)
{
  ...
  PRINTSCF(L_CORE_SC,"%d: SerRead() not allowed for ISO compliant cards (T=%d)",slotnum,atr->T);
  return -1;
}
...
int cSmartCard::SerWrite(const unsigned char *data, int len)
{
  ...
  PRINTSCF(L_CORE_SC,"%d: SerWrite() not allowed for ISO compliant cards (T=%d)",slotnum,atr->T);
  return -1;
}
...
bool cSmartCard::IsoRead(const unsigned char *cmd, unsigned char *data)
{
  ...
  PRINTSCF(L_CORE_SC,"%d: can't IsoRead() from incompatible card (T=%d)",slotnum,atr->T);
  return false;
}
...
bool cSmartCard::IsoWrite(const unsigned char *cmd, const unsigned char *data)
{
  ...
  PRINTSCF(L_CORE_SC,"%d: can't IsoWrite() to incompatible card (T=%d)",slotnum,atr->T);
  return false;
}
...
bool cSmartCard::Status(void)
{
  ...
  while(m->sb[0]!=0xFF) {
    if(sb[0]==m->sb[0] && (m->sb[1]==0xFF || sb[1]==m->sb[1])) {
      if(!m->retval) PRINTSCF(L_CORE_SC,"%d: %s (status: %02x %02x)",slotnum,m->message,sb[0],sb[1]);
      return m->retval;
      }
    m++;
    }
  PRINTSCF(L_CORE_SC,"%d: unknown (status: %02x %02x)",slotnum,sb[0],sb[1]);
  return false;
}
...
int cSmartCard::CheckSctLen(const unsigned char *data, int off)
{
  int l=SCT_LEN(data);
  if(l+off > MAX_LEN) {
    PRINTSCF(L_CORE_SC,"section too long %d > %d",l,MAX_LEN-off);
    l=-1;
    }
  return l;
}
...
{
  PRINTSCF(L_CORE_DYN,"registering %s smartcard (id %x)",scl->name,scl->id);
  scl->next=first;
  first=scl;
}
