...
{
//  PRINTSCF(L_GEN_DEBUG,"cCiFrame::Get rb=%d ",rb);
  if(rb) {
    int c;
    ...
    if(data) {
      if(c>LEN_OFF) {
        ...
        if(c>=s+LEN_OFF) {
          l=glen=s;
      LDUMP(L_GEN_DEBUG,data,c,"cCiFrame::Get l=%d s=%d glen=%d c=%d avail=%d data=%d -",l,s,glen,c,rb->Available(),data);
//	if(s==49) return data; //HACK
          return data+LEN_OFF;
          }
        }
...
{
  cMutexLock lock(&ciMutex);
//  PRINTSCF(L_GEN_DEBUG,"cScCiAdapter::Read cam:%d rb:%d Buffer:%d MaxLength:%d",cam,rb,Buffer,MaxLength);
  if(cam && rb && Buffer && MaxLength>0) {
    int s;
    unsigned char *data=frame.Get(s);
//    PRINTSCF(L_GEN_DEBUG,"cScCiAdapter::Read s:%d data:%d",s,data);
    if(data) {
      if(s<=MaxLength) {
//	if(s==49) data=data-2;
//	if(s==49) s=158;
/*	if(s==49) {
	  PRINTSCF(L_GEN_DEBUG,"cScCiAdapter::Read return 0  s=%d",s);
	  return 0;
	}*/
	memcpy(Buffer,data,s);
      }else{
	PRINTSCF(L_GEN_DEBUG,"cScCiAdapter::Read else sc-ci %d rb frame size exceeded %d",cardIndex,s);
      }
      frame.Del();
      if(Buffer[2]!=0x80 || LOG(L_CORE_CIFULL)) {
        LDUMP(L_CORE_CI,Buffer,s,"%d.%d cScCiAdapter::Read s=%d <-",cardIndex,Buffer[0],s);
//	PRINTSCF(L_GEN_DEBUG,"satellit: sc-ci cardindex %d rb frame size %d",cardIndex,s);
	LDUMP(L_CORE_CI,data,s,"%d.%d <-",cardIndex,data[0]);
//	PRINTSCF(L_GEN_DEBUG,"satellit: sc-ci cardindex %d rb frame size %d",cardIndex,s);
        readTimer.Set();
        }
      return s;
...
{
  cMutexLock lock(&ciMutex);
  PRINTSCF(L_GEN_DEBUG,"cScCiAdapter::Write 1. buff:%d len:%d",buff,len);
  if(cam && buff && len>=5) {
    struct TPDU *tpdu=(struct TPDU *)buff;
    int slot=tpdu->slot;
    cCiFrame *slotframe=slots[slot]->Frame();
    if(buff[2]!=0xA0 || buff[3]>0x01 || LOG(L_CORE_CIFULL))
      LDUMP(L_GEN_DEBUG,buff,len,"%d.%d cScCiAdapter::Write 2. len=%d ->",len,cardIndex,slot);
    if(slots[slot]) {
      switch(tpdu->tag) {
        case 0x81: // T_RCV
          {
  ...
  if(ringBuffer) {
    ...
    while(Running()) {
      if(firstRead || Poller.Poll(100)) {
        firstRead=false;
//     PRINTSCF(L_GEN_DEBUG,"cDeCsaTSBuffer::Action cam.c 3025 firstRead=%s",firstRead ? "true" : "false");
