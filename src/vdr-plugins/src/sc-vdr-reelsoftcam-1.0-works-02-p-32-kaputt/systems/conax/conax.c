...
bool cSystemConax::ProcessECM(const cEcmInfo *ecm, unsigned char *source)
{
  ...
  if(!(pk=keys.FindKey('C',keyid,'E',-1))) {
    if(doLog) PRINTSCF(L_SYS_KEY,"missing %02X E key",keyid);
    return false;
    }
  pk->Get(exp);
  if(!(pk=keys.FindKey('C',keyid,'M',-1))) {
    if(doLog) PRINTSCF(L_SYS_KEY,"missing %02X M key",keyid);
    return false;
    }
  pk->Get(mod);
  ...
  for(int i=length ; i>0 ;) {
    ...
    if(rsa.RSA(source+index,source+index,64,exp,mod,false)<=0) {
      if(doLog) PRINTSCF(L_SYS_CRYPTO,"RSA failed");
      return false;
      }
    if(nano==0x63) {
  ...
  if(memcmp(hash,source+2,3) || memcmp(source+5,source+length-5,3)) {
    if(doLog) PRINTSCF(L_SYS_ECM,"signature check failed");
    return false;
    }
