...
bool cCCcamCrypt::DcwChecksum(const unsigned char *data)
{
  ...
  if(((data[0]+data[1]+data[2])&0xff)!=data[3] ||
    ...
    res=false;
    PRINTSCF(L_CC_CCCAM2EX,"warning: even CW checksum failed");
    }
  if(((data[8]+data[9]+data[10])&0xff)!=data[11] ||
     ((data[12]+data[13]+data[14])&0xff)!=data[15]) {
    res=false;
    PRINTSCF(L_CC_CCCAM2EX,"warning: odd CW checksum failed");
    }
  return res;
}
...
void cCmd0cCrypt::Reset(void)
{
  ...
  PRINTSCF(L_CC_CCCAM2EX,"cmd 0c crypt mode now %d",mode);
}
...
int cEcmShares::FindStatus(const cEcmInfo *ecm, int id)
{
  ...
  if(e) {
    PRINTSCF(L_CC_CCCAM2SH,"shareid %08x for %04x/%x/%x status %d",e->shareid,ecm->ecm_pid,ecm->source,ecm->transponder,e->status);
    return e->status;
    }
  return 0;
}
...
void cEcmShares::AddStatus(const cEcmInfo *ecm, int id, int status)
{
  ...
  PRINTSCF(L_CC_CCCAM2SH,"%s shareid %08x for %04x/%x/%x status %d",t,id,ecm->ecm_pid,ecm->source,ecm->transponder,status);
  e->status=status;
}
...
void cShare::Init(void)
{
  ...
  if(!CheckNull(ua,sizeof(ua)))
    switch(caid>>8) {
      ...
      // fall through
      default:   PRINTSCF(L_CC_CCCAM2,"share %08x: can't handle unique updates for CAID %04x",shareid,caid);
                 break;
      }
  SetCache();
}
...
void cShare::SetCache(void)
{
  if(emmready) {
    ...
    if(!cache) {
      emmready=false;
      PRINTSCF(L_CC_CCCAM2,"share %08x: failed to alloc EMM cache",shareid);
      }
    }
  else {
...
bool cShare::CheckAddProv(const unsigned char *pr, const unsigned char *sa)
{
  ...
  if(!CheckNull(s,sizeof(s)))
    switch(caid>>8) {
      ...
      default:   PRINTSCF(L_CC_CCCAM2,"share %08x: can't handle shared updates for CAID %04x",shareid,caid);
                 break;
    }
...
static cCardClientLinkReg<cCardClientCCcam2> __ncd_2("C");
  ...
  if(plen<=length) {
    ...
    switch(hdr->cmd) {
      ...
      case 1:
        ...
        PRINTSCF(L_CC_CCCAM2,"got CW, current shareid %08x (pending %d, EMM %d, maxecmcount=%d)",shareid,pendingDCW,pendingEMM,maxecmcount);
        ...
        else PRINTSCF(L_CC_CCCAM2,"pending DCW, skipping bad CW");
        decr.Decrypt(tempcw,tempcw,16);
        if(maxecmcount>0 && --maxecmcount<2) {
          PRINTSCF(L_CC_CCCAM2,"disconnecting due to key limit...");
          Logout();
          }
        break;
        }
      case 2:
        ...
        PRINTSCF(L_CC_CCCAM2EX,"EMM ack (pending %d)",pendingEMM);
        break;
      case 4:
        {
        ...
        for(cShare *s=shares.First(); s;) {
          ...
          if(s->ShareID()==shareid) {
            PRINTSCF(L_CC_CCCAM2SH,"REMOVE share %08x caid: %04x (count %d)",s->ShareID(),s->CaID(),shares.Count());
            int caid=s->CaID();
        ...
        if(check && emmProcessing) {
          ...
          if(!emm) {
            emmProcessing=false;
            PRINTSCF(L_CC_CCCAM2,"disabled EMM processing");
            }
          }
        shares.Unlock();
        break;
      ...
      case 5:
        {
        PRINTSCF(L_CC_CCCAM2,"got cmd 05 (payload length=%d)",plen);
        ...
        if(plen==0) mode=CMD05_MODE_LEN0;
          ...
          PRINTSCF(L_CC_CCCAM2EX,"cmd 05 mode %d",mode);
          switch(mode) {
        ...
        PRINTSCF(L_CC_CCCAM2EX,"sending cmd 05 response (len=%d)",l);
        SETCMDLEN(&gen->header,l);
        if(!CryptSend((unsigned char *)gen,l))
          PRINTSCF(L_CC_CCCAM2,"failed to send cmd 05 response");
        break;
        }
      case 6:
        PRINTSCF(L_CC_CCCAM2,"server PONG");
        break;
      case 7:
        {
        ...
        if(s->EmmReady() && !emmProcessing && emmAllowed) {
          emmProcessing=true;
          PRINTSCF(L_CC_CCCAM2,"enabled EMM processing");
          }
        shares.Unlock();
        break;
        }
      case 8:
        ...
        if(plen==0x48) {
          ...
          cmd05off=plen;
          PRINTSCF(L_CC_CCCAM2EX,"cmd 05 offset now %d",cmd05off);
          }
          ...
          PRINTSCF(L_CC_CCCAM2EX,"cmd 05 mode now RC4");
          ...
          PRINTSCF(L_CC_CCCAM2EX,"cmd 05 mode now AES (offset=%d)",cmd05off);
          LDUMP(L_CC_CCCAM2EX,((struct GenericCmd *)hdr)->payload,plen,"cmd 05 key payload");
          LDUMP(L_CC_CCCAM2EX,cmd05aes,sizeof(cmd05aes),"cmd 05 aes key");
          }
        else if(plen==0x21) {
          ...
          cmd05mode=CMD05_MODE_RC4;
          PRINTSCF(L_CC_CCCAM2EX,"cmd 05 mode now CC (offset=%d)",cmd05off);
          LDUMP(L_CC_CCCAM2EX,((struct GenericCmd *)hdr)->payload,plen,"cmd 05 key payload");
          }
        else if(plen==0x22) {
          cmd05mode=CMD05_MODE_PLAIN;
          PRINTSCF(L_CC_CCCAM2EX,"cmd 05 mode now PLAIN");
          }
        else if(plen==0x23) {
          cmd05mode=CMD05_MODE_UNKNOWN;
          PRINTSCF(L_CC_CCCAM2EX,"cmd 05 mode now UNKNOWN");
          }
        else if(plen==0x2c) {
          memcpy(cmd05aes,((struct GenericCmd *)hdr)->payload+strlen(password),sizeof(cmd05aes));
          cmd05mode=CMD05_MODE_AES;
          PRINTSCF(L_CC_CCCAM2EX,"cmd 05 mode now AES (offset=password=%d)",strlen(password));
          LDUMP(L_CC_CCCAM2EX,((struct GenericCmd *)hdr)->payload,plen,"cmd 05 key payload");
          LDUMP(L_CC_CCCAM2EX,cmd05aes,sizeof(cmd05aes),"cmd 05 aes key");
          }
        else if(plen==0x2d) {
          ...
          cmd05mode=CMD05_MODE_AES;
          PRINTSCF(L_CC_CCCAM2EX,"cmd 05 mode now AES (offset=username=%d)",strlen(username));
          LDUMP(L_CC_CCCAM2EX,((struct GenericCmd *)hdr)->payload,plen,"cmd 05 key payload");
          LDUMP(L_CC_CCCAM2EX,cmd05aes,sizeof(cmd05aes),"cmd 05 aes key");
          }
        else {
          PRINTSCF(L_CC_CCCAM2,"unknown format of serverinfo");
          }
        break;
        }
      case 0xb:
        {
        PRINTSCF(L_CC_CCCAM2,"got cmd 0b (payload length=%d)",plen);
        ...
        PRINTSCF(L_CC_CCCAM2EX,"sending cmd 0b response");
        SETCMDLEN(&gen->header,sizeof(resp));
        if(!CryptSend((unsigned char *)gen,sizeof(resp)))
          PRINTSCF(L_CC_CCCAM2,"failed to send cmd 0b response");
        break;
        }
      case 0xc:
        {
        PRINTSCF(L_CC_CCCAM2,"got cmd 0c (payload length=%d)",plen);
      ...
      case 0xd:
        {
        PRINTSCF(L_CC_CCCAM2,"got cmd 0d (payload length=%d)",plen);
        unsigned char *out=AUTOMEM(plen);
        cmd0Ccrypt.Decrypt(((struct GenericCmd *)hdr)->payload,plen,out);
        cmd0Ccrypt.SetKey(out,plen);
        break;
      ...
      case 0xfe:
        if(pendingDCW>0) pendingDCW--;
        if(hdr->cmd==0xfe) PRINTSCF(L_CC_CCCAM2,"share not found on server (%d pending)",pendingDCW);
        else PRINTSCF(L_CC_CCCAM2,"server can't decode this ecm, (%d pending)",pendingDCW);
        cwmutex.Lock();
        newcw=false;
        cwwait.Broadcast();
        cwmutex.Unlock();
    ...
    PRINTSCF(L_CC_CCCAM2,"cmdlen mismatch: cmdlen=%d length=%d",plen,length);
}
...
/*
  CCcam Client Version x.x.x and Build
	...
	2.1.3	3165
*/
  strn0cpy(versstr,"2.1.3",sizeof(versstr));
  strn0cpy(buildstr,"3165",sizeof(buildstr));
  ...
  if(!ParseStdConfig(config,&num)
     || ((n=sscanf(&config[num],":%20[^:]:%63[^:]:%255[^:]",username,password,params))<2 
     && (n=sscanf(&config[num],"%20s %63s",username,password))<2))
     return false;
  PRINTSCF(L_CC_CORE,"%s: username=%s password=%s",name,username,password);
  if(n>2) {
    char *save;
        ...
        *v++=0;
        if(!strcasecmp(p,"NODEID")) {
          ...
          if(GetHex(v2,nodeid,sizeof(nodeid),false)!=sizeof(nodeid)) {
            PRINTSCF(L_CC_CORE,"NODEID parameter format error");
            return false;
            }
          }
          ...
          wantEmus=(atoi(v)!=0);
          if(wantEmus) PRINTSCF(L_CC_CORE,"requesting emulations from server");
          }
        else {
          PRINTSCF(L_CC_CORE,"unknown parameter '%s'",p);
          return false;
          }
        }
      else {
        PRINTSCF(L_CC_CORE,"bad parameter format '%s'",p);
        return false;
        }
      p=strtok_r(0,",",&save);
      }
  ...
  PRINTSCF(L_CC_CORE,"pretended CCcam version '%s' build '%s'",versstr,buildstr);
  return true;
}

void cCardClientCCcam2::Logout(void)
{
  PRINTSCF(L_CC_CCCAM2,"logout from server initiated");
  login=false;
  Cancel(cThread::ThreadId()!=readerTid ? 2:-1);
  readerTid=0;
  PRINTSCF(L_CC_CCCAM2EX,"reader thread stopped");
  cCardClient::Logout();
  PRINTSCF(L_CC_CCCAM2EX,"network shut down");
  shares.Lock(true);
  shares.Clear();
  emmProcessing=false;
  shares.Unlock();
  ...
  cmd05mode=CMD05_MODE_UNKNOWN; cmd05off=0;
  cmd0Ccrypt.Reset();
  PRINTSCF(L_CC_CCCAM2EX,"logout done");
}
...
bool cCardClientCCcam2::Login(void)
{
  ...
  if((len=RecvMsg(buffer,16))<0) {
    PRINTSCF(L_CC_CCCAM2,"no welcome from server");
    return false;
    }
  LDUMP(L_CC_CCCAM2DT,buffer,len,"welcome answer:");
  if(!cCCcamCrypt::CheckConnectChecksum(buffer,len)) {
    PRINTSCF(L_CC_CCCAM2,"bad welcome from server");
    Logout();
    return false;
    }
  PRINTSCF(L_CC_CCCAM2EX,"welcome checksum correct");
  ...
  if(!CryptSend(buff2,20)) {
    PRINTSCF(L_CC_CCCAM2,"failed to send welcome response");
    return false;
    }
  ...
  if(!CryptSend(buffer,20)) {
    PRINTSCF(L_CC_CCCAM2,"failed to send username");
    return false;
    }

  encr.Encrypt((unsigned char *)password,buffer,strlen(password));
  if(!CryptSend((unsigned char *)cccamstr,6)) {
    PRINTSCF(L_CC_CCCAM2,"failed to send password hash");
    return false;
    }

  if((len=CryptRecv(buffer,20))<0) {
    PRINTSCF(L_CC_CCCAM2,"no login answer from server");
    return false;
    }
  LDUMP(L_CC_CCCAM2DT,buffer,len,"login answer:");

  if(strcmp(cccamstr,(char *)buffer)!=0) {
    PRINTSCF(L_CC_CCCAM2,"login failed");
    Logout();
    return false;
    }
  PRINTSCF(L_CC_LOGIN,"CCcam login succeed");
  ...
  if(!CryptSend((unsigned char*)&clt,sizeof(clt))) {
    PRINTSCF(L_CC_CCCAM2,"failed to send clientinfo");
    return false;
    }
  login=true;
  Start();
...
bool cCardClientCCcam2::ProcessECM(const cEcmInfo *ecm, const unsigned char *data, unsigned char *Cw, int cardnum)
{
  ...
  PRINTSCF(L_CC_CCCAM2,"%d: ECM caid %04x prov %04x sid %d pid %04x",cardnum,ecm->caId,ecm->provId,ecm->prgId,ecm->ecm_pid);
  int sctlen=SCT_LEN(data);
  if(sctlen>=256) {
    PRINTSCF(L_CC_CCCAM2,"ECM data length >=256 not supported by CCcam");
    return false;
    }
  ...
  if(curr.GetShares(ecm,&shares)<1) {
    PRINTSCF(L_CC_CCCAM2,"no shares for this ECM");
    return false;
    }
  if(LOG(L_CC_CCCAM2SH)) {
    PRINTSCF(L_CC_CCCAM2SH,"share try list for caid %04x prov %06x pid %04x",ecm->caId,ecm->provId,ecm->ecm_pid);
    for(cShare *s=curr.First(); s; s=curr.Next(s))
      PRINTSCF(L_CC_CCCAM2SH,"shareid %08x hops %d %c lag %4d",s->ShareID(),s->Hops(),s->Status()>0?'+':(s->Status()<0?'-':' '),s->Lag());
    }
  ...
  for(cShare *s=curr.First(); s && !max.TimedOut(); s=curr.Next(s)) {
    ...
    PRINTSCF(L_CC_CCCAM2EX,"now try shareid %08x",shareid);
    LDUMP(L_CC_CCCAM2DT,req,ecm_len,"send ecm:");
    if(pendingDCW>0)
      PRINTSCF(L_CC_CCCAM2,"WARN: there are pending %d DCW answers. This may cause trouble...",pendingDCW);
    pendingDCW++;
    if(!CryptSend((unsigned char *)req,ecm_len)) {
      PRINTSCF(L_CC_CCCAM2,"failed to send ecm request");
      break;
      }
    cwmutex.Lock();
    newcw=false;
    ...
    if(cwwait.TimedWait(cwmutex,MAXLAG)) {
      ...
      PRINTSCF(L_CC_CCCAM2EX,"wait returned after %lld",l);
      if(newcw) {
        ...
        PRINTSCF(L_CC_CCCAM2,"got CW");
        return true;
        }
      else PRINTSCF(L_CC_CCCAM2EX,"no CW from this share");
      }
    else {
      uint64_t l=lag.Elapsed();
      shares.SetLag(shareid,l);
      PRINTSCF(L_CC_CCCAM2EX,"getting CW timed out after %lld",l);
      }
    ecmshares.AddStatus(ecm,shareid,-1);
    cwmutex.Unlock();
    }
  PRINTSCF(L_CC_ECM,"%s: unable to decode the channel",name);
  return false;
}
            ...
            if(id>0 || emmAllowed>1) {
              if(len<256) {
                ...
                if(!CryptSend((unsigned char *)req,emm_len))
                  PRINTSCF(L_CC_CCCAM2,"failed to send emm request");
                }
              else PRINTSCF(L_CC_CCCAM2,"EMM data length >=256 not supported by CCcam");
...
void cCardClientCCcam2::Action(void)
{
  ...
  while(Running() && so.Connected()) {
    ...
    if(len>0) {
      PRINTSCF(L_CC_CCCAM2DT,"net read: len=%d cnt=%d dec=%d",len,cnt+len,dec);
      cnt+=len;
      }
    ...
    while(proc+(int)sizeof(struct CmdHeader)<=cnt) {
      ...
      if((a=proc+sizeof(struct CmdHeader)-dec)>0) {
        ...
        dec+=a;
        PRINTSCF(L_CC_CCCAM2DT,"hdr decrypt: cnt=%d proc=%d dec=%d",cnt,proc,dec);
        }
      ...
      if(proc+l>cnt) {
        PRINTSCF(L_CC_CCCAM2DT,"cmd incomplete: proc+l=%d cnt=%d dec=%d",proc+l,cnt,dec);
        break;
        }
      ...
      if((a=proc+l-dec)>0) {
        decr.Decrypt(recvbuff+dec,recvbuff+dec,a);
        dec+=a;
        PRINTSCF(L_CC_CCCAM2DT,"payload decrypt: cnt=%d proc=%d dec=%d",cnt,proc,dec);
        }
    ...
    if(lastsend.TimedOut()) {
      ...
      if(!CryptSend((unsigned char *)&ping,sizeof(ping)))
        PRINTSCF(L_CC_CCCAM2,"failed to send server PING");
      }
    usleep(10);
    }
  readerTid=0;
