...
bool cEmu::GetPkKeys(const unsigned char *select, unsigned char *pkset)
{
  ...
  while(!Run(MAX_COUNT)) {
    ...
    if(pc==FindKeyEnd) {
      if(!cc.c) {
        PRINTSCF(L_SYS_EMU,"Updating PK keys");
        unsigned short loc=(Get(Rc1H)<<8)+Get(Rc1L);
        for(int i=0; i<45; i+=15) GetMem(loc+4+i,pkset+i,15);
        return true;
        }
      else {
        PRINTSCF(L_SYS_EMU,"Updating PK keys failed. Used a correct EEPROM image for provider %04x ?",((select[0]<<8)|select[1]));
        break;
        }
      }
    }
...
{
  PRINTSCF(L_SYS_EMU,"Unsupported math call $%02x in ROM %d, please report",a,romNr);
  return false;
}
...
{
  PRINTSCF(L_SYS_EMU,"math call: $%02x",a);
  switch(a) {
    case 0x02:
      switch(Get(0x41)) {
...
bool cNagra1::DecryptECM(const unsigned char *in, unsigned char *out, const unsigned char *vkey, int len, BIGNUM *e1, BIGNUM *n1, BIGNUM *n2)
{
  ...
  if(rsa.RSA(result,&in[2],len,pubExp,n2)<=0) {
    PRINTSCF(L_SYS_CRYPTO,"error decrypting ECM (round 1)");
    return false;;
    }
  cBN mod;
  ...
  if(rsa.RSA(out,64,result,e1,n1,false)!=64) {
    PRINTSCF(L_SYS_CRYPTO,"error: result of ECM decryption is not 64 bytes");
    return false;
    }
  if(vkey && !SigCheck(out,&out[56],vkey,7)) {
    PRINTSCF(L_SYS_CRYPTO,"ECM decryption failed");
    return false;
    }
  return true;
}
...
bool cNagra1::DecryptEMM(const unsigned char *in, unsigned char *out, const unsigned char *vkey, int len, BIGNUM *e1, BIGNUM *n1, BIGNUM *n2)
{
  ...
  if(rsa.RSA(result,&in[9],len,pubExp,n2)<=0) {
    PRINTSCF(L_SYS_CRYPTO,"error decrypting EMM (round 1)");
    return false;;
    }
  cBN mod;
  ...
  BN_add(result,result,mod);
  if(rsa.RSA(out,64,result,e1,n1,false)!=64) {
    PRINTSCF(L_SYS_CRYPTO,"error: result of EMM decryption is not 64 bytes");
    return false;
    }
  ...
  PRINTSCF(L_SYS_CRYPTO,"warning: signature failed,trying signature exchange method");
  unsigned char buf[8];
  ...
  if(vkey && !SigCheck(out,buf,vkey,8)) {
    PRINTSCF(L_SYS_CRYPTO,"fatal: signature failed in signature exchange method");
    return false;
    }
  return true;
}
...
bool cSystemNagra::ProcessECM(const cEcmInfo *ecm, unsigned char *data)
{
  ...
  cTimeMs minTime;
  if(data[3]!=0x03) {
    PRINTSCF(L_SYS_ECM,"invalid ECM");
    return false;
    }
  data+=7;
  if(data[0]!=0x10) {
    PRINTSCF(L_SYS_ECM,"no ECM data available");
    return false;
    }
  ...
  if(ecmParm==7) decrypt=false;
    ...
    PRINTSCF(L_SYS_ECM,"unknown ecmParm, ignoring ECM");
    return false;
    }
  ...
  else if(doLog) PRINTSCF(L_SYS_KEY,"missing %04X V TYP%d key (non-fatal)",id,verType);
  ...
  if(decrypt && hasVerifyKey && !SigCheck(decrypted,data+3,verifyKey,desLen/8)) {
    PRINTSCF(L_SYS_ECM,"signature check failed in DES decrypt");
    return false;
    }
  ...
  switch(decrypted[0]) {
    ...
    case 0x12: // Even CW
      ...
      break;
    default: PRINTSCF(L_SYS_ECM,"failed to get CW"); return false;
    }
  ...
  if((cmdLen-(ecmLen+2))>64 && (*mecmData==0x20 || *mecmData==0x39)) {
    if(mecmRSALen!=64) {
      if(mecmRSALen>64 && doLog)
        PRINTSCF(L_SYS_ECM,"ECM too big (len: %d)",mecmRSALen);
      return false;
      }
    ...
    if(!(pk=keys.FindKey('N',mecmProvId,MBC3('E','1',KEYSET(0,pkey,keyType)),-1))) {
      if(doLog) PRINTSCF(L_SYS_KEY,"missing %04x E1 PK%d TYP%d key",mecmProvId,pkey,keyType);
      return false;
      }
    pk->Get(e1);
    if(!(pk=keys.FindKey('N',mecmProvId,MBC3('N','1',KEYSET(0,pkey,keyType)),-1)))  {
      if(doLog) PRINTSCF(L_SYS_KEY,"missing %04x N1 PK%d TYP%d key",mecmProvId,pkey,keyType);
      return false;
      }
    pk->Get(n1);
    if(!(pk=keys.FindKey('N',mecmProvId,MBC3('N','2',KEYSET(0,0,keyType)),-1)))  {
      if(doLog) PRINTSCF(L_SYS_KEY,"missing %04x N2 TYP%d key",mecmProvId,keyType);
      return false;
      }
    pk->Get(n2);
    hasVerifyKey=false;
    ...
    else if(doLog) PRINTSCF(L_SYS_KEY,"missing %04x V TYP%d key (non-fatal)",mecmProvId,keyType);
...
void cSystemNagra::ProcessEMM(int pid, int caid, const unsigned char *buffer)
{
  ...
  if(memcmp(&buffer[3],tester,sizeof(tester)) || SCT_LEN(buffer)<(12+9+64)) {
    PRINTSCF(L_SYS_EMM,"%d: not a valid EMM structure",CardNum());
    return;
    }
    ...
    while(i<sizeof(emmdata)) {
      if((emmdata[i]&0xF0)==0xF0) { // Update with CPU code
        ...
        if(!emu || !emu->Matches(romNr,id)) {
          delete emu; emu=0;
          PRINTSCF(L_SYS_EMM,"%d: setting defaults for ROM %d",CardNum(),romNr);
          switch(romNr) {
            case 3:  emu=new cEmuRom3;  break;
            case 7:  emu=new cEmuRom7;  break;
            case 10: emu=new cEmuRom10; break;
            case 11: emu=new cEmuRom11; break;
            default: PRINTSCF(L_SYS_EMM,"%d: unsupported ROM",CardNum()); return;
            }
          if(!emu || !emu->Init(romNr,id)) {
            delete emu; emu=0;
            PRINTSCF(L_SYS_EMM,"%d: initialization failed for ROM %d",CardNum(),romNr);
            return;
            }
          }
        unsigned char ki[2];
        if((gotKeys=emu->GetOpKeys(emmdata,ki,key0,key1))) {
          keyId=(ki[0]<<8)+ki[1];
          PRINTSCF(L_SYS_EMM,"%d: got keys for %04X (ROM %d)",CardNum(),keyId,romNr);
          }
        ...
        if(emu->GetPkKeys(&select[0],&pkset[0][0])) {
          ...
          PRINTSCF(L_SYS_EMM,"%d: got PK keys for %04X (ROM %d)",CardNum(),pkKeyId,romNr);
          for(int i=0; i<3; i++) {
        ...
        if(++nrKeys==2) {
          gotKeys=true;
          PRINTSCF(L_SYS_EMM,"%d: got keys for %04X (plain)",CardNum(),keyId);
          break;
          }
        }
      else {
        PRINTSCF(L_SYS_EMM,"%d: ignored nano %02x",CardNum(),emmdata[i]);
        break;
        }
      }
    if(gotKeys) {
