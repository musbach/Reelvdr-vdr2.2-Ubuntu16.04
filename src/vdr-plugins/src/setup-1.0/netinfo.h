...
class cNetInterface
{
  ...
  private:
    ...
    char addr_v6_[128];          /* IP address  v6  */
    struct sockaddr netmask_;    /* IP network mask */
    //int metric;                /* routing metric  */

    //private functions
    const char *IpAddr(sockaddr_in * sa);
    const char *IpAddrv6(sockaddr_in6 * sa);
    int GetIfKernelConf();
};
...
//inline const char *cNetInterface::IpAddress() const
inline const char *cNetInterface::IpAddress()
{
...
//inline const char *cNetInterface::NetMask() const
inline const char *cNetInterface::NetMask()
{
