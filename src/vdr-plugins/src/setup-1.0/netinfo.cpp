...
// defined in menus.cpp. Must go into ReelLib
...
#if 0 // parsing a file to detect network interfaces causes crash sometimes
    for (int i = 0; i < static_cast < int >(ifNames_.size()); i++)
    {
        ReadListProcNet(ifNames_[i]);
    }
#else
    std::vector<std::string>::iterator it = ifNames_.begin();
    while (it != ifNames_.end())
    {
        if (IsNetworkDevicePresent(it->c_str()))
            netInterfaces_.push_back(new cNetInterface(*it));
        ++it;
    }
#endif
...
int cNetInterface::GetIfKernelConf()
{
    ...
    if (fd >= 0) {
        ...
        /** TB: there's now method to query the IPv6 address via ioctl?! */
        FILE *process;
...
const char *cNetInterface::HwAddress() const
{
    ...
    char *inet_hw = MALLOC(char, 18); sprintf(inet_hw, "%02hX:%02hX:%02hX:%02hX:%02hX:%02hX",
            static_cast < unsigned char >(hwaddr_[0]),
            static_cast < unsigned char >(hwaddr_[1]),
            static_cast < unsigned char >(hwaddr_[2]),
            static_cast < unsigned char >(hwaddr_[3]),
            static_cast < unsigned char >(hwaddr_[4]),
            static_cast < unsigned char >(hwaddr_[5]));

    dsyslog("HWAddres:  %s\n",inet_hw);
    return inet_hw;
}

const char *cNetInterface::IpAddr(sockaddr_in * sa)
{
...
const char *cNetInterface::IpAddrv6(sockaddr_in6 * sa) {
