...
void cSystemScViaccess::ParseCADescriptor(cSimpleList<cEcmInfo> *ecms, unsigned short sysId, int source, const unsigned char *data, int len)
{
  ...
  if(pid>=0xAA && pid<=0xCF) {
    PRINTSCF(L_CORE_ECMPROC,"sc-viaccess: dropped \"fake\" ecm pid 0x%04xn",pid);
    return;
    }
...
bool cSmartCardDataViaccess::Parse(const char *line)
{
  ...
  if(!strncasecmp(line,"PIN",3))       { type=dtPIN; line+=3; }
    ...
    PRINTSCF(L_CORE_LOAD,"smartcarddataviaccess: format error: datatype");
    return false;
    }
  line=skipspace(line);
  if(GetHex(line,id,IdLen())!=IdLen()) {
    PRINTSCF(L_CORE_LOAD,"smartcarddataviaccess: format error: serial");
    return false;
    }
  line=skipspace(line);
  if(type==dtPIN) {
    ...
    for(i=0; i<16; i++) {
      if(!isdigit(*line)) {
        PRINTSCF(L_CORE_LOAD,"smartcarddataviaccess: format error: pin");
        return false;
        }
      pin[i]|=(*line++ - '0')<<shift;
      shift^=4;
...
LDUMP(L_CORE_LOAD,pin,sizeof(pin),"smartcarddataviaccess: pin ");
    ...
    if(GetHex(line,aes,16)!=16) {
      PRINTSCF(L_CORE_LOAD,"smartcarddataviaccess: format error: aes");
      return false;
      }
    }
  return true;
...
bool cSmartCardViaccess::Init(void)
{
  ...
  if(atr->T!=0 || atr->histLen<7 || memcmp(&atr->hist[5],verifyBytes,sizeof(verifyBytes))) {
    PRINTSCF(L_SC_INIT,"doesn't look like a Viaccess card");
    return false;
    }
  ...
  PRINTSCF(L_SC_INIT,"card v.%s",ver);
  ...
  do {
    ...
    if(!IsoRead(insc0,buff) || !Status()) {
      PRINTSCF(L_SC_ERROR,"failed to read prov properties");
      return false;
      }
    ...
    if(!IsoWrite(insac,buff2) || !Status()) {
      PRINTSCF(L_SC_ERROR,"failed to request sa");
      return false;
      }
    insb8[4]=0x06; // read sa
    if(!IsoRead(insb8,buff2) || !Status()) {
      PRINTSCF(L_SC_ERROR,"failed to read sa");
      return false;
      }
    ...
    if(p) {
      ...
      if(!IsoWrite(insac,buff) || !Status()) {
        PRINTSCF(L_SC_ERROR,"failed to request prov name");
        return false;
        }
      insb8[4]=0x02; // read name nano + len
      if(!IsoRead(insb8,buff) || !Status()) {
        PRINTSCF(L_SC_ERROR,"failed to read prov name length");
        return false;
        }
      unsigned int nameLen=buff[1];
      if(nameLen>=sizeof(p->name)) {
        PRINTSCF(L_SC_ERROR,"provider name buffer overflow");
        nameLen=sizeof(p->name)-1;
        }
      insb8[4]=nameLen;
      if(!IsoRead(insb8,buff) || !Status()) {
        PRINTSCF(L_SC_ERROR,"failed to read prov name");
        return false;
        }
      memcpy(p->name,buff,nameLen); p->name[nameLen]=0;

      PRINTSCF(L_SC_INIT,"provider %06x (%s)",(int)p->ProvId(),p->name);
      infoStr.Printf("Prov %06x (%s) SA %08u\n",(int)p->ProvId(),p->name,Bin2Int(&buff2[2],4));
      }
    else PRINTSCF(L_SC_ERROR,"no memory for provider");

    insa4[2]=0x02; // next issuer
    if(!IsoWrite(insa4,buff) || !Status()) {
      PRINTSCF(L_SC_ERROR,"failed to select next issuer");
      return false;
      }
    issuer++;
    } while(GetSW1()==0x00);
  ...
  if(disableParental) {
      ...
      if(entry) {
        ...
        if(!IsoWrite(ins24,pindt) || !Status())
          PRINTSCF(L_SC_ERROR,"failed to disable parental lock");
        }
  ...
  cSmartCardDataViaccess *entry=(cSmartCardDataViaccess *)smartcards.FindCardData(&cd);
  if(entry) {
    PRINTSCF(L_SC_INIT,"got AES key from smartcard.conf.");
    SetKey(entry->aes);
    }
    ...
    if(CheckKey(p,keynr)) {
      ...
      if(cwmask) {
        if(hasD2 && !Decrypt(cwbuff,16))
          PRINTSCF(L_SC_ERROR,"no AES key for this card");
        if(cwmask&1) memcpy(cw,cwbuff,8);
        if(cwmask&2) memcpy(cw+8,cwbuff+8,8);
        return true;
        }
      }
    else PRINTSCF(L_SC_ERROR,"key not found on card");
    }
  else PRINTSCF(L_SC_ERROR,"provider not found on card");
  return false;
}
