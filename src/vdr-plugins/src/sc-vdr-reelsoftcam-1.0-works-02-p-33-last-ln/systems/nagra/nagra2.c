...
{
  if(Running()) Update();
  PRINTSCF(L_SYS_EMU,"n2timer %d: read %u %02x %02x = %02x",nr,cycles,ctrl&0xff,latch&0xff,val);
  return val;
}
...
void cN2Timer::Ctrl(unsigned char val)
{
  if(Running()) {
    ...
    if(!Running()) {
      ...
      --cycles;
      Update();
      PRINTSCF(L_SYS_EMU,"n2timer %d: stopped cycles=%d (%02x) ctrl=%02x latch=%02x\n",nr,cycles,val,ctrl,latch&0xff);
      }
    }
  else {
    ...
    if(Running()) {
      ...
      if((ctrl&tmINTERRUPT)) {
        ...
        if(nr==2) PRINTSCF(L_SYS_EMU,"n2timer %d: set for %u cycles (%02x %02x)",nr,intrCycles-cycles,Latch(),ctrl&0xff);
        }
      ctrl&=~tmLATCHED;
      }
    }
...
:cMapMem(HW_OFFSET,HW_REGS)
{
  ...
  PRINTSCF(L_SYS_EMU,"mapmemhw: new HW map off=%04x size=%04x",offset,size);
}
...
{
  PRINTSCF(L_SYS_MAP,"%04x: calling function %02X",mapid,f);
  cycles=0;
  unsigned int startcycles=CpuCycles();
  interrupted=false; interruptible=true;
  try {
    if(!Map(f,data,l) && !MapGeneric(f,data,l))
      PRINTSCF(L_SYS_MAP,"%04x: unsupported map call %02x",mapid,f);
    if(cycles) {
    ...
    interrupted=true;
    PRINTSCF(L_SYS_MAP,"%04x: call %02x interrupted (%d cycles)",mapid,f,CpuCycles()-startcycles);
    }
  interruptible=false;
...
{
  PRINTSCF(c,"provider %04x capabilities%s%s%s%s%s",id,
...
{
  PRINTSCF(L_CORE_DYN,"n2providers: registering prov %04X with flags %d",plink->id,plink->flags);
  plink->next=first;
  first=plink;
}
...
bool cNagra2::DecryptECM(const unsigned char *in, unsigned char *out, const unsigned char *key, int len, const unsigned char *vkey, BIGNUM *m)
{
  ...
  if(rsa.RSA(out,in+1,64,pubExp,m)<=0) {
    PRINTSCF(L_SYS_CRYPTO,"first RSA failed (ECM)");
    return false;
    }
  ...
  if(rsa.RSA(out,out,64,pubExp,m,false)<=0) {
    PRINTSCF(L_SYS_CRYPTO,"second RSA failed (ECM)");
    return false;
    }
  if(vkey && !Signature(vkey,out,out+8,len-8)) {
    PRINTSCF(L_SYS_CRYPTO,"signature failed (ECM)");
    return false;
    }
  return true;
}
...
bool cNagra2::DecryptEMM(const unsigned char *in, unsigned char *out, const unsigned char *key, int len, const unsigned char *vkey, BIGNUM *m)
{
  ...
  if(rsa.RSA(out,in+1,96,pubExp,m)<=0) {
    PRINTSCF(L_SYS_CRYPTO,"first RSA failed (EMM)");
    return false;
    }
  ...
  if(rsa.RSA(out,out,96,exp,m,false)<=0) {
    PRINTSCF(L_SYS_CRYPTO,"second RSA failed (EMM)");
    return false;
    }
  if(vkey && !Signature(vkey,out,out+8,len-8)) {
    PRINTSCF(L_SYS_CRYPTO,"signature failed (EMM)");
    return false;
    }
  return true;
}
...
#define NA_SOURCE_END   0x85c8 // cSource::FromString("S148W");
  ...
  if(cmdLen<64 || SCT_LEN(data)<cmdLen+10) {
    if(doLog) PRINTSCF(L_SYS_ECM,"bad ECM message msgLen=%d sctLen=%d",cmdLen,SCT_LEN(data));
    return false;
    }
  ...
  if(!(pk=keys.FindKey('N',id,MBC('M','1'),-1)))  {
    if(doLog) PRINTSCF(L_SYS_KEY,"missing %04x M1 key",id);
    return false;
    }
  pk->Get(m1);
  ...
  else if(doLog && id!=lastEcmId) PRINTSCF(L_SYS_KEY,"missing %04x V key (non-fatal)",id);
  ...
  if(!DecryptECM(data+9,buff,ideaKey,cmdLen,hasVerifyKey?vKey:0,m1)) {
    if(doLog) PRINTSCF(L_SYS_ECM,"decrypt of ECM failed (%04x)",id);
    return false;
    }
    ...
    switch(buff[i]) {
      ...
      case 0x11:
        if(buff[i+1]==0x09) {
          ...
          PRINTSCF(L_SYS_ECM,"bad length %d in CW nano %02x",buff[i+1],buff[i]);
          i++;
          }
        break;
      case 0x00:
  ...
  if(mecmAlgo>0) {
    ...
    else { PRINTSCF(L_SYS_ECM,"MECM for provider %04x not supported",id); return false; }
    }
...
void cSystemNagra2::ProcessEMM(int pid, int caid, const unsigned char *buffer)
{
  ...
  if(cmdLen<96 || SCT_LEN(buffer)<cmdLen+15) {
    PRINTSCF(L_SYS_EMM,"bad EMM message msgLen=%d sctLen=%d",cmdLen,SCT_LEN(buffer));
    return;
    }
  ...
  if(!(pk=keys.FindKeyNoTrig('N',id,MBC(N2_MAGIC,keyset+0x10+rsasel),96)))  {
    PRINTSCF(L_SYS_EMM,"missing %04x NN %.02X RSA key (96 bytes)",id,keyset+0x10+rsasel);
    return;
    }
  pk->Get(n);
  ...
  else if(id!=lastEmmId) PRINTSCF(L_SYS_EMM,"missing %04x NN %.02X signature key (non-fatal)",id,0x03+sigsel);
  if(!(pk=keys.FindKeyNoTrig('N',id,MBC(N2_MAGIC,keyset),24))) {
    if(!(pk=keys.FindKeyNoTrig('N',id,MBC(N2_MAGIC,keyset+sel),16))) {
      PRINTSCF(L_SYS_EMM,"missing %04x NN %.02x IDEA key (24 or 16 bytes)",id,keyset+sel);
      return;
      }
  ...
  if(!DecryptEMM(buffer+14,emmdata,ideaKey,cmdLen,hasVerifyKey?vKey:0,n)) {
    PRINTSCF(L_SYS_EMM,"decrypt of EMM failed (%04x)",id);
    return;
    }
    ...
    switch(emmdata[i]) {
        ...
        if((((emmdata[i+3]|0xF3)+1)&0xFF) != 0) {
          ...
          if(len>0 && ulen>0 && off+ulen<=len) {
            ...
            if((emmdata[i+1]&0x7F)==0) ok=true;
              ...
              else PRINTSCF(L_SYS_EMM,"POSTAU for provider %04x not supported",id);
              }
            if(ok) {
          ...
          else PRINTSCF(L_SYS_EMM,"nano42 key size mismatch len=%d off=%d ulen=%d",len,off,ulen);
          }
        else PRINTSCF(L_SYS_EMM,"nano42 0xf3 status exceeded");
        i+=7;
        break;
      ...
      case 0xBC: case 0xBD: case 0xBE: case 0xBF:
        ...
        if(!emmP || !emmP->HasFlags(N2FLAG_Bx)) {
          PRINTSCF(L_SYS_EMM,"B%X for provider %04x not supported",bx,id);
          i=cmdLen;
          break;
          }
        int r;
        if((r=emmP->ProcessBx(emmdata,cmdLen,i+1))>0)
          ...
          PRINTSCF(L_SYS_EMM,"B%X executing failed for %04x",bx,id);
          i=cmdLen;
          }
        break;
        }
      ...
      case 0xE3:                                // Eeprom update
        ...
        if(!emmP || !emmP->HasFlags(N2FLAG_Ex)) {
          i+=emmdata[i+4]+5;
          PRINTSCF(L_SYS_EMM,"E%X for provider %04x not supported",ex,id);
          break;
          }
        int r;
        if((r=emmP->ProcessEx(emmdata,cmdLen,i+1))>0)
          ...
          PRINTSCF(L_SYS_EMM,"E%X executing failed for %04x",ex,id);
          i=cmdLen;
          }
        break;
        }
