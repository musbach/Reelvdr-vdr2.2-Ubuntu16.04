...
bool cCamCryptNagra::DecryptDT08(const unsigned char *dt08, unsigned int irdid, cBN *irdmod, bool fakeid)
{
  ...
  if(memcmp(signature,buff,8)) {
    PRINTSCF(L_SC_PROC,"DT08 signature failed");
    return false;
    }
...
bool cSmartCardDataNagra::Parse(const char *line)
{
  ...
  if(GetHex(line,id,sizeof(id))!=sizeof(id)) {
    PRINTSCF(L_CORE_LOAD,"smartcarddatanagra: format error: card id");
    return false;
    }
  cardid=UINT32_BE(id);
  line=skipspace(line);
  if(!strncasecmp(line,"SK",2)) { isSK=true; dlen=96; line+=2; }
    ...
    if(GetHex(line,bk,sizeof(bk))!=sizeof(bk)) {
      PRINTSCF(L_CORE_LOAD,"smartcarddatanagra: format error: boxkey");
      return false;
      }
    line=skipspace(line);
    if(!strncasecmp(line,"IRDMOD",6)) { isIrdMod=true; line+=6; }
      ...
      PRINTSCF(L_CORE_LOAD,"smartcarddatanagra: format error: IRDMOD/CARDMOD");
      return false;
      }
    }
  line=skipspace(line);
  ...
  if(GetHex(line,buff,dlen)!=dlen) {
    PRINTSCF(L_CORE_LOAD,"smartcarddatanagra: format error: data block");
    return false;
    }
  if(!isSK) {
    mod.Get(buff,64);
    ...
    if(UINT16_BE(sk->cs)!=CRC16(buff,sizeof(*sk)-sizeof(sk->cs),false)) {
      PRINTSCF(L_CORE_LOAD,"smartcarddatanagra: secondary key CRC failed");
      return false;
      }
...
bool cSmartCardNagra::SetIFS(unsigned char size)
{
  ...
  PRINTSCF(L_SC_INIT,"Setting IFS to 0x%02X",size);
  if(SerWrite(buf,5)!=5) {
    PRINTSCF(L_SC_ERROR,"failed to write IFS command");
    return false;
    }
  if(SerRead(buff,5,cardCfg.workTO)!=5 || XorSum(buff,5)) {
    PRINTSCF(L_SC_ERROR,"setting IFS to %02x failed",size);
    return false;
    }
  return true;
}
...
bool cSmartCardNagra::GetCardStatus(void)
{
  if(!DoBlkCmd(0xC0,0x02,0xB0,0x06) || !Status()) {
    PRINTSCF(L_SC_ERROR,"failed to read card status");
    return false;
    }
...
bool cSmartCardNagra::Init(void)
{
  ...
  if(!memcmp(atr->hist,atrDNASP,sizeof(atrDNASP))) {
    ...
    PRINTSCF(L_SC_INIT,"detected native T1 nagra card (N%d Mode)",isN3?3:2);
    ...
    PRINTSCF(L_SC_INIT,"detected nagra tiger card");
    if(!SetIFS(0xFE)) return false;
    memcpy(rominfo,atr->hist,sizeof(rominfo));
    cardId=0xFFFFFFFF;
    isTiger=true;
    ...
    PRINTSCF(L_SC_INIT,"detected tunneled T14 nagra card");
    if(!allowT14) {
      PRINTSCF(L_SC_INIT,"Nagra mode for T14 card disabled in setup");
      return false;
      }
    PRINTSCF(L_SC_INIT,"using nagra mode");
    isT14Nagra=true;
    if(!DoBlkCmd(0x10,0x02,0x90,0x11)) {
      PRINTSCF(L_SC_ERROR,"get rom version failed");
      return false;
      }
    memcpy(rominfo,&buff[2],15);
    }
  else {
    PRINTSCF(L_SC_INIT,"doesn't look like a nagra card");
    return false;
    }
  ...
  PRINTSCF(L_SC_INIT,"rom version: %s revision: %s",rom,rev);
  ...
  if(!HasCamMod()) {
    ...
    if(entry) {
      ...
      PRINTSCF(L_SC_ERROR,"can't find CARD modulus");
      return false;
      }
    }
...
bool cSmartCardNagra::DoCamExchange(void)
{
  if(!isTiger) {
    ...
    if(SENDDATETIME())
       SendDateTimeCmd();
    if(RENEW_SESSIONKEY()) {
      PRINTSCF(L_SC_ERROR,"Session key negotiation failed!");
      return false;
      }
    }
  else {
...
bool cSmartCardNagra::GetDataType(unsigned char dt, int len, int shots)
{
  ...
  for(int i=0; i<shots; i++) {
    if(!DoBlkCmd(0x22,0x03,0xA2,len,&dt) || !Status()) {
     PRINTSCF(L_SC_ERROR,"failed to get datatype %02X",dt);
     return false;
     }
    if(buff[5]==0) break;
...
bool cSmartCardNagra::ParseDataType(unsigned char dt)
{
  switch(dt) {
    case IRDINFO:
      ...
      PRINTSCF(L_SC_INIT,"CAID: %04x PROV: %04x CARD ID: %08x IRD ID: %08x",caid,provId,cardId,irdId);
    ...
    case TIERS:
      ...
      if(chid) {
        ...
        PRINTSCF(L_SC_INIT,"|%04X|%04X|%s-%s|",id,chid,ds,de);
        }
      break;
      }
    case CAMDATA:
      if(buff[14]==0x49) {
        ...
        if(entry) {
          ...
          PRINTSCF(L_SC_ERROR,"can't find IRD modulus");
          return false;
          }
        }
      break;
  ...
  if(!isTiger) {
    ...
    for(int i=1; i>=0; i--) {
      if(((!isN3 && DoBlkCmd(data[3],data[4]+2,0x87,0x02,pkt)) ||
          ...
          Status())
        break;
      PRINTSCF(L_SC_ERROR,"ECM cmd failed%s",(i>0)?", retrying":"");
      if(i==0) return false;
      }
    cCondWait::SleepMs(10);
...
bool cSmartCardNagra::DoBlkCmd(unsigned char cmd, int ilen, unsigned char res, int rlen, const unsigned char *data)
{
  ...
  */
  ...
  int dlen=ilen-2;
  if(dlen<0) {
    PRINTSCF(L_SC_ERROR,"invalid data length encountered");
    return false;
    }
  msg[c+2]=dlen;
  if(isT14Nagra) msg[c]++;
  ...
  if(SerWrite(msg,c)==c) {
    ...
    if(SerRead(buff,3,cardCfg.workTO)!=3) {
      PRINTSCF(L_SC_ERROR,"reading back reply failed");
      return false;
      }
    int xlen=buff[2]+1;
    if(SerRead(buff+3,xlen,cardCfg.workTO)!=xlen) {
      PRINTSCF(L_SC_ERROR,"reading back information block failed");
      return false;
      }
    xlen+=3;
    if(XorSum(buff,xlen)) {
      PRINTSCF(L_SC_ERROR,"checksum failed");
      return false;
      }
    LDUMP(L_CORE_SC,buff,xlen,"NAGRA: ->");

    if(buff[3]!=res) {
      PRINTSCF(L_SC_ERROR,"result not expected (%02X != %02X)",buff[3],res);
      return false;
      }
    if(buff[2]-2!=rlen) {
      PRINTSCF(L_SC_ERROR,"result length not expected (%d != %d)",buff[2]-2,rlen);
      return false;
      }
