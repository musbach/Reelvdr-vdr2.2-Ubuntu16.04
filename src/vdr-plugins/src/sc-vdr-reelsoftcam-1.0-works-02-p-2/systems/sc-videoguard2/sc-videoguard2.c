...
bool cSmartCardDataVideoGuard2::Parse(const char *line)
{
  ...
  if(!strncasecmp(line,"BOXID",5))     { type=dtBoxId; line+=5; }
    ...
    PRINTSCF(L_CORE_LOAD,"smartcarddatavideoguard2: format error: datatype");
    return false;
    }
  line=skipspace(line);
  switch(type) {
    case dtBoxId:
      if(GetHex(line,boxid,sizeof(boxid))!=sizeof(boxid)) {
        PRINTSCF(L_CORE_LOAD,"smartcarddatavideoguard2: format error: boxid");
        return false;
        }
      break;
    case dtSeed:
      if(GetHex(line,seed0,sizeof(seed0))!=sizeof(seed0)) {
        PRINTSCF(L_CORE_LOAD,"smartcarddatacryptoworks: format error: seed0");
        return false;
        }
      line=skipspace(line);
      if(GetHex(line,seed1,sizeof(seed1))!=sizeof(seed1)) {
        PRINTSCF(L_CORE_LOAD,"smartcarddatacryptoworks: format error: seed1");
        return false;
        }
      break;
    }
...
bool cSmartCardVideoGuard2::Init(void)
{
  ...
  if(atr->histLen<4 || memcmp(&atr->hist[3],vg2Hist,4)) {
    PRINTSCF(L_SC_INIT,"doesn't look like a VideoGuard2 card");
    return false;
    }
  ...
  if((l=ReadCmdLen(ins7401))<0) {
    PRINTSCF(L_SC_ERROR,"bogus answer. Now try delayed mode");
  ...
  if(!IsoRead(ins7401,buff) || !Status()) {
    PRINTSCF(L_SC_ERROR,"failed to read cmd list");
    return false;
    }
  ...
  if(DoCmd(ins7416)<0 || !Status()) {
    PRINTSCF(L_SC_ERROR,"cmd 7416 failed");
    return false;
    }
  ...
  if(!boxidOK) {
    PRINTSCF(L_SC_ERROR,"no boxID available");
    return false;
    }
  ...
  PRINTSCF(L_SC_INIT,"cardtype: %c%c.%d boxID %s caid %04x cardID %s",atr->hist[10],atr->hist[11],atr->hist[12],HexStr(str,boxID,4),CAID,HexStr(str2,cardID,4));

  cSmartCardDataVideoGuard2 cd(dtSeed);
  cSmartCardDataVideoGuard2 *entry=(cSmartCardDataVideoGuard2 *)smartcards.FindCardData(&cd);
  if(!entry) {
    PRINTSCF(L_SC_ERROR,"no NDS seed available");
    return false;
    }
...
void cSmartCardVideoGuard2::ReadTiers(void)
{
  ...
  for(int i=0; i<num; i++) {
    ...
    infoStr.Strcat(str);
    PRINTSCF(L_SC_INIT,"Tier %02x%02x expires %04d/%02d/%02d-%02d:%02d:%02d",buff[5+2],buff[5+3],y,m,d,H,M,S);
    }
}
...
int cSmartCardVideoGuard2::ReadCmdLen(const unsigned char *cmd)
{
  ...
  if(!IsoRead(cmd2,resp) || !Status()) {
    PRINTSCF(L_SC_ERROR,"failed to read %02x%02x cmd length",cmd[1],cmd[2]);
    return -1;
    }
  return resp[0];  
}
