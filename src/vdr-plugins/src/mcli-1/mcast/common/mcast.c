...
//	dbg ("Multicast streamer initialized successfully ! \n");
//	DERR("Multicast streamer initialized successfully ! \n");
	PRINTF("Multicast streamer initialized successfully ! \n");

	pthread_setcancelstate (PTHREAD_CANCEL_ENABLE, NULL);
	return s;
      error:
//	err ("Cannot init udp_server  !\n");
	DERR("Cannot init udp_server  !\n");
	if (s) {
		free (s);
	}
...
//		dbg ("setsockopt SO_BINDTODEVICE %s failed\n", ifname);
		DERR("setsockopt SO_BINDTODEVICE %s failed\n", ifname);
	}
#endif
...
//				DLOG("Selecting interface %s (%d)", ifname, s->idx);
				PRINTF("Selecting interface %s (%d)", ifname, s->idx);
			}
		} else {
			s->idx = 0;
		}
...
//		dbg ("Cannot raise priority to -15\n");
		DLOG("Cannot raise priority to -15\n");
	}
#endif
...
#  endif
	...
	if (ifname && strlen (ifname) && setsockopt (recvfd, SOL_SOCKET, SO_BINDTODEVICE, ifname, strlen (ifname) + 1)) {
//		dbg ("setsockopt SO_BINDTODEVICE %s failed\n", ifname);
//		DLOG("setsockopt SO_BINDTODEVICE %s failed\n", ifname);
		PRINTF("setsockopt SO_BINDTODEVICE %s failed\n", ifname);
	}
#endif
	if (bind (recvfd, (struct sockaddr *) &s->dest_addr, s->dest_addr_len) < 0) {
		warn ("bind failed\n");
