...
bool cSmartCardDataConax::Parse(const char *line)
{
  ...
  if(!strncasecmp(line,"PIN",3)) { type=dtPIN; line+=3; }
    ...
    PRINTSCF(L_CORE_LOAD,"smartcarddataconax: format error: datatype");
    return false;
    }
  line=skipspace(line);
  if(GetHex(line,id,IdLen())!=IdLen()) {
    PRINTSCF(L_CORE_LOAD,"smartcarddataconax: format error: serial");
    return false;
    }
  line=skipspace(line);
  if(type==dtPIN) {
    for(int i=0; i<4; i++) {
      if(!isdigit(*line)) {
        PRINTSCF(L_CORE_LOAD,"smartcarddataconax: format error: pin");
        return false;
        }
      pin[i]=*line++;
      }
...
bool cSmartCardConax::Init(void)
{
  ...
  if(atr->histLen<4 || memcmp(atr->hist, cnxHist, atr->histLen)) {
    PRINTSCF(L_SC_INIT,"doesn't look like a Conax card");
    return false;
    }
  ...
  if(!IsoWrite(ins26,hostVer) || !Status() || (l=GetLen())<=0) {
    PRINTSCF(L_SC_ERROR,"card caid request failed");
    return false;
    }
  insca[4]=l;
  if(!IsoRead(insca,buff) || !Status()) {
    PRINTSCF(L_SC_ERROR,"card caid read failed");
    return false;
    }
  ...
  PRINTSCF(L_SC_INIT,"card v.%d caid %04x",cardVer,sysId);
  ...
  if(!IsoWrite(ins82,ins82Data) || !Status() || (l=GetLen())<=0) {
    PRINTSCF(L_SC_ERROR,"card serial request failed");
    return false;
    }
  insca[4]=l;
  if(!IsoRead(insca,buff) || !Status()) {
    PRINTSCF(L_SC_ERROR,"card serial read failed");
    return false;
    }
  LBSTARTF(L_SC_INIT);
  LBPUT("card serials");
  ...
  if(IsoWrite(insc6,stdEntits) && Status() && (l=GetLen())>0) {
    ...
    do {
      insca[4]=l;
      if(!IsoRead(insca,buff) || !Status()) {
        PRINTSCF(L_SC_ERROR,"failed to read entitlements");
        break;
        }
      for(int i=0; i<l; i+=buff[i+1]+2) {
        if(buff[i]!=STDENTTAG) {
          PRINTSCF(L_SC_ERROR,"bad entitlement format %02x != %02x",buff[i],STDENTTAG);
          break;
          }
    ...
    PRINTSCF(L_SC_ERROR,"requesting entitlements failed");
...
/*
  ...
  if(IsoWrite(ins26,ppvEntits) && Status()) {
    while((l=GetLen())>0) {
      insca[4]=l;
      if(!IsoRead(insca,buff) || !Status()) {
        PRINTSCF(L_SC_ERROR,"failed to read PPV entitlements");
        break;
        }
    ...
    PRINTSCF(L_SC_ERROR,"getting PPV entitlements failed");
*/
...
bool cSmartCardConax::Decode(const cEcmInfo *ecm, const unsigned char *data, unsigned char *cw)
{
        ...
        case 0x31:
          ...
          if(entry) {
            ...
            if(!IsoWrite(insc8,insc8data) || !Status()) {
              PRINTSCF(L_SC_ERROR,"failed to send PIN");
              return false;
              }
            // resend ECM request
            ...
            PRINTSCF(L_SC_ERROR,"no PIN available");
            return false;
            }
          break;
          }
  ...
  if(gotIdx!=3) PRINTSCF(L_SC_ERROR,"strange, only got index %d cw. Failing... (this may be a bug)",gotIdx==1?0:1);
  return gotIdx==3;
}
