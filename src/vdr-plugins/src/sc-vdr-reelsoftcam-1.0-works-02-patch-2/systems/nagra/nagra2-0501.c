...
bool cN2Prov0501::RomInit(void)
{
  ...
  if(   AddMapper(new cMapRom(0x3800,"Rom120_003800-003FFF.bin",0x00000),0x3800,0x0800,0x00)
    ...
    hasMaprom=true;
    PRINTSCF(L_SYS_EMU,"%04x: using native MAP rom",id);
    }
  return true;
}
...
{
  if(data[pos-1]!=0xBC) {
    PRINTSCF(L_SYS_EMU,"%04X: bad nano %02X for ROM 120",id,data[pos-1]);
    return -1;
    }
  if(pos!=(0x93-0x80)) { // maybe exploitable
    PRINTSCF(L_SYS_EMU,"%04X: refuse to execute from %04x",id,0x80+pos);
    return -1;
    }
  if(Init(id,120)) {
