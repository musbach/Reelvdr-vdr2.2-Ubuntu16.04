//  $Id: inifile.cpp,v 1.5 2005/05/22 21:56:28 ralf Exp $
//
// CIniFile Class Implementation
//
...
CIniFile::CIniFile(t_Str szFileName)
{
	m_bDirty = false;
...
// SetFileName
// Set's the m_szFileName member variable. For use when creating the CIniFile
// object by hand (-vs- loading it from a file
void CIniFile::SetFileName(t_Str szFileName)
{
	if (m_szFileName.size() != 0 && CompareNoCase(szFileName, m_szFileName) != 0)
	{
		m_bDirty = true;

		Report(E_WARN, "[CIniFile::SetFileName] The filename has changed from <%s> to <%s>.",
...
bool CIniFile::Load(t_Str szFileName)
{
	...
	{
		Report(E_INFO, "[CIniFile::Load] Unable to open file. Does it exist?");
		return false;
	}
...
// was never called (the CIniFile object was created manually), then you
...
bool CIniFile::Save()
{
	...
	{
		// no point in saving
		Report(E_INFO, "[CIniFile::Save] Nothing to save.");
		return false; 
	}

	if ( m_szFileName.size() == 0 )
	{
		Report(E_ERROR, "[CIniFile::Save] No filename has been set.");
		return false;
	}
	...
	{
		Report(E_ERROR, "[CIniFile::Save] Unable to save file.");
		return false;
	}
...
bool CIniFile::SetKeyComment(t_Str szKey, t_Str szComment, t_Str szSection)
{
	KeyItor k_pos;
	t_Section* pSection;
...
// was not found.
bool CIniFile::SetSectionComment(t_Str szSection, t_Str szComment)
{
	SectionItor s_pos;
...
bool CIniFile::SetValue(t_Str szKey, t_Str szValue, t_Str szComment, t_Str szSection)
{
...
bool CIniFile::SetFloat(t_Str szKey, float fValue, t_Str szComment, t_Str szSection)
{
	char szStr[64];
...
bool CIniFile::SetInt(t_Str szKey, int nValue, t_Str szComment, t_Str szSection)
{
	char szStr[64];
...
bool CIniFile::SetBool(t_Str szKey, bool bValue, t_Str szComment, t_Str szSection)
{
...
t_Str CIniFile::GetValue(t_Str szKey, t_Str szSection)
{
	t_Key* pKey = GetKey(szKey, szSection);
...
// Returns the key value as a t_Str object. A return value of
// t_Str("") indicates that the key could not be found.
t_Str CIniFile::GetString(t_Str szKey, t_Str szSection)
{
...
// not found.
float CIniFile::GetFloat(t_Str szKey, t_Str szSection)
{
...
// not found.
int	CIniFile::GetInt(t_Str szKey, t_Str szSection)
{
	t_Str szValue = GetValue(szKey, szSection);
...
// not found.
bool CIniFile::GetBool(t_Str szKey, t_Str szSection)
{
	bool bValue = false;
	t_Str szValue = GetValue(szKey, szSection);
...
bool CIniFile::DeleteSection(t_Str szSection)
{
	SectionItor s_pos;
...
bool CIniFile::DeleteKey(t_Str szKey, t_Str szFromSection)
{
	KeyItor k_pos;
	t_Section* pSection;
...
// the new value. If it does not locate the key, it will create a new key with
// the proper value and place it in the section requested.
bool CIniFile::CreateKey(t_Str szKey, t_Str szValue, t_Str szComment, t_Str szSection)
{
	bool bAutoKey = (m_Flags & AUTOCREATE_KEYS) == AUTOCREATE_KEYS;
	bool bReturn  = false;
...
bool CIniFile::CreateSection(t_Str szSection, t_Str szComment)
{
	t_Section* pSection = GetSection(szSection);

	if ( pSection )
	{
		Report(E_INFO, "[CIniFile::CreateSection] Section <%s> allready exists. Aborting.", szSection.c_str());
		return false;
	}
...
bool CIniFile::CreateSection(t_Str szSection, t_Str szComment, KeyList Keys)
{
...
t_Key*	CIniFile::GetKey(t_Str szKey, t_Str szSection)
{
	KeyItor k_pos;
	t_Section* pSection;
...
t_Section* CIniFile::GetSection(t_Str szSection)
{
	SectionItor s_pos;
...
t_Str CIniFile::CommentStr(t_Str szComment)
{
