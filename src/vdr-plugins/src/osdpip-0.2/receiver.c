...
#if VDRVERSNUM < 10716
cReceiver(Channel->Ca(), 0, Channel->Vpid()), player(pipplayer), channel(Channel)
#else
cReceiver(Channel->GetChannelID(), 0, Channel->Vpid()), player(pipplayer), channel(Channel)
#endif
//,player(pipplayer), channel(Channel)
{
#if VDRVERSNUM < 10716
...
{
#if VDRVERSNUM > 10720 
   /* starting and stopping PiP() destroys SWDecoder in reelbox plugin
      and so it loose the height of prev pip window ==> bug #470  <- so only use it for reelbox-pi...*/
    if(cReelBoxBase::Instance() && !cReelBoxBase::Instance()->NeedPipStartInActivate()) return;
    if (On)
    {
        //printf("------------------StartPip------------------------\n");
