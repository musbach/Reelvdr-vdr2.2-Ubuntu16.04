...
//	ringBuffer=new cRingBufferLinear(MEGABYTE(2),TS_SIZE,true,"FFdecsa-TS");
//	ringBuffer->SetTimeouts(100,100);
	buff=new cRingBufferLinear(MEGABYTE(2),TS_SIZE,true,"FFdecsa-TS");
	buff->SetTimeouts(100,100);
	softcsa=true;
	delivered = false;
	decsa = 0;
#if APIVERSNUM >= 10500
	printf("Creating ci adapter\n");
	cam=new cCam(this,n);
	...
	cam=new cCam(this,n);
#endif
	PRINTSCF(L_GEN_INFO,"Using software decryption on card %d",n);
...
void cScMcliDevice::DumpAV7110(void)
{
}
...
// ----------------------------------------------------------------------- 
bool cScMcliDevice::ScActive(void)
{
#if APIVERSNUM >= 10500
...
bool cScMcliDevice::GetTSPacket(uchar *&Data)
{
  int Count=0;
  if(delivered) { buff->Del(TS_SIZE); delivered=false; }
  uchar *p=buff->Get(Count);
  if(p && Count>=TS_SIZE) {
    if(*p!=TS_SYNC_BYTE) {
...
int cScMcliDevice::GetTSPackets (uchar *Data, int count)
{
  int done = 0;
  count=1;
  while ( done < count ) {
    ...
    if (temp != 0 ) {
      memcpy(Data + (done * TS_SIZE), temp, TS_SIZE);
      done++;
    } else break;
  }
  return done * TS_SIZE; 
...
bool cScMcliDevice::SoftCSA(bool live)
{
  return true;
}
...
//	DBSCG(">>>>>cScMcliDevice::HandleTsData %p %d CardIndex %d\n", buffer, len, CardIndex());
  int olen=len;
  if (cMcliDevice::HasInput() == false )
    return len;
  cMcliDevice::HandleTsData(buffer,len);
//  tsMutex.Lock();
  cMutexLock lock(&dataMutex);
  if ( buff ) {
    size_t avail = buff->Free();
    if ( avail < len ) {
      len = avail;
    } 
    if ( len ) {
...
//	for(int i=0;i<MAXDEVICES;i++) {
	for(int n=cDevice::NumDevices(); --n>=0;) {
		cScMcliDevice *test = dynamic_cast<cScMcliDevice *>(cDevice::GetDevice(n));
		if(test) test->EarlyShutdown();
	} // for
} // cScMcliDevice::Shutdown

#if APIVERSNUM >= 10501
bool cScMcliDevice::HasCi(void)
{
  return ciadapter;
}
#endif
