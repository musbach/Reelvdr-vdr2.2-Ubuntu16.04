...
:cOsdMenu(tr("SoftCAM"),25)
{
  ...
  for(int d=0; d<MAX_DVBDEVICES; d++) {
    int n=0;
    char *ks;
    do {
...
{
  if(AutoUpdate==0)
    PRINTSCF(L_GEN_WARN,"Keys updates (AU) are disabled.");
  for(int i=0; i<MAXSCCAPS; i++)
    if(ScCaps[i]>=16) {
      PRINTSCF(L_GEN_WARN,"ScCaps contains unusual value. Check your config! (You can ignore this message if you have more than 16 dvb cards in your system ;)");
      break;
      }

  PRINTSCF(L_CORE_LOAD,"** Plugin config:");
  PRINTSCF(L_CORE_LOAD,"** Key updates (AU) are %s (%sprestart)",AutoUpdate?(AutoUpdate==1?"enabled (active CAIDs)":"enabled (all CAIDs)"):"DISABLED",PrestartAU?"":"no ");
  PRINTSCF(L_CORE_LOAD,"** Local systems %stake priority over cached remote",LocalPriority?"":"DON'T ");
  PRINTSCF(L_CORE_LOAD,"** Concurrent FF recordings are %sallowed",ConcurrentFF?"":"NOT ");
  PRINTSCF(L_CORE_LOAD,"** %sorce transfermode with digital audio",ForceTransfer?"F":"DON'T f");
  PRINTSCF(L_CORE_LOAD,"** ECM cache is set to %s",EcmCache ? (EcmCache==1?"READ-ONLY":"DISABLED"):"enabled");
  LBSTART(L_CORE_LOAD);
...
bool cSoftCAM::Load(const char *cfgdir)
{
  ...
  if(Feature.KeyFile() && keys.Count()<1)
    PRINTSCF(L_GEN_ERROR,"no keys loaded for softcam!");
...
//    if(dev) dev->CaidsChanged_2();
//    if(dev) dev=&SoftCAM::CaidsChanged_2();
    }
}
...
bool cScDll::Load(void)
{
  ...
  PRINTSCF(L_CORE_DYN,"loading library: %s",base);
  if(!handle) {
    ...
    if(handle) {
      ...
      else PRINTSCF(L_GEN_ERROR,"dload: %s: SCAPI version doesn't match (plugin=%d, library=%d)",base,SCAPIVERS,!error?*libapivers:0);
      }
    else PRINTSCF(L_GEN_ERROR,"dload: %s: %s",base,dlerror());
    }
  return false;
}
...
cScDlls::~cScDlls()
{
  Clear();
  if(handle) dlclose(handle);
  PRINTSCF(L_CORE_DYN,"unload done");
}
...
bool cScDlls::Load(void)
{
  ...
  if(!dladdr((void *)&marker,&info)) {
    PRINTSCF(L_GEN_ERROR,"dladdr: %s",dlerror());
    return false;
    }
  ...
  if(!handle) {
    PRINTSCF(L_GEN_ERROR,"dlopen myself: %s",dlerror());
    return false;
    }
  ...
  PRINTSCF(L_CORE_DYN,"library path %sn",path);
...
const char *cScPlugin::MainMenuEntry(void) 
{
	return tr("SoftCAM");
}
...
cScPlugin::cScPlugin(void)
{
  ...
  ScOpts->Add(new cOptBool ("OddEven"      ,trNOOP("Activate OddEven Fix") ,&ScSetup.OddEven));
...
{
  bool succ = false;
  PRINTSCF(L_GEN_INFO,"SC version %s initializing (VDR %s)",ScVersion,VDRVERSION);
//  return dllSuccess && cScDvbDevice::Initialize();
  if (dllSuccess) {
    succ=cScDvbDevice::Initialize();
#ifdef USE_MCLI
    succ=true;
#endif    
  }
  return dllSuccess && succ;
}

bool cScPlugin::Start(void)
{
  PRINTSCF(L_GEN_INFO,"SC version %s starting (VDR %s)",ScVersion,VDRVERSION);
  if(APIVERSNUM<MINAPIVERSNUM) {
    PRINTSCF(L_GEN_ERROR,"SC plugin needs at least VDR API version %d.%d.%d",MIN_VERS,MIN_MAJOR,MIN_MINOR);
    return false;
    }
  if(sizeof(int)!=4) {
    PRINTSCF(L_GEN_ERROR,"compiled with 'int' as %d bit. Only supporting 32 bit.",(int)sizeof(int)*8);
    return false;
    }
  if(sizeof(long long)!=8) {
    PRINTSCF(L_GEN_ERROR,"compiled with 'long long' as %d bit. Only supporting 64 bit.",(int)sizeof(long long)*8);
    return false;
    }
...
void cScPlugin::Stop(void)
{
  ...
  LogStatsDown();
  cSoftCAM::Shutdown();
  PRINTSCF(L_GEN_DEBUG,"SC cleanup done");
}
...
bool cScPlugin::Service (const char *Id, void *Data) {
	if (Id && strcmp(Id, "OnNewMcliDevice-"MCLI_DEVICE_VERSION) == 0) {
		cMcliDevice **dev_ptr = (cMcliDevice **)Data;
	        cMcliDevice *dev = *dev_ptr;
/*		if(dev && *dev ) {
 			(*dev)->SetEnable (false);
			delete *dev;*/
       		if ( dev ) {
...
//		if(dev) *dev = new cScMcliDevice();
// 		Wait till sc has truly started
	        while ( keeper == 0 ) {
          		cCondWait::SleepMs(200);
        	}
        	cScMcliDevice *newdev = new cScMcliDevice();
        	*dev_ptr = newdev;
		return true;
	} // if
	return false;
