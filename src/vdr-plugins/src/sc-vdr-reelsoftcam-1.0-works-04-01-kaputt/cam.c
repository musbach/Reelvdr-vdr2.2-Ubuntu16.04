...
unsigned char *cCiFrame::Get(int &l)
{
  if(rb) {
    ...
    if(data) {
      if(c!=rb->Available()) PRINTSCF(L_CORE_CI,"Error ########### c=%d unequal Available=%d",c,rb->Available());
      int s=*((short *)data);
//      LDUMP(L_GEN_DEBUG,data,c,"cCiFrame::Get c=%d s=%d avail=%d data=%d -",c,s,rb->Available(),data);
//      LDUMP(L_GEN_DEBUG,data,c,"cCiFrame::Get        c=%d         s=%d         data=%d -",c,s,data);
      if(c>LEN_OFF) {
//        int s=*((short *)data);
        if(c>=s+LEN_OFF) {
          l=glen=s;
//          LDUMP(L_GEN_DEBUG,data,c,"cCiFrame::Get return c=%d s+LEN_OFF=%d data+LEN_OFF=%d -",c,s+LEN_OFF,data+LEN_OFF);
          return data+LEN_OFF;
        }
      }
      LDUMP(L_GEN_DEBUG,data,c,"cCiFrame::Get Clear  c=%d         s=%d         data=%d -",c,s,data);
//      LDUMP(L_GEN_DEBUG,data,c,"cCiFrame::Get c=%d avail=%d -",c,rb->Available());
      rb->Clear();
    }
  }
  return 0;
...
*/
    BuildCaids(true);
#ifdef UE_MCLI
    firstTime=true; // First time GetCaids() is called rebuild the cache
#else
    firstTime=false;
#endif
...
{
#ifdef USE_MCLI
  BuildCaids(firstTime ? true :false);
  firstTime=false;
#else
...
//      if(Buffer[2]!=0x80 || LOG(L_CORE_CIFULL) || s!=49 ) {
...
//         (hwciadapter ? cDevice.Ready():true);
  return (ciadapter   ? false:true) &&
         (hwciadapter ? false:true);
//  return true;
}
