...
bool cCardClientCommon::ParseKeyConfig(const char *config, int *num)
{
  ...
  if(sscanf(&config[*num],":%32[^:]%n",hexkey,num)==1) {
    *num+=startNum;
    PRINTSCF(L_CC_CORE,"%s: key=%s",name,hexkey);
    ...
    if(n!=(int)sizeof(binkey))
      PRINTSCF(L_CC_CAMD,"warning AES key not %d bytes long",(int)sizeof(binkey));
...
int cCardClientCommon::RecvMsg(unsigned char *data, int len, int to)
{
  ...
  if(n>0) {
    if(n&15) PRINTSCF(L_CC_CAMD,"AES crypted message length not a multiple of 16");
    Decrypt(data,n);
    }
  return n;
}
...
bool cCardClientCommon::Login(void)
{
  ...
  PRINTSCF(L_CC_LOGIN,"%s: connected to %s:%d (%s)",name,hostname,port,name);
  emmProcessing=false;

  unsigned char buff[128];
  if(conReply) {
    if(RecvMsg(buff,16)<0) {
      PRINTSCF(L_CC_CAMD,"bad connect reply");
      return false;
      }
    }
  ...
  PRINTSCF(L_CC_CAMD,"login user='%s' password=hidden version=%s",username,CCVERSION);
  if(!SendMsg(buff,32)) return false;

  if(emmAllowed && logReply) {
    PRINTSCF(L_CC_CAMD,"waiting for login reply ...");
    int r=RecvMsg(buff,emmReqLen);
    if(r>0) HandleEMMRequest(buff,r);
    }
  PRINTSCF(L_CC_LOGIN,"%s: login done",name);
  return true;
}
  ...
  if(emmAllowed && CanHandleEMM(caSys)) {
    ...
    if(MatchEMM(source)) {
      ...
      if(id>0 || emmAllowed>1) {
        ...
        //PRINTSCF(L_CC_CAMD,"%s: sending EMM for caid 0x%04X",name,caSys);
...
bool cCardClientCommon::ProcessECM(const cEcmInfo *ecm, const unsigned char *source, unsigned char *cw, int cardnum)
{
  ...
  if((so.Connected() || Login()) && (!emmProcessing || CanHandle(ecm->caId))) {
    ...
    if(SendMsg(buff,length+7)) {
        ...
        while((n=RecvMsg(buff,-32,0))>0) {
          if(n>=21 && buff[0]==2) {
            if(!CheckNull(buff+5,16)) {
              ...
              else PRINTSCF(L_CC_CAMD,"unexpected CW packet");
              }
            else {
              PRINTSCF(L_CC_ECM,"%s: server is unable to handle ECM",name);
              n=-1;
              break;
              }
            }
      ...
      if(!res && time.TimedOut()) PRINTSCF(L_CC_ECM,"%s: CW request timed out",name);
      exclusive=false;
      sleepCond.Broadcast();
      }
    }
  ...
  if(len>=emmReqLen && buff[0]==0 && !CheckNull(buff,len) && memcmp(buff,lastEmmReq,emmReqLen)) {
    ...
    switch(CAID>>8) {
      ...
      case 0x06:
        ...
        PRINTSCF(L_CC_LOGIN,"%s: CAID: %04x HexSerial: %02X%02X%02X, HexBase: %02X",name,CAID,buff[3],buff[4],buff[5],buff[6]);
        PRINTSCF(L_CC_LOGIN,"%s: Provider00: %02X%02X%02X, Provider10: %02X%02X%02X",name,buff[7],buff[8],buff[9],buff[10],buff[11],buff[12]);
        if(!emmAllowed) PRINTSCF(L_CC_EMM,"%s: EMM disabled from config",name);
        emmProcessing=true;
        break;
      }
    }
...
bool cCardClientCamd35::ParseUserConfig(const char *config, int *num)
{
  ...
  if(sscanf(&config[*num],":%32[^:]:%32[^:]%n",username,password,num)==2) {
    *num+=startNum;
    ...
    PRINTSCF(L_CC_CORE,"%s: username=%s password=%s ucrc=%08x",name,username,password,ucrc);;
  ...
  if((unsigned int)n>=HDSIZE(cb)) {
    if(cb->ucrc==ucrc) {
      ...
      if((unsigned int)n<cb->udp_header.len+HDSIZE(cb))
        PRINTSCF(L_CC_CAMD35,"packet length doesn't match data length");
      else if(cb->udp_header.crc!=bswap_32(crc32_le(0,&cb->data[0],cb->udp_header.len)))
        PRINTSCF(L_CC_CAMD35,"data crc failed");
      else {
        HEXDUMP(L_CC_CAMDEXTR,cb,n,"recv:");
        return n;
        }
      }
    else PRINTSCF(L_CC_CAMD35,"wrong ucrc: got %08x, want %08x",cb->ucrc,ucrc);
    }
  else PRINTSCF(L_CC_CAMD35,"short packet received");
  return -1;
}
...
void cCardClientCamd35::HandleEMMRequest(const struct CmdBlock *cb)
{
  ...
  if(c!=lastEmmReq) {
    ...
    if(cb->udp_header.cmd==0x02 && cb->udp_header.len>=9) {
      ...
      Dx[3]=req->D3;
      char str[20];
      PRINTSCF(L_CC_LOGIN,"%s: CAID: %04x HexSerial: %s, HexBase: %02X (D0-%d D2-%d D3-%d)\n",
             name,Caids[0],HexStr(str,((cCardIrdeto *)card)->hexSer,3),((cCardIrdeto *)card)->hexBase,
             Dx[0],Dx[2],Dx[3]);
      if(!emmProcessing || emmCmd06) PRINTSCF(L_CC_CAMD35,"got cmd 02, doing cmd02 EMM");
      emmCmd06=false;
      }
...
//        if(map & (1<<i)) {    //XXX interpretation of provMap
      ...
      if(!emmProcessing || !emmCmd06) PRINTSCF(L_CC_CAMD35,"got cmd 05, doing cmd06 EMM");
      emmCmd06=true;
      }
    else return;
...
bool cCardClientCamd35::ProcessEMM(int caSys, const unsigned char *data)
{
  if(emmProcessing && emmAllowed) {
    cMutexLock lock(this);
    PRINTSCF(L_CC_CAMDEXTR,"got EMM caSys=%.4x",caSys);
    if(CanHandleEMM(caSys)) {
      ...
      if(MatchAndAssemble(&ad,&upd,&p)) {
        PRINTSCF(L_CC_CAMDEXTR,"EMM matched upd=%d provId=%.4llx",upd,p ? p->ProvId() : 0);
        while((data=ad.Assembled())) {
          ...
          if(Dx[upd]) {
            ...
            if(len+off<=255) {
              ...
              else PRINTSCF(L_CC_CAMDEXTR,"not send, already in cache");
              }
            else PRINTSCF(L_CC_EMM,"%s: EMM length %d > 255, not supported by UDP protocol",name,len+off);
            }
          else PRINTSCF(L_CC_CAMDEXTR,"dropped, updtype %d blocked",upd);
          }
        return true;
        }
      else PRINTSCF(L_CC_CAMDEXTR,"dropped, doesn't match card data");
      }
    else PRINTSCF(L_CC_CAMDEXTR,"dropped, doesn't want caSys %.4x",caSys);
    }
  return false;
}
...
bool cCardClientCamd35::ProcessECM(const cEcmInfo *ecm, const unsigned char *data, unsigned char *cw, int cardnum)
{
  ...
  if(length<=512) {
    ...
    while((n=RecvBlock(cb,sizeof(buff),0))>0) {
      if(cb->udp_header.cmd==0x01 || cb->udp_header.cmd==0x44)
        PRINTSCF(L_CC_CAMD35,"unexpected CW answer on flush");
      else
        HandleEMMRequest(cb);
      }
      ...
      do {
        ...
        while((n=RecvBlock(cb,sizeof(buff),0))>0) {
          if(cb->udp_header.cmd==0x01) {
            ...
            else PRINTSCF(L_CC_CAMD35,"unexpected/bad CW packet");
            }
  ...
  else PRINTSCF(L_CC_ECM,"%s: ECM length %d > 255, not supported by UDP protocol",name,length);
  return res;
}
