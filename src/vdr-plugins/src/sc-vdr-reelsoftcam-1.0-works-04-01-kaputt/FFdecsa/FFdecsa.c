...
//icc craziness  i=(int)&pad1;//////////align!!! FIXME
  ...
  do{ // find a new packet
    ...
    do{ // handle this packet
      xc0=pkt[3]&0xc0;
      DBSCG(fprintf(stderr,"   exam pkt=%p, xc0=%02x, can_adv=%i\n",pkt,xc0,can_advance));
      if(xc0==0x00){
        DBSCG(fprintf(stderr,"skip clear pkt %p (can_advance is %i)\n",pkt,can_advance));
        advanced+=can_advance;
        stat_no_scramble++;
        break;
      }
      if(xc0==0x40){
        DBSCG(fprintf(stderr,"skip reserved pkt %p (can_advance is %i)\n",pkt,can_advance));
        advanced+=can_advance;
        stat_reserved++;
        break;
      }
        ...
        if(group_ev_od==ev_od){ // could be added to group
          ...
          if(pkt[3]&0x20){ // incomplete packet
            ...
            if(n==0){ // decrypted==encrypted!
              DBSCG(fprintf(stderr,"DECRYPTED MINI! (can_advance is %i)\n",can_advance));
              advanced+=can_advance;
              stat_decrypted_mini++;
          ...
          DBSCG(fprintf(stderr,"%2i: eo=%i pkt=%p len=%03i n=%2i residue=%i\n",grouped,ev_od,pkt,len,n,residue));
          grouped++;
          advanced+=can_advance;
          ...
          can_advance=0;
          DBSCG(fprintf(stderr,"skip pkt %p and can_advance set to 0\n",pkt));
  ...
  DBSCG(fprintf(stderr,"-- result: grouped %i pkts, advanced %i pkts\n",grouped,advanced));
  ...
  *clst2=NULL;
  ...
  DBSCG(fprintf(stderr,"PRESORTING\n"));
  for(i=0;i<grouped;i++){
    DBSCG(fprintf(stderr,"%2i of %2i: pkt=%p len=%03i n=%2i residue=%i\n",i,grouped,g_pkt[i],g_len[i],g_n[i],g_residue[i]));
    }
...
DBSCG(fprintf(stderr,"t23 after for =%i\n",t23));
    
    for(;tsmall>=0;tsmall--){
      if(g_n[tsmall]==23) break;
    }
DBSCG(fprintf(stderr,"tsmall after for =%i\n",tsmall));
    
    if(tsmall-t23<1) break;
    
DBSCG(fprintf(stderr,"swap t23=%i,tsmall=%i\n",t23,tsmall));

    g_swap(t23,tsmall);

    t23++;
    tsmall--;
DBSCG(fprintf(stderr,"new t23=%i,tsmall=%i\n\n",t23,tsmall));
  }
  DBSCG(fprintf(stderr,"packets with n=23, t23=%i   grouped=%i\n",t23,grouped));
  DBSCG(fprintf(stderr,"MIDSORTING\n"));
  for(i=0;i<grouped;i++){
    DBSCG(fprintf(stderr,"%2i of %2i: pkt=%p len=%03i n=%2i residue=%i\n",i,grouped,g_pkt[i],g_len[i],g_n[i],g_residue[i]));
    }
  ...
  DBSCG(fprintf(stderr,"POSTSORTING\n"));
  for(i=0;i<grouped;i++){
    DBSCG(fprintf(stderr,"%2i of %2i: pkt=%p len=%03i n=%2i residue=%i\n",i,grouped,g_pkt[i],g_len[i],g_n[i],g_residue[i]));
    }
  ...
  DBSCG(fprintf(stderr,"ALIVE\n"));
  for(i=0;i<=23;i++){
    DBSCG(fprintf(stderr,"alive%2i=%i\n",i,alive[i]));
    }
...
DBSCG(fprintf(stderr,"--- WARNING: you could gain speed by not initializing unused memory ---\n"));
#else
DBSCG(fprintf(stderr,"--- WARNING: DEBUGGING IS MORE DIFFICULT WHEN PROCESSING RANDOM DATA CHANGING AT EVERY RUN! ---\n"));
#endif

  for(g=0;g<grouped;g++){
    encp[g]=g_pkt[g];
    DBSCG(fprintf(stderr,"header[%i]=%p (%02x)\n",g,encp[g],*(encp[g])));
...
DBSCG(dump_mem("stream_out_ib ",ib+8*g,8,8));
...
#endif
DBSCG(dump_mem("after_stream_xor_ib ",ib+8*g,8,8));
    }
    // alive packets: decrypt data
    for(g=0;g<alive[iter];g++){
DBSCG(dump_mem("before_ib_decrypt_data ",encp[g],8,8));
#ifdef RBMINI
      XOR_4_BY(encp[g],ib+8*g,block_out+8*g);
      XOR_4_BY(encp[g]+4,ib+8*g+4,block_out+8*g+4);
#else
      XOR_8_BY(encp[g],ib+8*g,block_out+8*g);
#endif
DBSCG(dump_mem("after_ib_decrypt_data ",encp[g],8,8));
    }
    // just dead packets: write decrypted data
    for(g=alive[iter];g<alive[iter-1];g++){
DBSCG(dump_mem("jd_before_ib_decrypt_data ",encp[g],8,8));
      COPY_8_BY(encp[g],block_out+8*g);
DBSCG(dump_mem("jd_after_ib_decrypt_data ",encp[g],8,8));
    }
    // just dead packets: decrypt residue
    for(g=alive[iter];g<alive[iter-1];g++){
DBSCG(dump_mem("jd_before_decrypt_residue ",encp[g]+8,g_residue[g],g_residue[g]));
      FFTABLEOUTXORNBY(g_residue[g],encp[g]+8,stream_out,g);
DBSCG(dump_mem("jd_after_decrypt_residue ",encp[g]+8,g_residue[g],g_residue[g]));
    }
    // alive packets: pointers++
    for(g=0;g<alive[iter];g++) encp[g]+=8;
  };
  // ITER N
DBSCG(fprintf(stderr,">>>>>ITER 23\n"));
  iter=23;
  // calc block
  block_decypher_group(k->kkmulti,ib,block_out,alive[iter-1]);
DBSCG(dump_mem("23BLO_ib ",block_out,8*alive[iter-1],8));
  // just dead packets: write decrypted data
  for(g=alive[iter];g<alive[iter-1];g++){
DBSCG(dump_mem("23jd_before_ib_decrypt_data ",encp[g],8,8));
    COPY_8_BY(encp[g],block_out+8*g);
DBSCG(dump_mem("23jd_after_ib_decrypt_data ",encp[g],8,8));
  }
  // no residue possible
  // so do nothing

  DBSCG(fprintf(stderr,"returning advanced=%i\n",advanced));
