...
bool cMenuSearchResultsItem::Update(bool Force)
{
   ...
   cTimer* timer = NULL;
   eTimerMatch tM;
   tM = (eTimerMatch) timerMatch;
   if (event) timer = Timers.GetMatch(event, &tM);
   timerMatch = (int) tM;
   ...
   if (!isUTF8)
   ...
   else
     {
      ...
      buffer = strreplaceall(buffer, '|', '\t');

      if (buffer && !strcasestr(buffer, "%subtitle%")  && cTemplFile::GetTemplateByName("MenuFavorites") != menuTemplate)
...
cMenuSearchResultsItem::cMenuSearchResultsItem(cRecording *Recording)
{
   ...
   cString s = cString::sprintf("%s\t%s", tstr, *name);           
           
   printf("cMenuSearchResultsItem: Title'%s'\n", *s);
   SetText(s);
#else
...
eOSState cMenuSearchResults::ShowSummary()
{
   if(Count() == 1)
   {
       // checks if the 1 item is a text "- no broadcast found -" which is counted with => Count() = 1
       cOsdItem *item = dynamic_cast<cOsdItem*>(Get(Current()));
       if(item)
           return osContinue;
   }
...
bool cMenuSearchResultsForSearch::BuildList()
{
   ...
   if (Count())
   {
       if (EPGSearchConfig.showChannelNr == 1)
         SetCols(23, 6, 5, 5, 4);
       /*
       //RC: is set in templatefile.c ~l. 290
       else
         SetCols(11, 5, 5, 4);
       */
       SetCurrent(Get(0));
   ...
   else
   {
       ...
       Add(new cOsdItem(tr("no broadcast found")));

   }

   SetHelpKeys(true);

   cString szTitle = cString::sprintf("%s (%d) - %s",  tr("Search results"),
                                      Empty_Result ? 0 : Count(), searchExt->search);
   SetTitle(szTitle);
...
void cMenuSearchResultsForSearch::SetHelpKeys(bool Force)
{
   ...
   if (toggleKeys==0)
      SetHelp((EPGSearchConfig.redkeymode==0?(hasTimer?trVDR("Button$Timer"):trVDR("Button$Record")):tr("Button$Functions")),
	      m_bSort? tr("Button$by channel"):tr("Button$by time"),
	      modeYellow==showTitleEpisode?tr("Button$Episode"):tr("Button$Title"),
      ...
      SetHelp((EPGSearchConfig.redkeymode==1?(hasTimer?trVDR("Button$Timer"):trVDR("Button$Record")):tr("Button$Functions")),
	      m_bSort? tr("Button$by channel"):tr("Button$by time"),
	      modeYellow==showTitleEpisode?tr("Button$Episode"):tr("Button$Title"),
	      ButtonBlue[(int)nextModeBlue]
#if REELVDR && APIVERSNUM >= 10718
...
void cMenuSearchResultsForBlacklist::SetHelpKeys(bool Force)
{
   ...
   if (toggleKeys==0)
      SetHelp((EPGSearchConfig.redkeymode==0?(hasTimer?trVDR("Button$Timer"):trVDR("Button$Record")):trVDR("Button$Functions")), m_bSort? tr("Button$by channel"):tr("Button$by time"), modeYellow==showTitleEpisode?tr("Button$Episode"):tr("Button$Title"), NULL);
   else
      SetHelp((EPGSearchConfig.redkeymode==1?(hasTimer?trVDR("Button$Timer"):trVDR("Button$Record")):trVDR("Button$Functions")), m_bSort? tr("Button$by channel"):tr("Button$by time"), modeYellow==showTitleEpisode?tr("Button$Episode"):tr("Button$Title"), NULL);
     }
}
...
bool cMenuSearchResultsForRecs::BuildList()
{
...
#if REELVDR
   // show only 'Date' and 'Title' in this menu
   SetCols(9); 
#endif
   ...
   if (num==0) {
       Add(new cOsdItem(tr("- no recording found -")));
   }
...
#else
#if REELVDR
         cReplayControl::SetRecording(recording->FileName(),Title());
#else
...
void cMenuSearchResultsForList::SetHelpKeys(bool Force)
{
   ...
   if (NewHelpKeys != helpKeys || Force)
     {
   if (toggleKeys==0)
      SetHelp((EPGSearchConfig.redkeymode==0?(hasTimer?trVDR("Button$Timer"):trVDR("Button$Record")):trVDR("Button$Functions")), m_bSort? tr("Button$by channel"):tr("Button$by time"), modeYellow==showTitleEpisode?tr("Button$Episode"):tr("Button$Title"), ButtonBlue[0]);
   else
      SetHelp((EPGSearchConfig.redkeymode==1?(hasTimer?trVDR("Button$Timer"):trVDR("Button$Record")):trVDR("Button$Functions")), m_bSort? tr("Button$by channel"):tr("Button$by time"), modeYellow==showTitleEpisode?tr("Button$Episode"):tr("Button$Title"), ButtonBlue[0]);
     }
}
