...
void cMenuEventSearch::Set()
{
   ...
   if (event)
   {
      ...
      if (channel)
      {
         ...
         //can switch to another channel OR to a future program event
         canSwitch = channel->Number() != cDevice::CurrentChannel() ||
          event->StartTime() > time(NULL);
      }
      ...
      {
         if (eventObjPrev && eventObjPrev->Event() && Setup.ExpertNavi)
             szGreen = strdup(GETTIMESTRING(eventObjPrev->Event()));
         if (eventObjNext && eventObjNext->Event() && Setup.ExpertNavi)
             szYellow = strdup(GETTIMESTRING(eventObjNext->Event()));
...
eOSState cMenuEventSearch::ProcessKey(eKeys Key)
{
   ...
   if (state == osUnknown) {
      switch (Key) {
         ...
         {
            cEventObj* eventObjPrev = GetPrev(event);
            if (eventObjPrev && eventObjPrev->Event() && (surfMode == SurfModeUnknown || Setup.ExpertNavi))
            {
         ...
         {
            cEventObj* eventObjNext = GetNext(event);
            if (eventObjNext && eventObjNext->Event() && (surfMode == SurfModeUnknown || Setup.ExpertNavi))
            {
	 ...
	 case kRed:
         {
	    return AddSubMenu(new cMenuMyEditTimer(new cTimer(event), false, event, Channels.GetByChannelID(event->ChannelID()),  &timerCreated_));
         }
	 break;
      ...
      case kBlue:
          if (event)
          {
              ...
              // if event is in future, add switchtimer (even to current channel)
              if (event->StartTime() > time(NULL))
              { // start time is in the future, and not current channel
                  ...
                  state = osContinue; // do nothing if in current channel
