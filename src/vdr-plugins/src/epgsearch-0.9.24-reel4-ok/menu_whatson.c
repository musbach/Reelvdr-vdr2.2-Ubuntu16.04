...
bool cMenuMyScheduleItem::Update(bool Force)
{
   ...
   r[0] = event && event->IsRunning() ? '*' : ' ';
      ...
      if(event && timer && timer->Recording())
         recording=true;
      else
         recording=false;
...
bool cMenuMyScheduleItem::UpdateRecordingState()
{
    bool result=false;
    if(event)
    {
        int TimerMatchTmp = tmNone;
        cTimer* timer = NULL;
	eTimerMatch TMT;
	TMT = (eTimerMatch) TimerMatchTmp;
        timer = Timers.GetMatch(event, &TMT);
	TimerMatchTmp = (int) TMT;
        if(timer && (TimerMatchTmp == tmFull))
            if(recording != timer->Recording())
                result = Update(true); // Force update this item
    }
    return result;
}
...
void cMenuWhatsOnSearch::LoadSchedules()
{
   ...
   if (shiftTime != 0) {
      if (currentShowMode == showNow || currentShowMode == showNext)
         ...
         if (seekTime < now) seekTime += HOURS2SECS(24);
      }
  ...
  szTitle = cString::sprintf("%s - %02d:%02d", tr("Overview"), tm_seek.tm_hour, tm_seek.tm_min);
      ...
      if (mode) {
         seekTime = GetTimeT(mode->GetTime());
         if (seekTime < now && currentShowMode != showNow && currentShowMode != showNext) {
  ...
  float secs2 = ((float)((1000000 * now2.tv_sec + now2.tv_usec) - (now1.tv_sec * 1000000 + now1.tv_usec))) / 1000000;
  PRINTF("Time spent in %s: %f\n", __PRETTY_FUNCTION__, secs2);
#endif
}
...
void cMenuWhatsOnSearch::CreateShowModes()
{
   ...
   for(int i=showUserMode1; i<showModeMax; i++) {
      ...
      if (SeekTime - now > HOURS2SECS(SKIPHOURS))
         continue;
      cShowMode* Mode = new cShowMode((showMode)i, EPGSearchConfig.ShowModes[i].GetDescription(), 1, EPGSearchConfig.ShowModes[i].GetTime(), SeekTime);
      showModes.Add(Mode);
   }
	 ...
	 tm = (int) TM;
      ...
      if (EPGSearchConfig.onePressTimerCreation == 0 || t || !item->event || (!t && item->event && item->event->StartTime() - (Setup.MarginStart+2) * 60 < time(NULL)))
      {
         ...
         if (EPGSearchConfig.useVDRTimerEditMenu)
            ...
            return AddSubMenu(new cMenuMyEditTimer(timer, !t, item->event, item->channel, NULL));
      }
      else
      {
         string fullaux = "";
...
bool cMenuWhatsOnSearch::UpdateRecordingState()
{
    bool result = false;
    for(cOsdItem *item=First(); item; item=Next(item))
    {
...
void cMenuWhatsOnSearch::SetHelpKeys(bool Force)
{
  ...
  if (toggleKeys==0)
  {
      SetHelp(hasTimer?trVDR("Button$Timer"):trVDR("Button$Record"),
                                          szButtonGreen,
                                          tr("Button$Schedule"),
              EPGSearchConfig.bluekeymode==0?trVDR("Button$Switch"):tr("Button$Functions"));
  }
  else
  {
      SetHelp((hasTimer?trVDR("Button$Timer"):trVDR("Button$Record")),
