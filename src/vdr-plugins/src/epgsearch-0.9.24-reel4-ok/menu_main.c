...
:cOsdMenu("", GetTab(1), GetTab(2), GetTab(3), GetTab(4), GetTab(5))
{
    ...
    }  // TB: reelvdr doesn't call Display() of this (and only of this) plugin - the only way I found to draw every menu of epgsearch just once
        else Display();
}
...
#if REELVDR
void cMenuSearchMain::Display()
{
    cOsdMenu::Display();

    // do not overdraw submenus
    if (!HasSubMenu())
        ShowSideNote();

}

void cMenuSearchMain::ShowSideNote()
{
    cMenuMyScheduleItem* item = dynamic_cast<cMenuMyScheduleItem*> (Get(Current()));
...
/*
** * @brief SameDay checks if given two time_t variables are in the same day
** * @param d1 time_t object
** * @param d2 time_t object
** * @return  true if day, month and year of the given time_t objects match
** */
bool IsSameDay(time_t d1, time_t d2)
{
    struct tm tm_d1, tm_d2;
    struct tm *tm1 = localtime_r(&d1, &tm_d1);
    struct tm *tm2 = localtime_r(&d2, &tm_d2);
...
{
    Clear();
    //asprintf(&buffer, "%s - %s", tr("Schedule"), Channel->Name()); // TB: orig
    //asprintf(&buffer, "%s %s", tr("EPG -"), Channel->Name());      // TB: reel
    //cString buffer = cString::sprintf("%s - %s", trVDR("Schedule"), Channel->Name());
    cString buffer = cString::sprintf("%s %s", trVDR("EPG -"), Channel->Name());
   ...
   if (schedules) {
	...
	if (Schedule && Schedule->Events()->First())
	{
		...
		if (Event->EndTime() > now || (shiftTime==0 && Event == PresentEvent))
		{
			...
			bool isNewDay = !IsSameDay(EventDate, lastEventDate); /*day change*/ 
			bool isToday  = IsSameDay(EventDate, now);
			bool nonEmptyMenu = (Count()>0);
			
			/* show the "day" string:
			 *   - if the day has changed AND
			 *   - if the day is not Today OR
			 *   - if the day is Today and there are already items in the menu
			 *            so label today's event also
			 */
			if (isNewDay && (!isToday || (isToday && nonEmptyMenu)) )
			{
			    char* szSep = NULL;
                            //std::string cmd = std::string(tr("° EPG for :")) + " %s";
                            std::string cmd = "%s";
			    asprintf(&szSep, cmd.c_str(), GETDATESTRING(Event));
...
#if REELVDR
    if (Count()==0) {
        AddFloatingText(tr("No EPG data available"), 50);
    }
#endif
}
...
bool cMenuSearchMain::UpdateRecordingState()
{
    bool result = false;
    for(cOsdItem *item=First(); item; item=Next(item))
    {
        if(item->Selectable() && ((cMenuMyScheduleItem *)item)->UpdateRecordingState())
            result=true;
    }
    return result;
}
...
eOSState cMenuSearchMain::Record(void)
{
  ...
  if (item) {
	  ...
	  if (timer)
		  ...
		  return AddSubMenu(new cMenuMyEditTimer(timer, false, item->event, item->channel, NULL));
          }
      }
...
void cMenuSearchMain::SetHelpKeys(bool Force)
{
    ...
    if (toggleKeys==0)
    {
       SetHelp(hasTimer?trVDR("Button$Timer"):trVDR("Button$Record"), trVDR("Button$Now"), trVDR("Button$Next"), EPGSearchConfig.bluekeymode==0?trVDR("Button$Switch"):tr("Button$Functions"));
    }
    else
    {
       ...
       SetHelp(hasTimer?tr("Button$Timer"):trVDR("Button$Record"),
            (EPGSearchConfig.toggleGreenYellow==0?trVDR("Button$Now"):szGreenToggled),
            (EPGSearchConfig.toggleGreenYellow==0?trVDR("Button$Next"):szYellowToggled),
             EPGSearchConfig.bluekeymode==1?trVDR("Button$Switch"):tr("Button$Functions"));
...
#if REELVDR
   if (!HasSubMenu() && Key != kNone && state != osUnknown)
       ShowSideNote();
#endif /* REELVDR */
...
*/
	 ...
	 case kBlue:
		 ...
		 return Switch();
             {
                cMenuMyScheduleItem *mi = dynamic_cast<cMenuMyScheduleItem *>(Get(Current()));
                if (mi)
                {
                  if (mi->event)
                  {
...
#if REELVDR
                return osContinue; // do not allow vdr to handle colorkeys when menu is open
#endif
             }
	     break;
    case kInfo:
  ...
  if (!HasSubMenu()) {
	...
	// when switch from the other menus to the schedule, try to keep the same time
...
#ifdef USEMYSQL
      if(((Setup.ReelboxModeTemp == eModeClient) || (Setup.ReelboxModeTemp == eModeServer)) && !HasSubMenu() && Update()) // Need to update everytime if DB is used
      {
        Display();
        SetHelpKeys();
      }
      ...
      // Update if any recording state of items changed
