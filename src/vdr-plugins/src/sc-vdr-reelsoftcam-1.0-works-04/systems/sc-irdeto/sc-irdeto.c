...
bool cSmartCardDataIrdeto::Parse(const char *line)
{
  ...
  if(*line=='[') { // parse acs & caid
    ...
    if(GetHex(line,buff,2)!=2) {
      PRINTSCF(L_CORE_LOAD,"smartcarddatairdeto: format error: acs");
      return false;
      }
    ...
    if(*line=='/') {
      line++;
      if(GetHex(line,buff,2)!=2) {
        PRINTSCF(L_CORE_LOAD,"smartcarddatairdeto: format error: caid");
        return false;
        }
    ...
    if(!*line==']') {
      PRINTSCF(L_CORE_LOAD,"smartcarddatairdeto: format error: closing ]");
      return false;
      }
    line++;
    }
  ...
  if((l=GetHex(line,buff,sizeof(buff),false))<=0) {
    PRINTSCF(L_CORE_LOAD,"smartcarddatairdeto: format error: mod");
    return false;
    }
  BN_complement(buff,l,mod);
  if((l=GetHex(line,buff,sizeof(buff),false))<=0) {
    PRINTSCF(L_CORE_LOAD,"smartcarddatairdeto: format error: exp");
    return false;
    }
...
bool cSmartCardIrdeto::Init(void)
{
  ...
  if(atr->histLen<6 || memcmp(atr->hist,"IRDETO",6)) {
    PRINTSCF(L_SC_INIT,"doesn't looks like a Irdeto/Beta card");
    return false;
    }
  ...
  if((r=DoCmd(getCountryCode,0x0000))<=0 || !Status() || r<16) {
    PRINTSCF(L_SC_ERROR,"country code error");
    return false;
    }
  ...
  PRINTSCF(L_SC_INIT,"ACS Version %04x, CAID %04x, CoCo %s",ACS,caId,coco);
  ...
  if((r=DoCmd(getAsciiSerial,0x0000))<=0 || !Status() || r<10) {
    PRINTSCF(L_SC_ERROR,"ASCII serial error");
    return false;
    }
  strn0cpy(asciiSerial,(char*)buff+8,sizeof(asciiSerial));
  PRINTSCF(L_SC_INIT,"ASCII serial %s",asciiSerial);

  static unsigned char getHexSerial[] = { 0x01,0x02,0x01,0x03,0x00,0x00,0xCC };
  if((r=DoCmd(getHexSerial,0x0000))<=0 || !Status() || r<25) {
    PRINTSCF(L_SC_ERROR,"hex serial error");
    return false;
    }
  int numProv=buff[18];
  SetCard(new cCardIrdeto(buff[23],&buff[20]));
  PRINTSCF(L_SC_INIT,"Providers: %d HEX Serial: %02X%02X%02X  HEX Base: %02X",numProv,buff[20],buff[21],buff[22],buff[23]);
  ...
  if((r=DoCmd(getCardFile,0x0000))<=0 || !Status() || r<73) {
    PRINTSCF(L_SC_ERROR,"cardfile2 error");
    return false;
    }
  memcpy(encr,buff+8,64);
  getCardFile[3]=0x03;
  if((r=DoCmd(getCardFile,0x0000))<=0 || !Status() || r<73) {
    PRINTSCF(L_SC_ERROR,"cardfile3 error");
    return false;
    }
  ...
  if(!doPlain) {
    ...
    if(!(entry=(cSmartCardDataIrdeto *)smartcards.FindCardData(&cd))) {
      PRINTSCF(L_GEN_WARN,"didn't find Irdeto card specific certificate, falling back to default");
      cSmartCardDataIrdeto cd(-1,-1);
      if(!(entry=(cSmartCardDataIrdeto *)smartcards.FindCardData(&cd))) {
        PRINTSCF(L_GEN_WARN,"didn't find default Irdeto certificate, please add one");
        if(ACS!=0x0384) return false;
        PRINTSCF(L_GEN_WARN,"trying pre-coded ACS 384 challenge. This mode is DEPRECATED. There ARE valid certificates for these cards available!");
        }
      }
  ...
  if(doPlain) {
    ...
    if(entry) {
      if(!SetupCardFiles(encr,sizeof(encr),entry->exp,entry->mod)) {
        PRINTSCF(L_SC_ERROR,"decrypting cardfiles failed. Probably bad certificate.");
        return false;
        }
      if(!EncryptCamMessage(encr+6,plain+6)) {
        PRINTSCF(L_SC_ERROR,"encrypting CAM message failed. Probably bad certificate.");
        return false;
        }
      ...
      if((r=DoCmd(plain,0x5800,0x0000))<=0 || !Status() || r<73) {
        PRINTSCF(L_SC_ERROR,"card didn't give a proper reply (buggy RSA unit?), trying to continue...");
        // non-fatal
        }
      if(r==73 && memcmp(encr+6,buff+8,64)) {
        PRINTSCF(L_SC_ERROR,"card failed on RSA check, trying to continue...");
        // non-fatal
        }
      }
    else {
      ...
      if(caId==0x1702) memcpy(encr+6,enc384cz,sizeof(enc384cz));
        ...
        PRINTSCF(L_GEN_WARN,"no pre-coded Irdeto camkey challenge for caid %04x",caId);
        return false;
        }
  ...
  if(r>0 && Status() && r>=9) {
    ...
    if(r>=17) {
      ...
      if(memcmp(HelperKey(plain+6),buff+8,8)) {
        PRINTSCF(L_SC_ERROR,"camkey challenge failed");
        return false;
        }
      }
    ...
    PRINTSCF(L_SC_ERROR,"camkey error");
    return false;
    }
  ...
  for(int i=0; i<numProv; i++) {
    ...
    if((r=DoCmd(getProvider,0x0000))>0 && Status() && r>=33) {
      ...
      PRINTSCF(L_SC_INIT,"provider %d with ProvBase 0x%02x ProvId 0x%02x%02x%02x",i,buff[8]&0x0f,buff[9],buff[10],buff[11]);
      ...
      for(int l=0; l<10; l++) {
        ...
        for(int k=0; k<buff[7]; k+=6) {
          ...
          if(chanId!=0x0000 && chanId!=0xFFFF) {
            ...
            PRINTSCF(L_SC_INIT,"ChanId 0x%04x Date 0x%04x %s Duration 0x%02x %s",chanId,date,sdate,durr,edate);
  ...
  if((r=DoCmd(getCountryCode2,0x0000))>0 && Status() && r>=32) {
    PRINTSCF(L_SC_INIT,"max ChID's %d,%d,%d,%d",buff[14],buff[15],buff[16],buff[17]);
    }
#endif
...
bool cSmartCardIrdeto::Update(int pid, int caid, const unsigned char *data)
{
  ...
  if(MatchEMM(data)) {
    ...
    else PRINTSCF(L_SC_ERROR,"addrlen %d > %d",len,ADDRLEN);
    }
  return false;
}
...
int cSmartCardIrdeto::DoCmd(unsigned char *cmd, int goodSB, int secGoodSB)
{
  ...
  if(r>0) {
    PRINTSCF(L_SC_ERROR,"recover time, waiting %d ms",r);
  ...
  if(r<=4) {
    recoverTime.Set();
    PRINTSCF(L_SC_ERROR,"setting %d ms recover time",RECOVER_TIME);
    }
  return r;
}
