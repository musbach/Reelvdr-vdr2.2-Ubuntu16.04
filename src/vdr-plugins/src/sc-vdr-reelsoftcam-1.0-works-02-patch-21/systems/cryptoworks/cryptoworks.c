...
void cCwDes::CwL2Des(unsigned char *data, const unsigned char *key22, unsigned char algo) const
{
  ...
  for(int i=1; i>=0; i--) {
    ...
    DESROUND(C,D,T);
    T^=L; L=R; R=T;
    PRINTSCF(L_SYS_VERBOSE,"round %d key L2: %08x %08x",i,C,D);
    }
  BYTE4_BE(data  ,L);
  BYTE4_BE(data+4,R);
}
...
void cCryptoworks::EncDec(unsigned char *data, const unsigned char *key22, unsigned char algo, int mode)
{
  ...
  PRINTSCF(L_SYS_VERBOSE,"algo %d",algo);
...
{
  PRINTSCF(L_SYS_VERBOSE,"sig start");
  int algo=data[0]&7;
  if(algo==7) algo=6;
  memset(sig,0,8);
  int j=0;
...
bool cCryptoworks::DecryptRSA(unsigned char *data, int len, unsigned char algo, const unsigned char *key22, BIGNUM *mod)
{
  ...
  for(int k=0; k<len; k+=32) {
    ...
    if(rsa.RSA(buf,buf,64,exp,mod,true)==0) {
      PRINTSCF(L_SYS_CRYPTO,"RSA failed");
      return false;
      }
    RotateBytes(buf,8);
...
bool cSystemCryptoworks::ProcessECM(const cEcmInfo *ecmInfo, unsigned char *data)
{
  ...
  if(data[ECM_NANO_LEN]!=len-ECM_NANO_START) {
    PRINTSCF(L_SYS_ECM,"invalid ECM structure");
    return false;
    }
  ...
  if(prov<0) {
    PRINTSCF(L_SYS_ECM,"provider ID not located in ECM");
    return false;
    }
  ...
  if(!(pk=keys.FindKey('W',ecmInfo->caId,KEYSET(prov,CCTYP,CCID),6))) {
    if(doLog) PRINTSCF(L_SYS_KEY,"missing %04X %02X CC key",ecmInfo->caId,prov);
    return false;
    }
  pk->Get(key+16);
    ...
    switch(data[i]) {
      case 0x85:
        ...
        if(!(pk=keys.FindKey('W',ecmInfo->caId,KEYSET(prov,0x31,keyid),16))) {
          if(doLog) PRINTSCF(L_SYS_KEY,"missing %04X %02X 31 %02X key",ecmInfo->caId,prov,keyid);
          return false;
          }
        pk->Get(key);
        cBN mod;
        if(!(pk=keys.FindKey('W',ecmInfo->caId,KEYSET(prov,0x10,0x00),64))) {
          if(doLog) PRINTSCF(L_SYS_KEY,"missing %04X %02X 10 00 key",ecmInfo->caId,prov);
          return false;
          }
        pk->Get(mod);
  ...
  if(!(pk=keys.FindKey('W',ecmInfo->caId,KEYSET(prov,0x20,keyid),16))) {
    if(doLog) PRINTSCF(L_SYS_KEY,"missing %04X %02X 20 %02X key",ecmInfo->caId,prov,keyid);
    return false;
    }
  pk->Get(key);
    ...
    switch(data[i]) {
      ...
      case 0xDF:
        if(memcmp(&data[i+2],sig,8)) {
          PRINTSCF(L_SYS_ECM,"signature failed in ECM");
          return false;
          }
        break;
      }
