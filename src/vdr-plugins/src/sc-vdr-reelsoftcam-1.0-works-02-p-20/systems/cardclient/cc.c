...
#define CC_CONF_FILE         "CCcam.cfg"
#define CC_CONFDIR           "/var/etc/"
...
static const struct LogModule lm_cc = {
  ...
  { "core","login","ecm","emm","camd","camd35","camd35extra","radegast",
    "newcamd","gbox","cccam","cccam2","cccam2data","cccam2shares","cccam2extra","C"}
  };
...
bool cCardClient::ParseStdConfig(const char *config, int *num)
{
  int n, emm=0;
  if(!num) num=&n;
  if (sscanf(config,"%63[^:]:%d:%d%n",hostname,&port,&emm,num) <3
   && sscanf(config,"%63s %d%n",hostname,&port,num)<2)
  return false;

 if (FileExists(std::string(CC_CONFDIR) + CC_CONF_FILE)) {
   emm=1;
    }
    ...
    do {
      ...
      *num+=start;
      if(emmMask[numCaid]==0x0000 && emmCaid[numCaid]!=0x0000)
        PRINTSCF(L_GEN_WARN,"CAID %04x MASK %04x in cardclient config doesn't match anything!",emmCaid[numCaid],emmMask[numCaid]);
      numCaid++;
...
bool cCardClient::SendMsg(const unsigned char *data, int len)
{
  ...
  if(so.Write(data,len)<0) {
    PRINTSCF(L_CC_CORE,"send error. reconnecting...");
    Logout();
    return false;
    }
  return true;
...
int cCardClient::RecvMsg(unsigned char *data, int len, int to)
{
  ...
  if(n<0) {
    ...
    PRINTSCF(L_CC_CORE,"recv error. reconnecting...");;
    Logout();
    }
  return n;
}
...
{
  PRINTSCF(L_CORE_DYN,"cardclients: registering cardclient %s",ccl->name);
  ccl->next=first;
  first=ccl;
}
    ...
    if(cc) {
      ...
      if(id==0 || (id>0 && cc->ProcessECM(ecm,data,cw,CardNum()))) {
        ...
        if(dur>2000) {
          ...
          PRINTSCF(L_CC_CORE,"%s: lagged cw %d ms (%s)",bb,dur,cc->Name());
          }
        char buff[32];
      ...
      if(id>0) {
        PRINTSCF(L_CC_CORE,"client %s (%s:%d) ECM failed (%d ms)",cc->Name(),cc->hostname,cc->port,(int)start.Elapsed());
        cc->msECM.Cache(id,false,cw);
        }
      if(id<0) {
	PRINTSCF(L_CC_CORE,"client %s (%s:%d) ECM already cached as failed",cc->Name(),cc->hostname,cc->port);
	}
      }
    if(!cc) PRINTSCF(L_CC_CORE,"cc-loop");
    oldcc=cc;
    cc=staticCcl.FindBySysId(ecm->caId,cc);
    if(cc && cc!=startCc) PRINTSCF(L_CC_CORE,"now trying client %s (%s:%d)",cc->Name(),cc->hostname,cc->port);
...
,cStructListPlain<cCardClient>("cardclient config",FileExists(std::string(CC_CONFDIR) + CC_CONF_FILE)?CC_CONF_FILE:CONF_FILE,SL_MISSINGOK|SL_VERBOSE|SL_NOPURGE)
{
  ...
  if(sscanf(line,"%31[^:]:%n",name,&num)==1) {
    ...
    if(ccl) {
      ...
      if(cc) {
        if(cc->Init(&line[num])) {
          Add(cc);
          PRINTSCF(L_CC_CORE,"client '%s' ready",cc->Name());
          if(cc->Immediate()) cc->Login();
          return true;
          }
        else {
          delete cc;
          PRINTSCF(L_GEN_ERROR,"init of cardclient '%s' failed",name);
          }
        }
      else PRINTSCF(L_GEN_ERROR,"failed to create cardclient '%s'",name);
      }
    else PRINTSCF(L_GEN_ERROR,"no client found for card server type '%s'",name);
    }
  return false;
}
