...
bool UpdateFavChannelNames()
{
    ...
    while (ch) {
        if (!ch->GroupSep()) {
            vdrCh = Channels.GetByChannelID(ch->GetChannelID(),false,false);
            if (vdrCh) {
...
cChannel* IsChannelInFavourites(const cChannel* ch)
{
    ...
    return favourites.GetByChannelID(ch->GetChannelID(),false,false);
}
...
bool SwitchToFavChannel(cChannel *favCh)
{
    ...
    cChannel *ch = Channels.GetByChannelID(favCh->GetChannelID(),false,false);
...
void cMenuFavourites::ShowSideNoteInfo()
{
    ...
    if (currItem)
    {
        ...
        if (Setup.PreviewVideos && channel)  {
            ...
            cChannel* vdrChannel = Channels.GetByChannelID(channel->GetChannelID(),false,false);
            if (vdrChannel) {
...
bool DelFavChannel(cChannel* channel)
{
    ...
    cChannel *tobeDel = favourites.GetByChannelID(channel->GetChannelID(),false,false);
