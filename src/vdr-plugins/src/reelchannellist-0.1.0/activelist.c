...
cChannel* NextAvailableChannel(cChannel *channel, int direction)
{
    ...
    if (foundNext && ch)
        return Channels.GetByChannelID(ch->GetChannelID(),false,false);
...
cChannel * NextAvailableChannel(cChannel* currVdrChannel, int direction)
{
    ...
    if (!channel || !(channel->GetChannelID() == currVdrChannel->GetChannelID()))
        channel = activeChannelList.GetByChannelID(currVdrChannel->GetChannelID(),false,false);
    ...
    return Channels.GetByChannelID(nextChannel->GetChannelID(),false,false);
}
...
int NextAvailableChannel(int currChannel, int direction)
{
    ...
    if (vdrChannel) {
        channel = activeChannelList.GetByChannelID(vdrChannel->GetChannelID(),false,false);
        ...
        if (channel) {/* vdr's channel found in active channel list */
            ...
            if (nextChannel) { /* next channel in active list */
                cChannel *vdrNextChannel = Channels.GetByChannelID(nextChannel->GetChannelID(),false,false);
