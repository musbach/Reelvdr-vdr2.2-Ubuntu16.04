...
void cMenuEditMyCaItem::Set(void)
{
	...
	if (cDevice::GetDevice(0)->CiHandler())
	    sprintf(s, "%s (%s)", (cDevice::GetDevice(0))->CiHandler()->GetCamName(1) ?
	                           (cDevice::GetDevice(0))->CiHandler()->GetCamName(1) : tr("No CAM at"), trVDR("upper slot"));
	else
#endif
	    sprintf(s, "%s", trVDR("upper slot"));
    }
	...
	if( cDevice::GetDevice(0)->CiHandler())
	    sprintf(s, "%s (%s)", (cDevice::GetDevice(0))->CiHandler()->GetCamName(0) ?
	                           (cDevice::GetDevice(0))->CiHandler()->GetCamName(0) : tr("No CAM at"), trVDR("lower slot"));
	else
#endif
	    sprintf(s, "%s", trVDR("lower slot"));
    }
