            ...
            if (sscanf (string1, "%[^-]-%i -%i -%i ", string3, &int1, &int2, &int3) == 4)
              if (sscanf (string2, "%[^-]-%i -%i -%i ", string3, &int1, &int2, &int3) == 4) {
                ...
                if (!found) {
                  ...
                  cChannel *OriginalChannel = Channels.GetByChannelID (OriginalChID, false,false);
                  if (!OriginalChannel)
                  ...
                  if (sscanf (string2, "%[^-]-%i -%i -%i ", string3, &int1, &int2, &int3) == 4) {
                    ...
                    cChannel *EquivChannel = Channels.GetByChannelID (EquivChID, false,false);
                    if (EquivChannel) {
  ...
  if ((MHW == 1) || (nChannels == 0)) { //prevents MHW2 from reading channels twice while waiting for themes on same filter
    ...
    if (nChannels > MAX_CHANNELS) {
      ...
      for (int i = 0; i < nChannels; i++) {
        ...
        cChannel *VC = Channels.GetByChannelID (channelID, true,false);
        bool IsFound = (VC);
        if (!IsFound) {  //look on other satpositions
          for (int i = 0; i < NumberOfAvailableSources; i++) {
            ...
            VC = Channels.GetByChannelID (channelID, true,false);
            IsFound = (VC);
    ...
    :  SI::EIT (Data, false)
{
  ...
  cChannel *channel = Channels.GetByChannelID (channelID, true,false);
  if (!channel)
      ...
      case SI::LinkageDescriptorTag: {
        ...
        if (ld->getLinkageType () == 0xB0) { // Premiere World
          ...
          if (hit) {
            ...
            cChannel *link = Channels.GetByChannelID (linkID,false,false);
