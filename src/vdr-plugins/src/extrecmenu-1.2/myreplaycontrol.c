...
eOSState myReplayControl::ProcessKey(eKeys Key)
{
  if(Key!=kNone)
  {
    ...
    else
    {
        ...
        {
#if APIVERSNUM > 20101
            dsyslog("extrecmenu: cut '%s' (VideoDir:'%s')", filename, cVideoDirectory::Name());

            Hide();
            const char* f = strstr(filename,cVideoDirectory::Name());
#else	  
            ...
            {
#if APIVERSNUM > 20101
                rfilename = f + strlen(cVideoDirectory::Name());
#else	      
...
#if APIVERSNUM > 20101
                esyslog("ERROR: attempting to cut rec:'%s' NOT under VideoDir:'%s'", filename, cVideoDirectory::Name());
#else		
