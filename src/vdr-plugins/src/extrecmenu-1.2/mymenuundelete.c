...
cMenuUndelete::cMenuUndelete(const char *Base, int Level, bool OpenSubMenus)
 :cOsdMenu(tr("Undelete Recordings")), lockFile(NULL) //
{
    if (Level==0) {
        SetFreeSpaceTitle();
#if APIVERSNUM > 20101
        lockFile = new cLockFile(cVideoDirectory::Name());
#else	
    ...
    {
	char buffer[50];
	snprintf(buffer, sizeof(buffer), "%s - %s", tr("Undelete Recordings"), Base);
...
void cMenuUndelete::Set(bool Refresh)
{
    ...
    if (Count() <=0)
	AddFloatingText(tr("No recordings to undelete!"), 45);
    if (Refresh)
        Display();
}
...
void cMenuUndelete::SetHelpKeys()
{
  ...
  if (NewHelpKeys != helpKeys) {
     switch (NewHelpKeys) {
       case 0:
       case 1: SetHelp(NULL); break;
       //case 1: SetHelp(tr("Button$Open")); break;
       case 2: SetHelp(tr("Button$Remove"), tr("Button$Undelete")); break;
       case 3: SetHelp(tr("Button$Remove"), tr("Button$Undelete"), trVDR("Button$Info"));
       }
...
eOSState cMenuUndelete::ProcessKey(eKeys Key)
{
  ...
  if ((state == osUnknown) && (DeletedRecordings.Count())) {
     switch (Key) {
       ...
       //case kRed:    return if (helpKeys ==  1) Open(); // open directory
       case kRed:    return Remove(); // remove instantly from disk
       case kGreen:  return Undelete();
       case kYellow: return Info();
       case kBlue:   if (helpKeys > 1)  return Play(); break;
