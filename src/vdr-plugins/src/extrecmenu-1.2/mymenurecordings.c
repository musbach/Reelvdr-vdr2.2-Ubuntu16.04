...
void myMenuRecordingInfo::Display(void)
{
  ...
  else
  {
    ...
    cChannel *chan=Channels.GetByChannelID(((cRecordingInfo*)recording->Info())->ChannelID(),false,false);
    if(chan)
...
eOSState myMenuRecordings::MoveRec()
{
  ...
  if(item)
  {
    ...
    if(item->IsDirectory())
    {
        ...
        if (base && item->Name())
#if APIVERSNUM > 20101
        snprintf(buffer,255, "%s/%s/%s", cVideoDirectory::Name(),base, item->Name());
#else	  
        snprintf(buffer,255, "%s/%s/%s", VideoDirectory,base, item->Name());
#endif	
        else if (base && !item->Name())
#if APIVERSNUM > 20101
	  snprintf(buffer,255, "%s/%s", cVideoDirectory::Name(),base);
#else
	  snprintf(buffer,255, "%s/%s", VideoDirectory,base);
#endif	  
        else if (!base && item->Name())
#if APIVERSNUM > 20101
        snprintf(buffer,255, "%s/%s", cVideoDirectory::Name(), item->Name());
#else	  
...
#if APIVERSNUM > 20101
        return AddSubMenu(new cSelectfileMenu(buffer, cVideoDirectory::Name()));
#else	  
...
#if APIVERSNUM > 20101
             return AddSubMenu(new cSelectfileMenu(recording->FileName(), cVideoDirectory::Name()));
#else	   
