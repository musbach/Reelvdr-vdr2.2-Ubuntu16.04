...
int myRecListItem::Compare(const cListObject &ListObject)const
{
  ...
  if (SortByName) {
      const cRecordingInfo *info = recording->Info();
#if APIVERSNUM > 20101
      bool s1IsDir = (strchr(filename+strlen(cVideoDirectory::Name())+1, '/') != strrchr(filename+strlen(cVideoDirectory::Name())+1, '/'));
      bool s2IsDir = (strchr(item->filename+strlen(cVideoDirectory::Name())+1, '/') != strrchr(item->filename+strlen(cVideoDirectory::Name())+1, '/'));
#else
      bool s1IsDir = (strchr(filename+strlen(VideoDirectory)+1, '/') != strrchr(filename+strlen(VideoDirectory)+1, '/'));
      bool s2IsDir = (strchr(item->filename+strlen(VideoDirectory)+1, '/') != strrchr(item->filename+strlen(VideoDirectory)+1, '/'));
#endif
...
#if APIVERSNUM > 20101
        char *t = StripEpisodeName(strdup(filename+strlen(cVideoDirectory::Name())));
#else	
...
#if APIVERSNUM > 20101
        s1 = StripEpisodeName(strdup(filename+strlen(cVideoDirectory::Name())));
#else	
        s1 = StripEpisodeName(strdup(filename+strlen(VideoDirectory)));
#endif	
        /** ignore the '%': doesn't matter if a recording is cut or not... */
...
#if APIVERSNUM > 20101
        char *t = StripEpisodeName(strdup(item->filename+strlen(cVideoDirectory::Name())));
#else	
...
#if APIVERSNUM > 20101
        s2 = StripEpisodeName(strdup(item->filename+strlen(cVideoDirectory::Name())));
#else	
        s2 = StripEpisodeName(strdup(item->filename+strlen(VideoDirectory)));
#endif        /** ignore the '%': doesn't matter if a recording is cut or not... */
...
#if APIVERSNUM > 20101
      if(editedfilename && cVideoDirectory::RemoveVideoFile(editedfilename) && MakeDirs(editedfilename,true))
#else      
...
#endif	
      {
        ...
        if(e)
        {
          if(!strcmp(e,".rec"))
          {
            strcpy(e,".del");
#if APIVERSNUM > 20101
	    cVideoDirectory::RemoveVideoFile(s);	    
#else
            RemoveVideoFile(s);
#endif	
          }
