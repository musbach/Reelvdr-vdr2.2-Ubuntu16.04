...
//      Add(new cMenuSwitchTimerItem(switchTimer));
      Add(new cMenuSwitchTimerItem(switchTimer,switchTimer->event));
  }
...
//    satellit hack following three lines
//      return AddSubMenu(new cMenuMyEditTimer(item->switchTimer, false));
      event=item->event;
      ctimer=item->switchTimer;
      return AddSubMenu(new cMenuMyEditTimer(&ctimer, false, event, NULL));
...
#endif
{
    ...
    switch((int)Key) // update the info buttons
    {
        default: break;
        case kUp:
        case kUp|k_Repeat:
