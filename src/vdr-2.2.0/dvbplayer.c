...
#ifdef USE_JUMPPLAY
:cThread("dvbplayer"), marksreload(FileName)
#else
:cThread("dvbplayer")
#endif /* JUMPPLAY */
{
...
printf("TrickSpeed %d -> %d\n", trickSpeed, trickSpeed + Increment);
  ...
  if (Speeds[nts] == 1) {
     ...
     if(DeviceNeedTrickSpeedDir())
        DeviceTrickSpeed(sp,(playDir == pdForward) ? sp : -sp);
     else
...
//            abs(Index - marks.First()->Position()) <= int(round(RESUMEBACKUP * framesPerSecond)))
        if (Setup.PlayJump && marksreload.First() &&
            abs(Index - marksreload.First()->Position()) <= int(round(RESUMEBACKUP * framesPerSecond)))
          Index = 0;
#endif /* JUMPPLAY */
...
//    int Index = marks.First()->Position();
  if (Setup.PlayJump && readIndex <= 0 && marksreload.First() && index) {
    int Index = marksreload.First()->Position();
    uint16_t FileNumber;
    off_t FileOffset;
