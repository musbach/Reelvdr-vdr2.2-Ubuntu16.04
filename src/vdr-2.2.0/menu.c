...
eOSState cMenuRecordingEdit::ApplyChanges(void)
{
  ...
  if (Modified) {
#ifdef REELVDR    
     cMenuRecordings::SetRecording(recording->FileName(), recording->Title()); // makes sure the Recordings menu will reposition to the renamed recording
...
cMenuRecordings::~cMenuRecordings()
{
  if (cMenuRecordingItem *ri = (cMenuRecordingItem *)Get(Current())) {
     if (!ri->IsDirectory())
#ifdef REELVDR
        SetRecording(ri->Recording()->FileName(), ri->Recording()->Title());
...
#ifdef REELVDR
void cMenuRecordings::SetRecording(const char *FileName, const char *Title)
...
{
  cRecording *recording = Recordings.GetByName(Item->Recording()->FileName());
  if (!recording)
     Skins.Message(mtError, tr("Error while accessing recording!"));
  return recording;
}
...
#ifdef REELVDR  // satellit to be checked
	  cReplayControl::SetRecording(fileName, event->Title());
...
#ifdef REELVDR // to be checked satellit
     cReplayControl::SetRecording(cLiveRecorder::FileName(),cReplayControl::title);
...
#ifdef REELVDR
//static char * cReplayControl::title = NULL;
...
#ifdef REELVDR
  cStatus::MsgReplaying(this, Recording.Name(), Recording.FileName() ? Recording.FileName() : *fileName, true);
#else
...
void cReplayControl::Stop(void)
{
  ...
  cDvbPlayerControl::Stop();
#ifdef REELVDR  
  cMenuRecordings::SetRecording(NULL,NULL); // make sure opening the Recordings menu navigates to the last replayed recording
...
{
#ifdef REELVDR
  return cReplayControl::currentReplayControl ? FileName() : "";
#else
...
#ifdef REELVDR
  SetRecording(NULL,NULL);  //to be checked satellit
  fileName = NULL;
#else
  fileName = NULL;
#endif
...
eOSState cReplayControl::ProcessKey(eKeys Key)
{
  ...
  switch (int(Key)) {
    ...
    case kPlay:
        ...
        Play(); break;    //         Same when using ShowTimed(3). Bug?
...
#endif /*REELVDR*/
#ifndef USE_LIVEBUFFER /*USE_LIVEBUFFER*/
...
#endif /* JUMPINGSECONDS */
#ifdef USE_LIEMIEXT /* LIEMIEXT */
#ifndef USE_JUMPINGSECONDS /* JUMPINGSECONDS */
    case k1|k_Repeat:
    ...
    case kNext:    if (lastSkipTimeout.TimedOut()) {
                   ...
                   SkipSeconds(lastSkipSeconds); break;
#endif /* LIEMIEXT */
#ifdef USE_LIVEBUFFER /*USE_LIVEBUFFER*/
