...
#include "debug.h"
#if REELVDR
#define YEAR_TO_SECOND(x) (3600*24*365*(x))

enum eActiveMode {
      active,
      standby,
      poweroff,   // meaning power off after current activities like timers etc.
      restart,
      maintenance,
      unknown
};
...
#ifdef REELVDR
      //DDD("activeTimeout: %ld retry: %d", activeTimeout-time(NULL), (int) retry);

      time_t At = AtTime?AtTime: time(NULL);
      /* If active timeout is one year or more in the past,
       * assume clock has changed and do not report user as inactive */
      if (At - activeTimeout > YEAR_TO_SECOND(1)) {
          if(activeTimeout) isyslog("clock changed ? ... not reporting user as inactive");
          return false;
      }

      return Setup.EnergySaveOn && activeTimeout && activeTimeout <= (AtTime ? AtTime : time(NULL));  // Energy Save off, no auto standby
#else
...
#ifdef REELVDR
  bool RequestNewMode(eActiveMode mode);
       ///< Changes member variable 'requestedMode' to given param
  bool ActOnActiveMode();
       ///< Assumes that 'requestedMode' is the new mode to enter and does the appropriate action
       ///< no checks to see if param mode is a new/different mode
  eActiveMode GetActiveMode() { return ActiveMode; };
#endif
  };

extern cShutdownHandler ShutdownHandler;
#ifdef REELVDR
extern cTimeMs standbyCountdown;
       ///< when in standby and Setup.EnergySaveOn is 1 wait for 
       ///< Setup.StandbyTimeout(in mins) and go to poweroff
