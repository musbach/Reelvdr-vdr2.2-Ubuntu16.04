...
cRecordingInfo::cRecordingInfo(const cChannel *Channel, const cEvent *Event)
{
  ...
  aux = NULL;
#ifdef REELVDR
  isHD = Channel ? Channel->Vpid() ? (Channel->Vtype()==0x1b) ? true : false : false : false;
  isCompressed = false;
#endif
...
cRecordingInfo::cRecordingInfo(const char *FileName)
{
  ...
  lifetime = MAXLIFETIME;
#ifdef REELVDR
  fileName = strdup(cString::sprintf("%s%s", FileName, INFOFILESUFFIX ".txt"));
  isHD = false;
  isCompressed = false;
#else
...
cRecording::cRecording(cTimer *Timer, const cEvent *Event)
{
  ...
  name = NULL;
#ifdef REELVDR
  baseDir = strdup(cVideoDirectory::Name());
#endif
...
#ifdef REELVDR
void cRecording::SetTitle(const char *Title, const char *ShortText)
{
  info->SetTitle(Title);
  info->SetShortText(ShortText);
}
#endif
...
#ifdef REELVDR
void cRecording::ResetResumeToNew(void) const
{
  resume = -1;
}
#endif
...
bool cRecording::Rename(const char *newName)
{
  ...
  int ri = isPesRecording ? lifetime : instanceId;
#ifdef REELVDR
  char *newFileName = strdup(cString::sprintf(fmt, /* VideoDirectory */  (const char *)Folder() , localNewName, t->tm_year + 1900, t->tm_mon + 1, t->tm_mday, t->tm_hour, t->tm_min, ch, ri));
#else
  ...
  if (strcmp(FileName(), newFileName)) {
     if (access(newFileName, F_OK) == 0) {
        ...
        if (result)
#ifdef REELVDR
           result = cVideoDirectory::RenameVideoFile(FileName(), newFileName);
#else
