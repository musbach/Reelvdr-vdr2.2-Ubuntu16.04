...
#endif /*REELVDR*/
#ifdef USE_BOUQUETSXXX
  int Modulation(void) const { return false /*modulation*/ ; } // TODO: correct modulation
  bool IsRadioChannel(void) const { return vpid == 0 || vpid == 1; }
  //bool IsEncryptedRadioChannel(void) const { return vpid == 1; } 
  bool IsEncryptedRadioChannel(void) const { return IsRadioChannel() && Ca() > 0;}
  bool IsEncryptedVideoChannel(void) const { return !IsRadioChannel() && Ca() > 0;}
  bool IsHDChannel(void) const { return Modulation() == 10 && IsSat(); }
#endif /*USE_BOUQUETS*/
...
#ifdef REELVDR
  void ForceCaIds(const int *CaIds); // list must be zero-terminated
#endif 
