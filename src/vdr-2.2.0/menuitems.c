...
// --- cMenuEditStrItem ------------------------------------------------------
#if REELVDR
cMenuEditStrItem::cMenuEditStrItem(const char *Name, char *Value, int Length, const char *Allowed, const char* PlaceHolderText)
#else
...
:cMenuEditItem(Name)
{
  ...
  length = Length;
#if REELVDR
  placeHolderText = PlaceHolderText?PlaceHolderText:""; // let defaultValue not be null
  if (Allowed)
     allowed = strdup(Allowed);
  else { //TB: Hack: "!" should be selectable, but should not be within "FileNameChars"

      // bytes required for tr(FileNameChars)
      int lenBytes = sizeof(char)*(strlen(tr(FileNameChars)) + 1); /* +1 for '\0'*/

      allowed = (char*) malloc(lenBytes + 1*sizeof(char)); /* 1 more char '!' */

      Utf8Strn0Cpy(allowed, tr(FileNameChars), lenBytes);
      strcat(allowed, "!");
  }
#else
...
#ifdef REELVDR
     const char *charMap = tr("CharMap$ 0@+-_\t.?!,1#~\\^$[]|()*{}/:%&\tabc2\tdef3\tghi4\tjkl5\tmno6\tpqrs7\ttuv8\twxyz9");
     //const char *charMap = tr("CharMap$ 0@+-_\t.?!,1#~\\^$[]|()*{}/:%&\tabc2\tdef3\tghi4\tjkl5\tmno6\tpqrs7\ttuv8\twxyz9");
#else
...
void cMenuEditStrItem::SetHelpKeys(void)
{
  if (InEditMode())
#if REELVDR
     SetHelp( uppercase ? tr("Button$ABC/abc") : tr("Button$ABC/abc"), insert ? tr("Button$Overwrite") : tr("Button$Insert"), tr("Button$Delete"));
#else
...
{
#ifdef REELVDR
  SetTitle(cString::sprintf("%s: %s", tr("Settings"), Section));
#else
...
eOSState cMenuSetupPage::ProcessKey(eKeys Key)
{
  ...
  if (state == osUnknown) {
     switch (Key) {
       case kOk: Store();
#ifdef REELVDR
                 Skins.Message(mtInfo, tr("Changes applied"),1);
#endif
                 state = osBack;
                 break;
       default: break;
...
//   cPluginManager::GetPlugin(PluginName)->SetupStore(Name, PluginName, Value);
     cPluginManager::GetPlugin(PluginName)->SetupStore(PluginName);
}
#endif /* REELVDR */
