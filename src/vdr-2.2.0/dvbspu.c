...
#ifdef SPUSPUDEBUG
#define SPUDEBUG(format, args...) printf (format, ## args)
#else
#define SPUDEBUG(format, args...)
#endif
...
void cDvbSpuDecoder::SetSpuScaling(void)
{
    ...
    SPUDEBUG("dvbspu SetSpuScaling OsdSize %d x %d\n", OsdWidth, OsdHeight);
    ...
    SPUDEBUG("dvbspu xscaling = %f yscaling = %f\n", xscaling, yscaling);
}
                                  ...
                                  uint32_t palette)
{
    ...
    if (ne) {
        SPUDEBUG("setHighlight: %d,%d x %d,%d\n", sx, sy, ex, ey);
        hlpsize.x1 = sx;
        hlpsize.y1 = sy;
        hlpsize.x2 = ex;
        hlpsize.y2 = ey;
...
void cDvbSpuDecoder::Draw(void)
{
    ...
    if (osd == NULL) {
            ...
            if (osd->CanHandleAreas(&Area, 1) != oeOk) {
                SPUDEBUG("dvbspu CanHandleAreas (%d,%d)x(%d,%d), 4 failed\n", areaSize.x1, areaSize.y1, areaSize.x2, areaSize.y2);
                restricted_osd = true;
            }
            else
                osd->SetAreas(&Area, 1);
...
int cDvbSpuDecoder::setTime(uint32_t pts)
{
    ...
    while (DCSQ_offset != prev_DCSQ_offset) {   /* Display Control Sequences */
        ...
        if ((pts != 0) && (exec_time > pts))
            return 0;
        SPUDEBUG("offs = %d, rel = %d, time = %d, pts = %d, diff = %d\n",
        ...
        if (pts != 0) {
            ...
            SPUDEBUG("offs = %d, DCSQ = %d, prev_DCSQ = %d\n",
            ...
            while (spu[i] != CMD_SPU_EOF) {     // Command Sequence
                ...
                case CMD_SPU_SHOW:     // show subpicture
                    SPUDEBUG("\tshow subpicture\n");
                    state = spSHOW;
                    i++;
                    break;

                case CMD_SPU_HIDE:     // hide subpicture
                    SPUDEBUG("\thide subpicture\n");
                    state = spHIDE;
                    i++;
                    break;
