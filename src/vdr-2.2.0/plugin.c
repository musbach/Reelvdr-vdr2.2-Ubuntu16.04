...
bool cPluginManager::CallAllServices(const char *Id, void *Data)
{
  ...
  int i=0;
  i=0;
  printf("plugin 535 %i: Id: %s Data: %p - %i \n", i, Id, Data, (int)Data);
  if (pluginManager) {
     for (cDll *dll = pluginManager->dlls.First(); dll; dll = pluginManager->dlls.Next(dll)) {
        cPlugin *p = dll->Plugin();
	i=i+1;
//	printf("------ 544 %i: ddl: %p fileName: %s args: %s \n", i, dll, dll->fileName, dll->args);
//	printf("plugin 545 %i: *p = dll->Plugin(): %p - %i \n",i, dll->Plugin(),(int)dll->Plugin());
	printf("plugin 547 %i: p: %p p->Service(Id, Data): %s %d fileName: %s args: %s \n",i, p,p->Service(Id, Data)?"true":"false",p->Service(Id, Data),dll->fileName,dll->args);
        if (p && p->Service(Id, Data)){
//        if (p){
//        if (p->Service(Id, Data)){
            found = true;
	    printf("plugin 550 -> FOUND!!!!!!!!!!!! \n");
        }
//	printf("plugin 553 %i: found: %i - %s \n", i, found, found ? "true" : "false");
    }
  }
  
  if (!found)
      esyslog("CallAllServices('%s', %p) NOT handled by any plugin", Id, Data);
