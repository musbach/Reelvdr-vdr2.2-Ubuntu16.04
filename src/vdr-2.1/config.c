     ...
     for (cSetupLine *l = First(); l; l = Next(l)) {
         ...
         if (l->Plugin()) {
            ...
            if (p && !p->SetupParse(l->Name(), l->Value()))
               error = true;
else dsyslog("config parameter: %s%s%s = %s", l->Plugin() ? l->Plugin() : "", l->Plugin() ? "." : "", l->Name(), l->Value());
            }
         else {
            if (!Parse(l->Name(), l->Value()))
               error = true;
else dsyslog("config parameter: %s = %s", l->Name(), l->Value());
            }
         if (error) {
#if REELVDR
...
#endif /* LIRCSETTINGS */
  else if (!strcasecmp(Name, "LastReplayed"))        cReplayControl::SetRecording(Value, NULL);
