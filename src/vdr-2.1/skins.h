...
public:
  ...
  static bool HasKey(const eKeys *Keys, eKeys Key) {
       while(Keys && (*Keys != kNone))
...
public:
  ...
  virtual void SideNote(const cRecording *Rec);
...
public:
  ...
  virtual void SetMessage(eMessageType Type, const char *Text) = 0;
       ...
       ///< any previous contents overwritten by the message must be restored.
#ifdef REELVDR
  virtual void SetHelp(const char* r = NULL, const char* g=NULL, const char* y=NULL, const char *b = NULL);
#endif
  };
...
#ifdef REELVDR
  eKeys ProcessQueuedMessages(void);
       ///< Processes the first queued message, if any.
       ///< Returns pressed key if a key is pressed while displaying queued message.
#else
