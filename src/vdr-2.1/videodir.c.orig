...
int BufferDirectoryMinFree  = 0; /*MB, 0 = default cRecorder handling*/
...
//  esyslog("ERROR INFO: %s not in %s", FileName, Name());
  esyslog("Register INFO: Filename %s", FileName);
  esyslog("Register INFO: Name() %s", Name());
  const char *strstrFileName;
  strstrFileName = strstr(FileName, Name()); 
  esyslog("Register INFO: strstrFileName %s", strstrFileName);
...
cUnbufferedFile *cVideoDirectory::OpenVideoFile(const char *FileName, int Flags)
{
  ...
  esyslog("OPEN INFO: FileName: %s Current(): %s", FileName, Current()); 
  esyslog("OPEN INFO: FileName: %s Current(): %d", FileName, Current()); 
//  esyslog("OPEN INFO: Current(): %s", Current());
  if (Current()->Register(FileName)){
//     return cUnbufferedFile::Create(FileName, Flags, DEFFILEMODE);
     esyslog("OPEN INFO: before Create FileName/DEFFILEMODE: %s/%d", FileName, DEFFILEMODE); 
     cUnbufferedFile *File = cUnbufferedFile::Create(FileName, Flags, DEFFILEMODE);
     return File;
  }
  return NULL;
}
