...
public:
          ///< Reel extension
          ///< Gets various attributes of device (like names, reception values, ...)
          ///< Returns -1 when attribute or type not available
  virtual int GetAttribute(const char *attr_name, uint64_t *val) { return -1;}
  virtual int GetAttribute(const char *attr_name, char *val, int maxret) {return -1;}
...
#ifdef REELVDR
  virtual bool NeedDelayedTrickmode() { return false; }
       ///< If true the player is responible for single frame trickmode timing
  virtual bool NeedTrickSpeedDir() { return false; }
       ///< If true backwards speeds will be negative in TrickSpeed
  virtual bool NeedTSCheck() { return false; }
       ///< If true also check for discontinue, scrambled, ...
  virtual bool NeedTSCheckEOS() { return false; }
       ///< If true send EOS nal on ts check errors
  virtual void OverflowDetected() {}
       ///< Allow output to resync to new buffer state
#endif /*REELVDR*/
...
#if REELVDR  // for menu key during DVD playpack
  bool PlayerCanHandleMenuCalls();
  bool DVDPlayerIsInMenuDomain();
#endif
  };
