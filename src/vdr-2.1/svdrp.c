  ...
  if (*Option) {
     ...
     if (FileName) {
        if (grabImageDir) {
           ...
           const char *allowed_base_dir = "/media/reel/";
           if(strncmp(FileName, allowed_base_dir, strlen(allowed_base_dir))) {
              Reply(550, "Grabbing to file other that \"%s\" not allowed (use \"GRAB -\" instead)", allowed_base_dir);
              return;
              }
           }
#else
...
#if REELVDR && 0  // Use QueueMessage(), else vdr can crashes
     /*
     * Call to Skins.Message() from here sets up an indirect recurssion  which can accept 
     * another svdrp command (Interface->GetKey()). So, if two messages come 
     * in quick succession, the first message
     * may destroy the "osd" after timeout while the second message is using the same osd
     * ==> segfault; vdr crash
     */
     Skins.Message(mtInfo, Option);
     Reply(250, "Message accepted");  //TODO: queue message if in standby?
  }
#else
