...
#if 1
         /// DONOT make reelvdr exit because of a plugin
         if (!p->Start()) {
             esyslog("%s:%d starting plugin %s FAILED; removing it from dll list", __FILE__, __LINE__, p->Name());
             cDll *prev = dlls.Prev(dll);
             dlls.Del(dll, true);
             dll = prev;
             continue;
         }
...
bool cPluginManager::CallAllServices(const char *Id, void *Data)
{
  ...
  if (pluginManager) {
     for (cDll *dll = pluginManager->dlls.First(); dll; dll = pluginManager->dlls.Next(dll)) {
         cPlugin *p = dll->Plugin();
         if (p  && p->Name() != NULL && p->Service(Id, Data)) {
//         if (p  && p->Name() != "" && p->Service(Id, Data)) {
//         if (p  && p->Name() > 0 && p->Service(Id, Data)) {
            found = true;
            isyslog("'%s' plugin handled CallAllServices('%s', %p)", p->Name(), Id, Data);
            }
         }
     }
