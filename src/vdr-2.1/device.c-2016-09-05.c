  ...
  for (int j = 0; j < NumCamSlots || !NumUsableSlots; j++) {
      ...
      for (int i = 0; i < numDevices; i++) {
          ...
          if (device[i]->ProvidesChannel(Channel, Priority, &ndr)) { // this device is basicly able to do the job
             ...
             {
                imp <<= 8; imp |= device[i]->Priority() - IDLEPRIORITY;                       // use the device with the lowest priority (+MAXPRIORITY to assure that values -99..99 can be used)
             } else {
                imp <<= 8; imp |= (max(device[i]->Priority(), badPriority) - IDLEPRIORITY;          // use the device with the lowest priority (+MAXPRIORITY to assure that values -99..99 can be used)
             }
#else
...
#define TS_SCRAMBLING_TIME_OK    30 // seconds before a Channel/CAM combination is marked as known to decrypt
...
bool cDevice::AttachReceiver(cReceiver *Receiver)
{
  ...
  if (Receiver->device == this)
     return true;
#ifdef REELVDR
  Receiver->Detach(); // Make sure we detach an already attached receiver
#endif /*REELVDR*/
