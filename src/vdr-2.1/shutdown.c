...
#endif
     SetUserInactiveTimeout(0, true); //satelit hack changed from -3 to 0
     }
}
...
bool cShutdownHandler::RequestNewMode(eActiveMode mode)
{
    ...
    {
        dsyslog("Requested mode (%d) is already active. Doing nothing", mode);
        return false;
    }
...
bool cShutdownHandler::ActOnActiveMode()
{
    ...
    dsyslog("ActOnActiveMode(): Active mode(%d) changed to requestedMode (%d)", ActiveMode, requestedMode);

    // check for background activity
    if (ActiveMode == active && requestedMode == poweroff && !ShutdownHandler.ConfirmShutdown(false))
    {
	dsyslog("There are background activities. Changing to standby first");
	ActiveMode = standby;
    }
    else
    ...
    */
    ...
    dsyslog("Acting on  new Active mode %d", ActiveMode);
    switch (ActiveMode)
    {
        case active:
	    ...
	    {
               Setup.standbyCountdown.Set(Setup.StandbyTimeout*60*1000); //(Ms) after which go into poweroff
	    }
            else
               Setup.standbyCountdown.Set(INT_MAX);
